-- Active-user gate at top of start_quiz_session and batch_submit_quiz (root cause fix for CR #599 comments 1+3)
-- Replaces scattered deleted_at filters with explicit "user not found or inactive" exception when caller's
-- users row is missing or soft-deleted. Defense-in-depth filters lower in batch_submit_quiz preserved.
