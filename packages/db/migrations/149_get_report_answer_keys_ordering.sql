-- Migration 149: get_report_answer_keys reveals the `ordering` canonical sequence
-- in the post-session report (VFR RT Training Phase 5 — #697). Body is mig 133's
-- with ONE added RETURN QUERY branch for ordering; every guard + existing branch
-- is re-emitted verbatim. RETURNS TABLE unchanged → CREATE OR REPLACE (no DROP).
--
-- ordering: ONE row PER SLOT — answer_key = the item TEXT that belongs at that
-- slot (canonical = the stored array order of questions.ordering_items), blank_index
-- = the 0-based slot. WITH ORDINALITY gives a 1-based idx → blank_index = idx - 1 to
-- match the per-slot blank_index the grader stores (mig 147). DISTINCT ON dedupes
-- repeated answer rows. ordering_items is REVOKE-gated from authenticated (mig 094
-- omission, N6); SECURITY DEFINER bypasses that — gated to the owning student's own
-- COMPLETED session, like the sibling branches.
--
