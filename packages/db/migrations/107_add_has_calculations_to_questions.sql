...
-- p_calc_mode — see mig 108).

ALTER TABLE questions ADD COLUMN has_calculations BOOLEAN NOT NULL DEFAULT false;

-- Column-level SELECT gate (packages/db 094 ≡ supabase 20260610000100) REVOKEd the blanket
-- SELECT from `authenticated` and re-GRANTed every column EXCEPT the four answer-key
-- columns. New columns are NOT covered by that grant, so authenticated cannot read
