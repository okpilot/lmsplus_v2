-- Migration 121: batch_submit_quiz — type-aware dispatcher for MC +
-- short_answer + dialog_fill (#697, Phase 2).
-- Changes vs mig 112b: (1) widened _batch_questions temp table; (2) duplicate
-- guard keyed on (question_id, blank_index); (3) per-answer loop dispatches to
-- _grade_record_mc/short_answer/dialog_fill helpers (mig 120); (4) score
-- aggregation uses DISTINCT-question roll-up (same as submit_vfr_rt_exam_answers
-- L229-252) so a 3-blank dialog_fill counts as one question, not 3 rows; (5) all
-- entry guards before the loop are unchanged.
