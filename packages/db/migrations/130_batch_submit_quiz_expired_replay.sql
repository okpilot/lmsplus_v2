-- Migration 130: batch_submit_quiz — restore the `expired` flag on idempotent
-- replay (#839, sibling parity with mig 129 / submit_vfr_rt_exam_answers).
--
-- Bug (identical to mig 129): the fresh timer-expiry path (L112-120 below)
-- RETURNs `{... expired:true}`, but the idempotent-replay branch (v_ended_at IS
-- NOT NULL, L85-104) recomputes from persisted rows and DROPS the flag — so a
-- RETRY of an expired submit returns a different payload shape than the original.
-- The quiz client reads `expired` (apps/web/app/app/quiz/types.ts).
--
-- Fix: in the replay branch, detect the expiry via the append-only mode-keyed
-- expired audit event ('exam.expired' for mock_exam, 'internal_exam.expired' for
-- internal_exam — mig 121 L112-113) and re-add `expired:true`. The audit lookup
-- also catches expiry via complete_overdue_/complete_empty_exam_session (mig 102,
-- same event_types). audit_events has no deleted_at (append-only) — no soft-delete
-- filter required.
--
-- VERBATIM from mig 121 (20260601-family dispatch redefinition); the ONLY changes:
--   (1) DECLARE v_replay_expired boolean := false;
--   (2) the EXISTS lookup inside the `IF v_ended_at IS NOT NULL` replay branch;
--   (3) the `|| CASE WHEN v_replay_expired ...` concat on that replay RETURN.
-- Depends on: mig 120 (helpers).
...
BEGIN
  ...
  -- NULL (one entry per question); dialog_fill has one entry per blank.
