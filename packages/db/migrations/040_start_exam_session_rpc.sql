-- start_exam_session: creates a mock_exam quiz session with randomly selected
-- questions based on the exam_config distribution for the given subject.
--
-- Parameters:
--   p_subject_id: the EASA subject to start an exam for
--
-- Returns: JSON object with session_id and question_ids
--
...
BEGIN
  ...
  LOOP
    -- Select random questions matching this distribution row
    v_topic_ids := ARRAY(
      SELECT q.id
      FROM questions q
      WHERE q.subject_id = p_subject_id
        ...
        AND q.id != ALL(v_selected_ids)  -- prevent duplicates across distributions
      ORDER BY random()
