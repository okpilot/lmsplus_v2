-- Fix security.md rule 10 violation in batch_submit_quiz: audit_events INSERT
-- subqueries populating actor_role from `users` lacked `AND deleted_at IS NULL`.
-- Affected lines in 048_batch_submit_softdelete_regression.sql: 103 (exam.expired
-- branch) and 277 (exam.completed / quiz_session.batch_submitted branch).
-- Tracks GitHub issue #550. Body identical to mig 048 apart from the two filters.
