-- 041_batch_submit_bulk_fetch.sql
-- Fix N+1 query: bulk-fetch all questions before the answer loop.
--
-- Previously, each answer triggered a separate SELECT FROM questions.
-- Now we CREATE TEMP TABLE _batch_questions with a single query before
-- the loop, then look up from the temp table inside the loop.
--
-- Changes vs migration 037:
--   - Added CREATE TEMP TABLE _batch_questions before the loop
--   - Loop SELECT now reads from _batch_questions instead of questions
