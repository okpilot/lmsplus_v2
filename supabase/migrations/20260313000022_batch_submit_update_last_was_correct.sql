-- 022_batch_submit_update_last_was_correct.sql
-- Fix: batch_submit_quiz now updates fsrs_cards.last_was_correct atomically
-- within the transaction, eliminating the window between RPC commit and the
-- TypeScript FSRS update where the incorrect-filter counter could read stale data.
--
-- The full FSRS scheduling (due, stability, difficulty, etc.) continues to be
-- handled by the application layer (updateFsrsCard in lib/fsrs/update-card.ts).
-- This RPC only ensures last_was_correct is immediately correct.
