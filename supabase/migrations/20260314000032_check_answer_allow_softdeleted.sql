-- 032_check_answer_allow_softdeleted.sql
-- Fix: remove deleted_at IS NULL filter from question lookup in check_quiz_answer.
-- Same reasoning as migration 031 (batch_submit_quiz): session membership was
-- validated against config.question_ids (a snapshot locked at session start).
