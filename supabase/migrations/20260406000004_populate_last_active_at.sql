-- 004_populate_last_active_at.sql
-- Fix #479: users.last_active_at was never written anywhere.
-- 1. Backfill from quiz_sessions.ended_at (most recent completed session per student)
-- 2. Redefine complete_quiz_session to stamp last_active_at on completion
-- 3. Redefine batch_submit_quiz to stamp last_active_at on completion (active production path)

-- Backfill existing data
UPDATE users
SET last_active_at = sub.latest_ended
FROM (
  SELECT student_id, MAX(ended_at) AS latest_ended
  FROM quiz_sessions
  WHERE ended_at IS NOT NULL
    AND deleted_at IS NULL
  GROUP BY student_id
) sub
WHERE users.id = sub.student_id
  AND users.deleted_at IS NULL;

-- Redefine complete_quiz_session with last_active_at stamp
...
-- Redefine batch_submit_quiz with last_active_at stamp (active production path)
-- Only change vs migration 041: added UPDATE users SET last_active_at after session completion
...
BEGIN
  ...
  -- Fetch session row (allow already-completed sessions for idempotent replay)
  -- FOR UPDATE serializes concurrent retries so the second caller sees v_ended_at IS NOT NULL
  -- and takes the replay path instead of double-writing.
