-- 020_fix_student_responses_unique.sql
-- Add UNIQUE (session_id, question_id) to student_responses so that
...
-- Step 3: Re-create batch_submit_quiz with the explicit ON CONFLICT target so the
-- constraint is used properly and the idempotency guarantee is real.
...
BEGIN
  ...
  LOOP
    ...
    -- Insert to immutable response log (idempotent — explicit conflict target now enforced by unique constraint)
