-- 026_batch_submit_field_validation.sql
-- Fix: harden batch_submit_quiz field validation (CodeRabbit issues #33 and #38).
-- 1. (#33) Split session SELECT into two steps: fetch config first, then validate
--    jsonb_typeof(v_config->'question_ids') = 'array' BEFORE calling
--    jsonb_array_elements_text. The old single-SELECT approach evaluated
--    jsonb_array_elements_text inline, so it could throw before the guard ran.
-- 2. (#38) Validate v_selected_option and v_response_time inside the answer loop
--    after extraction, before any database operations.
...
BEGIN
  ...
  -- Extracting question_ids inline would throw before the guard if config is malformed.
  ...
  -- Step 2: Guard against malformed session config BEFORE extracting question_ids
  ...
  LOOP
    ...
    -- Validate answer fields before any database operations (#38)
    IF v_selected_option IS NULL OR v_selected_option = '' THEN
      RAISE EXCEPTION 'answer for question % has empty selected_option', v_question_id;
    END IF;
    IF v_response_time IS NULL OR v_response_time < 0 THEN
      RAISE EXCEPTION 'answer for question % has invalid response_time_ms', v_question_id;
    END IF;
