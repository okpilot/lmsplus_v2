-- calc-mode (#837): tri-state has_calculations filter for the student quiz builder.
--
-- Adds p_calc_mode {all|only|exclude} to the shared filtered-pool helper and both
-- wrapper RPCs (get_random_question_ids / get_filtered_question_counts). Unlike the
-- per-user p_filters (unseen/incorrect/flagged), which UNION/OR together, calc-mode
-- AND-restricts the pool (subtractive). 'only'/'exclude' are mutually exclusive by
-- construction (single enum on the app side).
--
-- Signature change: p_calc_mode is a NEW parameter, so the prior signatures must be
-- DROPPED before recreating — CREATE OR REPLACE cannot change a function's argument
-- list, and leaving the old overloads alive would make calls ambiguous. Drop the two
-- dependents before the shared helper.

DROP FUNCTION IF EXISTS public.get_random_question_ids(uuid, uuid[], uuid[], int, text[]);
DROP FUNCTION IF EXISTS public.get_filtered_question_counts(uuid, uuid[], uuid[], text[]);
DROP FUNCTION IF EXISTS public._filtered_question_pool(uuid, uuid[], uuid[], text[]);
...
AS $$
  SELECT p.id
  FROM public._filtered_question_pool(p_subject_id, p_topic_ids, p_subtopic_ids, p_filters, p_calc_mode) p
  ORDER BY random()
...
-- Grants
---------------------------------------------------------------------------
GRANT EXECUTE ON FUNCTION public._filtered_question_pool(uuid, uuid[], uuid[], text[], text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_random_question_ids(uuid, uuid[], uuid[], int, text[], text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_filtered_question_counts(uuid, uuid[], uuid[], text[], text) TO authenticated;

---------------------------------------------------------------------------
-- Comments
---------------------------------------------------------------------------
COMMENT ON FUNCTION public._filtered_question_pool(uuid, uuid[], uuid[], text[], text) IS
  'Internal: active, org-scoped, subject/topic/subtopic + per-user-filter (UNION) question pool, AND-restricted by p_calc_mode {all|only|exclude} on has_calculations (#837). SECURITY INVOKER; filters scope student_id = auth.uid() (§3). Used by get_random_question_ids and get_filtered_question_counts so count == quiz (#678/#679/#668). Prefer the wrapper RPCs over calling directly (direct calls may hit the PostgREST 1000-row cap).';

COMMENT ON FUNCTION public.get_random_question_ids(uuid, uuid[], uuid[], int, text[], text) IS
  'Up to LEAST(p_count, 500) random IDs from the filtered question pool. p_calc_mode {all|only|exclude} AND-restricts on has_calculations (#837). Server-side ORDER BY random() avoids the 1000-row cap that biased client-side sampling (#679/#668); the 500 cap (mirroring the Zod schema in start.ts) prevents a direct caller from bypassing the Server Action with an arbitrarily large p_count.';

COMMENT ON FUNCTION public.get_filtered_question_counts(uuid, uuid[], uuid[], text[], text) IS
  'Per-(topic, subtopic) counts over the filtered question pool. Total = sum(n). p_calc_mode {all|only|exclude} AND-restricts on has_calculations (#837). Replaces client-side counting that truncated at the 1000-row cap (#678/#668).';
