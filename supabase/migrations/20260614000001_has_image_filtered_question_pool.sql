...
COMMENT ON FUNCTION public.get_random_question_ids(uuid, uuid[], uuid[], int, text[], text, text) IS
  'Up to LEAST(COALESCE(p_count, 0), 500) random IDs from the filtered question pool (NULL p_count yields 0 rows). p_calc_mode {all|only|exclude} AND-restricts on has_calculations (#837); p_has_image {all|only|exclude} AND-restricts on question_image_url presence (#864). Server-side ORDER BY random() avoids the 1000-row cap that biased client-side sampling (#679/#668); the 500 cap (mirroring the Zod schema in start.ts) prevents a direct caller from bypassing the Server Action with a NULL or arbitrarily large p_count.';
