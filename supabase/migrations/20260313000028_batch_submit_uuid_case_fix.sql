-- 028_batch_submit_uuid_case_fix.sql
-- Fix: use case-insensitive regex (!~*) for UUID validation in batch_submit_quiz.
-- Uppercase UUIDs are valid per RFC 4122 but were rejected by the lowercase-only
-- regex in migration 027. Defense-in-depth: the guard should not reject valid input.
