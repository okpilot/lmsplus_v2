-- Migration 124: submit_vfr_rt_exam_answers coalesces its normalize_answer() calls
-- at the call site (#950, code-style.md §5(d) conformance). Defense-in-depth, NOT a
-- bug fix — the two short_answer / dialog_fill branches already guard
-- `v_response_text IS NULL` upstream (RAISE 'answer_type_mismatch'), so today
-- normalize_answer() is never called with NULL and v_norm is provably non-NULL.
--
-- But §5(d) prescribes coalescing the helper result AT THE CALL SITE rather than
-- relying on a distant guard, precisely so a future edit that removes/moves the guard
-- cannot let normalize_answer(NULL) → NULL propagate through `v_norm <> ''` (which is
-- NULL when v_norm is NULL) into the NOT NULL quiz_session_answers.is_correct /
-- student_responses.is_correct columns (23502 at execution). The mig 120 sibling
-- helpers (_grade_record_short_answer / _grade_record_dialog_fill) already coalesce;
-- this brings submit_vfr_rt_exam_answers into the same posture (rule's
-- sibling-consistency clause). The #950 audit confirmed this is the only §5(d)-divergent
-- site repo-wide; zero §5(c)/(d) bugs were found.
--
-- Based on the latest definition (mig 113, 20260619000300); the ONLY change is wrapping
-- the two `v_norm := normalize_answer(v_response_text)` assignments in
-- `coalesce(..., '')`. Every other line — auth, active-user gate, ownership scope,
-- timer-expiry, membership, per-type grading, idempotent replay, scoring, audit — is
-- byte-identical to mig 113. coalesce(non_null, '') === non_null, so behavior is
-- unchanged for every current call.
