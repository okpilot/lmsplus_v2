-- 025_batch_submit_input_validation.sql
-- Fix: harden batch_submit_quiz input validation.
-- 1. Validate p_answers is a non-null JSON array before jsonb_array_length.
-- 2. Reject duplicate question_id entries in payload.
-- 3. Validate submitted question_ids belong to the session (membership check).
