-- Fix #487: include soft-deleted students in get_admin_student_stats
-- so that the "All" student view shows historical stats for inactive users.
-- Previously AND u.deleted_at IS NULL in the final WHERE excluded inactive users,
-- causing zero stats on the dashboard.
...
BEGIN
  ...
  -- Resolve caller's org (admin's own profile must be active)
