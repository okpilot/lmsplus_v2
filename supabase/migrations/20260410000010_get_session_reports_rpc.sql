-- RPC: get_session_reports
-- Returns paginated, sorted session reports with subject name join.
-- Enables alphabetical subject-name sorting (PostgREST cannot join-sort).
...
DECLARE
  ...
  v_dir  TEXT;
BEGIN
  -- Auth check (SECURITY DEFINER bypasses RLS)
  v_uid := auth.uid();
