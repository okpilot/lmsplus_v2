-- 019_check_answer_rpc.sql
-- Replace client-side direct SELECT on questions.options (which exposes correct flag)
-- with a SECURITY DEFINER RPC that computes correctness server-side.
-- The RPC verifies auth.uid(), looks up the correct option server-side, and returns
-- only the derived result — never the raw options array with correct flags.
