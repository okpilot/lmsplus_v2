-- Migration 148: batch_submit_quiz dispatches the `ordering` question type
-- (VFR RT Training Phase 5 — #697). Body is mig 130/132's, with three additions:
--   (1) ordering_items fetched into the _batch_questions temp table + a local var;
--   (2) an `ordering` dispatch branch calling _grade_record_ordering (mig 147)
--       once per slot (the client fans out one entry per slot, item id in
--       selected_option, slot in blank_index — no new field extraction);
--   (3) the total_blanks CASE gains an `ordering` branch
--       (greatest(jsonb_array_length(ordering_items), 1)) so per-slot rows fold
--       into partial credit through the EXISTING DISTINCT-question rollup.
--
-- DEVIATION NOTE (spec N7 / Decision 51): N7 specified a single JSON row per
-- ordering question, but a single binary is_correct row cannot yield partial
-- credit through the row-counting rollup below, while Decision 47's partial-credit
-- model (confirmed by the user 2026-06-24 for Part 3) requires it. So ordering
-- stores PER-SLOT rows exactly like dialog_fill — reusing the proven rollup with
-- only a total_blanks CASE branch. The mig-131 trigger was widened (mig 144) to
-- admit ordering's non-null blank_index.
--
-- ROLLUP INDEPENDENCE: in the aggregation below, v_correct_count (sum correct_rows,
-- the stored integer item count) and v_correct_credit (sum LEAST(correct_rows/
-- total_blanks, 1), the score numerator) are independent SELECT-list exprs over
-- the same join; the ordering total_blanks branch feeds ONLY the credit
-- denominator. v_correct_count and the mock/internal-exam v_total denominator are
-- unchanged. CREATE OR REPLACE (signature unchanged).
...
DECLARE
  ...
  v_total           int;
  v_answered        int;
  v_correct_count   int;     -- correct item (blank/slot-row) count (integer; stored column)
...
BEGIN
  ...
  -- NULL (one entry per question); dialog_fill + ordering have one entry per blank/slot.
  -- Canonicalize "1"/"01" → same int string (matches insert-time ::int cast).
  IF (
  ...
  LOOP
    ...
    SELECT bq.question_type, bq.correct_option, bq.canonical_answer,
           bq.accepted_synonyms, bq.blanks_config, bq.ordering_items,
