-- 031_batch_submit_idempotent_softdelete.sql
-- Fix 1: Idempotent retry — if session already completed, return existing results
--        instead of raising 'session not found or already completed'
-- Fix 2: Soft-delete mid-quiz — allow scoring a question that was soft-deleted
--        after the quiz session started (membership already validated)
