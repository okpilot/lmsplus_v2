-- Migration 129: submit_vfr_rt_exam_answers — restore the `expired` flag on
-- idempotent replay (#839).
--
-- Bug: the fresh timer-expiry path (L86-101 below) RETURNs `{... expired:true}`,
-- but the idempotent-replay branch (v_already_ended) recomputes from persisted
-- rows and DROPS the flag — so a RETRY of an expired submit returns a different
-- payload shape than the original response. Phase B clients read `expired`.
--
-- Fix: in the replay branch, detect the expiry via the append-only
-- `vfr_rt_exam.expired` audit event and re-add `expired:true` to the return.
-- An audit lookup (not an inline timer re-check or a stored row flag) is
-- deliberate: it reflects ANY expiry origin — the in-RPC timer path here AND the
-- complete_overdue_exam_session / complete_empty_exam_session helpers (mig 102),
-- which emit the same event_type. audit_events has no deleted_at (append-only),
-- so no soft-delete filter is required (docs/security.md §9 is N/A here).
--
-- VERBATIM from mig 124 (20260623000300); the ONLY changes are:
--   (1) DECLARE v_replay_expired boolean := false;
--   (2) the EXISTS lookup inside `IF v_already_ended THEN`;
--   (3) the `|| CASE WHEN v_replay_expired ...` concat on the final RETURN.
