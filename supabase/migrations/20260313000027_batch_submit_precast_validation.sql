-- 027_batch_submit_precast_validation.sql
-- Fix: validate text format BEFORE casting to uuid/int in batch_submit_quiz.
-- 1. Duplicate-check now compares text (e->>'question_id') instead of casting
--    to uuid first, preventing "invalid input syntax for type uuid" on
--    malformed payloads.
-- 2. Per-answer loop extracts question_id and response_time_ms as text,
--    validates format with regex, THEN casts. This gives clear error messages
--    instead of raw Postgres cast errors.
