...
- [ ] Pre-push review gate loop on the branch diff (CR-local is a member of round 1 only) → learner → red-team if triggered → coderabbit-sync if triggered → /fullpush → push → open PR (DO NOT MERGE)
