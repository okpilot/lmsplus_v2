...
- **Algorithm:**
  ...
  2. **Idempotency guard:** `SELECT count(*) FROM student_responses WHERE student_id = victim AND response_time_ms = SENTINEL` (`SENTINEL = 987654`). If `count === 8`, skip the insert (re-derive `subjectIds` from the existing sentinel rows joined to `questions`, then return).
