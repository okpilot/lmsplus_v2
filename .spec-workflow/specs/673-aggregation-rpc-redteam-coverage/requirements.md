...
3. WHEN the victim-response seeder runs AND at least 8 sentinel-marked rows already exist for the victim THEN it SHALL make no further inserts (idempotent — the guard is `>= 8`, so a partial-then-full prior run that left 9–15 rows still short-circuits); otherwise it SHALL (re)insert the full deterministic set of 8 rows.
