...
- [ ] A1. Create `exam_configs` + `exam_config_distributions` tables with RLS
...
- [ ] A2. Add `time_limit_seconds` and `passed` columns to `quiz_sessions`
...
- [ ] A3. Create `start_exam_session` RPC
...
- [ ] A4. Add exam guard to `batch_submit_quiz` + `passed` computation
...
- [ ] B1. Add nav item + icon for Exam Config
...
- [ ] B2. Create admin exam-config page structure
...
- [ ] B3. Create exam config Server Actions
...
- [ ] B4. Create exam config components
