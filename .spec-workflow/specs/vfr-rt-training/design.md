...
6. ~~Whether RT should also be hidden from Dashboard/Progress subject lists (Phase 1 — R1.3 scopes exclusion to the picker; confirm).~~ **CLOSED 2026-08-11: RT stays VISIBLE on Dashboard and Progress.** VFR RT is a required part of the Slovenia PPL(A) path, so an unpractised RT subject is a genuine gap a student should see; the exclusion is picker-only because `/app/vfr-rt` owns RT setup (part-based topics), and `NAV_ITEMS` already routes there. Consequences, all accepted as correct rather than regressions: `get_admin_dashboard_kpis` gains a 0%-mastery RT row per student, so `weakestSubject` will normally read RT (it is the only 0% subject; the `ORDER BY sa.avg_mastery ASC LIMIT 1` has no tiebreaker, so a tie with another 0% subject resolves arbitrarily) and `examReadyStudents` (`HAVING BOOL_AND(mastery >= 90)`) reads 0 until students practise RT; `avgMastery` falls as the org-wide denominator grows. Mastery does move — `get_student_mastery_stats` filters on neither `question_type` nor `bank_id`, and `_grade_record_short_answer` writes the `student_responses` rows that feed the numerator.
