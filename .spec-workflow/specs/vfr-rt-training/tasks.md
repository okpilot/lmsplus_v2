...
- [x] **4.1 Post-session canonical delivery (SECURITY DEFINER) + query extension** _(per N1, N4, N10)_ — DECISION: NEW RPC `get_report_answer_keys` (mig 133), not extending `get_report_correct_options` (MC path untouched). Commit cb10fc02.
  ...
  - _Requirements: R5.4_
- [x] **4.2 Report types + builder + `isAnswered` fix + pagination** _(per N7)_ — commit e92d90ee. Discriminator sourced from the `questions` row (NOT `quiz_session_answers`, which has no `question_type` col); pagination = pages-of-questions on distinct `question_id`; correct_count→items (mig 132) + item-level header.
...
- [x] **4.3 Report row dispatch + sub-renderers (+ tests)** — `report-question-row.tsx` dispatch via `ReportAnswerBody`; keep `OptionsList` (MC); NEW `ShortAnswerReport`, `DialogFillReport` (3-state partial display) (+ tests). Commit e92d90ee. _Requirements: R5.2, R5.3_
