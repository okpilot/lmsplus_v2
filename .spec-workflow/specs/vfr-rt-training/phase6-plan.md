...
- `diagram-label-input.tsx` NEW — @dnd-kit/core DndContext (clone sensors verbatim: PointerSensor, TouchSensor{delay:250,tolerance:5}, KeyboardSensor), `collisionDetection={closestCenter}` (keyboard-accessible — matches OrderingInput; `pointerWithin` was the original clone choice but leaves the registered KeyboardSensor with no `over` target, so keyboard-only users can't drop — fixed per CR on PR #1059); useDroppable zones + useDraggable chips; SVG underlay; %-positioned zone boxes; consume-on-place pool; submit→mapping[]. Extract zone-box/chip/pool subcomponents + helpers for 150-line cap.
