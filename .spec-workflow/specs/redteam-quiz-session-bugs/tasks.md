...
- [ ] T2.8 — Pre-push review gate on the branch diff: round 1 = implementation-critic + code-reviewer + semantic-reviewer + doc-updater + test-writer + deletion-reviewer + `code-review (skill)` — the built-in `/code-review` skill, dispatched as a subagent in an isolated worktree on opus; round 2+ = code-reviewer + semantic-reviewer + `code-review (skill)`. Pool each round's findings into ONE fixup commit; stop on the first round with no APPLY-worthy finding, ceiling 3.
