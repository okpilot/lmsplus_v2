...
- [ ] T2.8 — Pre-push review gate on the branch diff: round 1 = implementation-critic + code-reviewer + semantic-reviewer + doc-updater + test-writer + CR-local; round 2+ = code-reviewer + semantic-reviewer (CR-local is round 1 only). Pool each round's findings into ONE fixup commit; stop on the first round with no APPLY-worthy finding, ceiling 3.
