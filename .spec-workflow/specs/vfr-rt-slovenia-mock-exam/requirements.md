...
- **File size limits**: the caps are data in `.claude/limits.json`, enforced by
  `.claude/hooks/check-file-size-guard.mjs`. Read them there — any number written into this spec
  can diverge the moment a cap changes, so none is written here — the eight per-task `(≤ N)`
  annotations this line used to license were removed rather than disclaimed. "Indicative
  only" is not an exception the live-mirror rule grants, and a number beside a task reads as
  an instruction whatever the caveat three sections above says.
