...
- [ ] **A.2 Migration `084` — `quiz_session_answers` + `student_responses` schema shift for text responses & per-blank answers**
  ...
  - **Why two files, not one:** combining the schema changes (mig 084) and the two `CREATE OR REPLACE FUNCTION` bodies (mig 084b) into a single migration file would exceed the 300-line cap in `code-style.md` §1 (each RPC body is ~150 lines verbatim). `db push` applies pending migrations in timestamp order and records each in `supabase_migrations.schema_migrations`, but Supabase does not document cross-file transaction atomicity — so do NOT assume 084 and 084b apply as one rollback-safe unit. Both files must ship in the same PR/release so they deploy together; the residual risk (084 applied, 084b missing or failed mid-deploy) is mitigated two ways: (1) the A.2 acceptance test (`batch_submit_quiz` idempotency under the new constraint) catches a skipped/failed 084b at PR time, and (2) recovery is to re-run `db push` (it resumes from the unrecorded migration) or apply 084b manually and record its `schema_migrations` entry.
...
- [ ] **A.2b Migration `084b` — update `batch_submit_quiz` and `submit_quiz_answer` ON CONFLICT clauses for the new constraint**
  ...
  - `CREATE OR REPLACE FUNCTION batch_submit_quiz(...)` — locate the LATEST body via Pre-Flag Verification at implementation time (trace BOTH `packages/db/migrations/*.sql` AND `supabase/migrations/*.sql` for the most recent `CREATE OR REPLACE FUNCTION batch_submit_quiz` definition; note mig 082 / #611 may insert another revision before this spec's implementation begins). Copy that body verbatim, change only `ON CONFLICT (session_id, question_id)` to `ON CONFLICT (session_id, question_id, blank_index)`. Apply the same change to `submit_quiz_answer` after locating ITS LATEST via the same Pre-Flag Verification procedure.
