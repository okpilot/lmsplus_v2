...
- [ ] **E.3 Pre-push review gate**
  - Run `git fetch origin || abort`, then `git diff origin/master...HEAD -- . ':(exclude).claude/agent-memory' || abort`, per `agent-workflow.md § Pre-Push Review Gate`. Round 1 = implementation-critic + code-reviewer + semantic-reviewer + doc-updater + test-writer + CR-local; round 2+ = code-reviewer + semantic-reviewer (CR-local is round 1 only). Stop on the first round with no APPLY-worthy finding, ceiling 3.
