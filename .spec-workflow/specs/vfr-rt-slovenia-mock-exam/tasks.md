...
- [ ] **E.3 Pre-push review gate**
  - Run `git fetch origin || abort`, then `git diff origin/master...HEAD -- . ':(exclude).claude/agent-memory' || abort`, per `agent-workflow.md § Pre-Push Review Gate`. Round 1 = implementation-critic + code-reviewer + semantic-reviewer + doc-updater + test-writer + `code-review (skill)` — the built-in `/code-review` skill, dispatched forked, round 1 only; round 2+ = code-reviewer + semantic-reviewer. Stop on the first round with no APPLY-worthy finding, ceiling 3.
