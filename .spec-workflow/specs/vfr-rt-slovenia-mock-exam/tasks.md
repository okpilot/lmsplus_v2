...
- [x] **A.11 SQL integration tests for new RPCs + extended RPCs**
  ...
  - **Constraint migration regression test**: assert that submitting the same `batch_submit_quiz` payload twice (existing PPL `mock_exam` / `internal_exam` / `smart_review` / `quick_quiz` modes) returns DO NOTHING on the second call (no duplicate row) — guards against mig 095/095b/095c breaking existing answer-submission idempotency.
...
- [ ] **E.2 Red-team review + spec mapping**
  - The orchestrator invokes the red-team agent once per branch, after the pre-push review loop, when the BRANCH diff touches migrations / RLS / SECURITY DEFINER RPCs (see `.claude/rules/agent-red-team.md § Trigger Conditions`). The agent maps changes to existing redteam specs; the orchestrator files GitHub Issues for any coverage gaps the agent identifies.
...
- [ ] **E.3 Pre-push review gate**
  - Run `git fetch origin` (abort on failure), then the review loop over `git diff origin/master...HEAD -- . ':(exclude).claude/agent-memory'` per `agent-workflow.md § Pre-Push Review Gate`. Round 1 = implementation-critic + code-reviewer + semantic-reviewer + doc-updater + test-writer + CR-local; round 2+ = code-reviewer + semantic-reviewer + CR-local. Stop on the first round with no APPLY-worthy finding, ceiling 3.
...
**Estimated scope:** 14 migration files (094–103, including 094b, 095b, 095c and 099b) + 1 SQL test file + 5 Server Action files + 9 new component files + 1 E2E spec + helpers. Roughly mid-sized — comparable to internal-exam-mode.
