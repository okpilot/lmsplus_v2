...
- Pre-push review gate over the branch diff: round 1 = implementation-critic, code-reviewer, semantic-reviewer, doc-updater, test-writer, `code-review (skill)`; round 2+ = code-reviewer, semantic-reviewer, `code-review (skill)`. Then learner, then red-team, then coderabbit-sync if the branch diff changed a trigger file. fullpush gate. Merge only when CI fully green + CR no-change-requested.
