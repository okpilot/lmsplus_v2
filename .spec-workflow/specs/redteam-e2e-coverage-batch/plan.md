...
- Pre-push review gate over the branch diff: round 1 = implementation-critic, code-reviewer, semantic-reviewer, doc-updater, test-writer, CR-local; round 2+ = code-reviewer, semantic-reviewer, CR-local. Then learner, then red-team. fullpush gate. Merge only when CI fully green + CR no-change-requested.
