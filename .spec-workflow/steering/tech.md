...
- **Code review**: CodeRabbit and the CI reviewer `.github/workflows/claude-review.yml` (Decision 78) on PRs, + in-session Claude Code subagents. The GATE runs once per branch over `git diff origin/master...HEAD`; within it, round 1 runs all seven reviewers — implementation-critic, code-reviewer, semantic-reviewer, doc-updater, test-writer, deletion-reviewer, and `code-review (skill)`, the built-in `/code-review` skill, dispatched as `subagent_type: code-review-skill` in an isolated worktree on opus — and later rounds run code-reviewer + semantic-reviewer + deletion-reviewer + `code-review (skill)` (`agent-workflow.md § Pre-Push Review Gate`).
