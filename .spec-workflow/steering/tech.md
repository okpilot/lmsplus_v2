...
- **Code review**: CodeRabbit (automated on PRs) + in-session Claude Code subagents. The GATE runs once per branch over `git diff origin/master...HEAD -- . ':(exclude).claude/agent-memory'`; within it, round 1 runs all six reviewers and later rounds run code-reviewer + semantic-reviewer (CR-local is round 1 only) (`agent-workflow.md § Pre-Push Review Gate`).
