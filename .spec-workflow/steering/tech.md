...
- **Code review**: CodeRabbit (automated on PRs) + in-session Claude Code subagents. The GATE runs once per branch over `git diff origin/master...HEAD -- . ':(exclude).claude/agent-memory'`; within it, round 1 runs all six reviewers — implementation-critic, code-reviewer, semantic-reviewer, doc-updater, test-writer, and `code-review (skill)`, the built-in `/code-review` skill, dispatched forked, round 1 only — and later rounds run code-reviewer + semantic-reviewer (`agent-workflow.md § Pre-Push Review Gate`).
