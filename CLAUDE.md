...
- **pre-commit:** biome lint/format + type-check + soft-delete column guard (`check-soft-delete-guard.mjs`) + file-size ratchet (`check-file-size-guard.mjs`, limits and grandfathered baseline in `.claude/limits.json`) + test-title leakage guard (`check-test-title-leakage.mjs`, diff-scoped, grandfathered). Each also runs in the CI lint job with its unit suites — do not restate how many there are; `.claude/pipeline.json` `hooks` is the enumeration and `.claude/pipeline.test.mjs` fails if it and `lefthook.yml` disagree. Unit tests deliberately excluded — full suite runs in CI
- **commit-msg:** conventional commit format + retracted-phrase guard (`check-retracted-phrase.mjs`: a claim corrected in one corpus file must not still stand in another — `code-style.md` §10 cl.3; the sole escape hatch is a `Retracted-ok: <token> — <reason>` trailer, which is why this runs at commit-msg rather than pre-commit) + commit-claims guard (`check-commit-claims.mjs`): a SHA in commit-reference position must resolve. Coverage is partial and proves only that the commit EXISTS, never that the claim about it is true — see `code-style.md` §10 cl.6 for the rule and `docs/decisions.md` Decision 64 for the measured recall and its denominator
