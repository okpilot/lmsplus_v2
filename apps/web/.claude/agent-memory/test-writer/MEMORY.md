...
- **Per-step error accumulator in `afterAll`** — use `const errors: string[] = []` + per-step `try/catch`, then `throw new Error(errors.join('; '))` at the end. Dependent steps (FK ordering) additionally gate on `errors.length === 0`.
