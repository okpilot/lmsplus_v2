...
import { bulkUpdateCalculations } from './bulk-update-calculations'
...
describe('bulkUpdateCalculations', () => {
  describe('input validation', () => {
    it('returns failure when ids array is missing', async () => {
      const result = await bulkUpdateCalculations({ has_calculations: true })
    ...
    it('returns failure when ids contains a non-UUID value', async () => {
      const result = await bulkUpdateCalculations({ ids: ['not-a-uuid'], has_calculations: true })
      expect(result.success).toBe(false)
      if (result.success) return
      expect(result.error).toBe('Invalid input')
    })

    it('returns failure when has_calculations is missing', async () => {
      const result = await bulkUpdateCalculations({ ids: [UUID_1] })
      expect(result.success).toBe(false)
      if (result.success) return
      expect(result.error).toBe('Invalid input')
    })

    it('returns failure when has_calculations is not a boolean', async () => {
      const result = await bulkUpdateCalculations({ ids: [UUID_1], has_calculations: 'yes' })
      expect(result.success).toBe(false)
      if (result.success) return
      expect(result.error).toBe('Invalid input')
    })
    ...
    it('returns failure when input is not an object', async () => {
      const result = await bulkUpdateCalculations(null)
      expect(result.success).toBe(false)
      if (result.success) return
      expect(result.error).toBe('Invalid input')
    })
  ...
  describe('happy path', () => {
    it('marks the supplied questions as calculation and revalidates', async () => {
      ...
      expect(updateArg.has_calculations).toBe(true)
    ...
    it('unmarks the supplied questions as calculation and revalidates', async () => {
      const chain = mockAdminWithResult({ data: [{ id: UUID_1 }], error: null })

      const result = await bulkUpdateCalculations({ ids: [UUID_1], has_calculations: false })

      expect(result.success).toBe(true)
      const updateArg = chain.update.mock.calls[0]?.[0] as { has_calculations: boolean }
      expect(updateArg.has_calculations).toBe(false)
    })
  ...
  describe('error paths', () => {
    it('returns a generic error and logs the raw cause server-side when the DB update fails', async () => {
      ...
      const result = await bulkUpdateCalculations({ ids: [UUID_1], has_calculations: true })

      expect(result.success).toBe(false)
      if (result.success) return
      // Raw DB error must not reach the client (code-style.md §5 / OWASP A10).
      expect(result.error).toBe('Failed to update calculation tags')
      expect(errorSpy).toHaveBeenCalledWith('[bulkUpdateCalculations] DB error:', 'update failed')
  ...
  describe('auth guard', () => {
    it('propagates the auth error to the caller', async () => {
      ...
      await expect(
        bulkUpdateCalculations({ ids: [UUID_1], has_calculations: true }),
