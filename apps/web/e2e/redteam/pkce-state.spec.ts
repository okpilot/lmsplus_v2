 ...
 *          and the callback redirects to the verify page with an error.
 *
...
test.describe('Red Team: PKCE Code Forwarding', () => {
  test('rejects forged PKCE auth code and redirects away from dashboard', async ({ page }) => {
    ...
    // The callback route redirects to /auth/verify with an error param on failure.
  ...
  test('rejects callback with no code parameter', async ({ page }) => {
    ...
    // Must redirect to /auth/verify with error, not crash with a 500.
    await page.waitForURL('**/auth/verify*', { timeout: 10_000 })
    expect(page.url()).toContain('error=')
  })
