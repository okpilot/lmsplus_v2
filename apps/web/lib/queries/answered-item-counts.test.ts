...
describe('fetchAnsweredItemCounts', () => {
  ...
  it('pages the session filter and counts answer rows per session', async () => {
    ...
    const result = await fetchAnsweredItemCounts(['sess-1', 'sess-2'])
  ...
  it('leaves a session with zero answer rows absent from the map', async () => {
    mockFromSequence({ count: 1, error: null }, { data: [{ session_id: 'sess-1' }], error: null })
    const result = await fetchAnsweredItemCounts(['sess-1', 'sess-empty'])
    expect(result.error).toBeNull()
