...
> Last updated: 2026-09-01 — **Student-read RPC active-user gates + DISTINCT `answered_count` (migs `20260824000200`/`20260824000300`):** the #883 active-user-gate sweep was recorded in `docs/security.md` §11c as having covered "every SECURITY DEFINER RPC by family"; a mechanical re-derivation found it had missed four — `list_my_internal_exam_history`, `list_my_active_internal_exam_codes`, `get_daily_activity`, `get_subject_scores` — each missing the active-user check (`users.deleted_at IS NULL`) — the internal-exam pair carrying only the `auth.uid()` null-check, the analytics pair that plus the pre-existing Decision-24 `IS DISTINCT FROM p_student_id` identity guard, so a student soft-deleted via toggle-student-status kept reading their exam history, outstanding exam codes and dashboard analytics for the life of their JWT (~1h), deactivation cascading to none of `quiz_sessions` / `internal_exam_codes` / `student_responses`. All four gated; `get_subject_scores` also gains the rule-9 `qs.deleted_at IS NULL` it never had, so a discarded session no longer moves a student's average. Separately `list_my_internal_exam_history.answered_count` moves from `count(*)` to `count(DISTINCT qsa.question_id)::int` — `quiz_session_answers` is one row per answer ITEM (`UNIQUE NULLS NOT DISTINCT (session_id, question_id, blank_index)`), so a non-MC internal exam rendered "Answered 3/1"; `start_internal_exam_session` draws its pool with no `question_type` filter, so non-MC questions genuinely reach internal exams. The §11c record is rewritten from a closed enumeration into a derivation, and it now states the bound the old text lacked: it covers SECURITY DEFINER only, and the SECURITY INVOKER RPCs granted to `authenticated` carry the same exposure and are NOT fixed here — they stand on ownership-only RLS *policies* (`students_select_sessions`, `students_read_responses` are the policies, not RPCs; neither reads `users.deleted_at`). §11c names that RPC set with an as-of date; read it there rather than a bare count here. Integration suite +2 files / +12 tests (`rpc-internal-exam-student-reads`, `rpc-analytics-guards` — the latter also closing the never-covered `p_days` clamp and `IS DISTINCT FROM p_student_id` identity guard, both pre-existing and both reachable only by a direct PostgREST call). All SIX mechanisms mutation-confirmed against the live local DB in FOUR runs — the four active-user gates reverted in their two per-migration pairs, plus one run each for the DISTINCT count and the soft-delete filter — every run confirmed to redden exactly its own test and nothing else. A fifth run re-confirms the internal-exam gate pair at the red-team tier via Vector FM; the analytics pair gets the sibling Vector FN (`rpc-analytics-active-user-gate.spec.ts`), which also pins the pre-existing Decision-24 identity guard, so both halves of the re-derivation are represented in the dedicated red-team CI job rather than at the integration tier alone. Prior: 2026-08-17 — **Zombie-session fix (#1190 partial) + dead RT filters (#1189):** app-layer only, no migrations. `/app/quiz` held two independent sources of truth for "you have an unfinished session" — `ActivePracticeBanner` (DB-backed, via `getActivePracticeSession`) and `QuizRecoveryBanner` (localStorage, via `readActiveSession`) — and each discard path cleared only its own, so discarding from the former soft-deleted the row and left `quiz-active-session:<userId>` behind: a reload then offered Resume for a session that no longer existed, the runner mounted (nothing on that path re-validates the id), and every answer failed against `verifySessionMembership`'s `deleted_at` filter, silently. Both previously-omitting discard sites now clear the key regardless of the Server Action's outcome (`quiz-submit.ts:68`'s documented disposition), guarded on the stored `sessionId` — a server-rendered banner is never revalidated, so an unguarded `userId`-keyed clear could wipe a NEWER session's answer buffer; rule 13 / mig 136 forecloses concurrent sessions but not a stale render. `check-answer.ts`'s session lookup split so only `PGRST116` reads as "session gone" and any other DB error returns `'Could not check answer'` instead of reporting a transient fault as a dead session (sibling of `check-non-mc-answer-helpers.ts`); `verifySessionMembership` + `gradeAnswer` extracted to `check-answer-helpers.ts` for the §1/§3 caps, giving the MC path the non-MC path's shape. Previously-silent early returns now log — both `Not authenticated` returns deliberately excluded, since none in this repo logs. VFR RT setup hides the calculation/image toggles behind a `showCalcImageToggles` opt-out mirroring `showCode` (prod: 40 RT questions, 0 with calculations, 0 with images, so the "Only …" pair yielded an empty pool and the "Exclude …" pair were no-ops). **#1189 closes on merge** (`Closes` trailer) — it was a VFR RT go-live blocker. **#1190 stays OPEN** for acceptance criterion 2 (a stale entry must not render a runner at all), deferred with its reviewed design: a typed result discriminant per `lookup.ts:97`, a guard at `useQuizPersistence`'s write choke point, and a latched terminal state; ~120 LOC, same class as #1026. Prior: 2026-08-15 — **Typo-tolerant grading (D56, migs 158–160):** `answer_matches` replaces exact comparison in all four text graders; tolerance is one edit per candidate word of 5+ characters (single adjacent transposition counted as one), whole-answer budget two, and any token containing a digit must match exactly. The 879-line source migration was split three ways for the code-style.md §1 cap. Prior: **Save-for-later drafts fix (#1085):** app-layer save/resume now properly closes orphaned practice sessions, enabling Decision 49 for drafted quizzes. `saveDraft` soft-deletes the practice session after persisting the draft; `resumeQuizSession` mints a fresh session from the draft's questions (closes the pre-fix backlog via a one-time cleanup script). Closes the bug where saving a draft blocked new quiz starts. Integration suite 392 (packages/db tier, 43 files — 370 as of the 2026-07-11 master e2e run, +21 for the typo-tolerance grading work in `rpc-check-non-mc-answer.integration.test.ts`, which goes 14 → 35 tests, and +1 in `rpc-vfr-rt-submit.integration.test.ts` (14 → 15) for mig 160's `question_blank_missing_canonical` guard; no new file. The app-layer #925 tier is separate at 124). Prior: **VFR RT Training Phase 7 (cleanup, #697) — feature-complete:** all five question types (MC + short_answer + dialog_fill + ordering + diagram_label) now ship on the shared `/app/quiz` Study UI. Docs-only closeout (no code/migrations): Phase 7 confirmed the practice path was type-agnostic from the start — the Phase-1 "MC-only filter" was never wired, so nothing needed removing; dead-code sweep of `apps/web/app/app/vfr-rt/` clean. #923 bespoke exam UI stays parked (Decision 45); the timed exam will return later as an exam-mode toggle on this shared UI inheriting all five types. Training go-live remains gated on RT question-content import + #1045 (ordering answer-key hardening). Prior: **VFR RT Training Phase 6 (`diagram_label` question type, #697):** migs 150–156 add the drag-to-label `diagram_label` type end-to-end — `questions.diagram_config` JSONB + `is_valid_diagram_config()` shape guard enforcing a distinct zone↔label bijection (mig 150), MAX_ZONES(50) and MAX_LABELS(60) caps for DB/client parity, the blank_index write-trigger widened to admit diagram_label (mig 151), `get_quiz_questions` answer-stripped delivery with shuffled labels (mig 152), `check_non_mc_answer` 6-arg immediate grader (mig 153), the REVOKE-gated `_grade_record_diagram_label` per-zone helper (mig 154), `batch_submit_quiz` INVERTED self-defence (distinct zone/label; partial + distractors allowed) + partial-credit rollup (mig 155), and `get_report_answer_keys` per-zone 2-hop canonical reveal (mig 156). App layer: `DiagramLabelInput` dnd-kit runner over a landscape 16:9 inline-SVG registry (keyed by image_ref) + `DiagramLabelReport`; the answer key never ships (column out of every GRANT SELECT, delivery omits it, grade helper REVOKE'd). Decision 52; RWY 27/09 left-hand traffic-pattern seed. Integration suite 362 (+3 constraint-validation tests added during Phase 6 CR hardening; +48 diagram_label tests in Phase 6 across 8 files). Prior: Earlier 2026-06-30 — **VFR RT Training Phase 5 (`ordering` question type, #697):** migs 143–149 add the `ordering` type end-to-end as a dialog_fill-style per-slot clone — `questions.ordering_items` column + question_type widening (mig 143), `is_valid_ordering_items()` shape guard enforcing non-empty distinct item ids (mig 143, #998 CR #452/#472), the blank_index write-time trigger widened to admit ordering (mig 144), `get_quiz_questions` delivers items SHUFFLED with the answer key hidden (mig 145), `check_non_mc_answer` immediate grader (mig 146), the REVOKE-gated `_grade_record_ordering` per-slot helper (mig 147), `batch_submit_quiz` ordering dispatch + permutation completeness guard (mig 148, #998 CR #466) + partial-credit rollup, and `get_report_answer_keys` per-slot canonical reveal (mig 149). N7 deviation (per-slot rows, not single JSON) documented (Decision 51). Integration suite 311 (+41 ordering execution tests across 5 files: get_quiz_questions shuffled delivery + ordering_items REVOKE-by-omission + is_valid_ordering_items CHECK reject [dup-id, blank-text, non-string-id, non-string-text, whitespace-id, missing-text-key, extra-key] + columns_check totality reject [non-array] [11]; check_non_mc_answer correct/permuted/wrong-length grading + guards [unauth, mock/internal-exam mode, soft-deleted caller, non-owner, non-member, answer_type_mismatch ×3, non-array p_order] [13]; batch_submit_quiz per-slot persistence + 3-of-5→60% partial credit + fully-correct→100% + permutation-guard reject [dup-item, subset, dup-slot] + `_grade_record_ordering` REVOKE [authenticated, anon] [8]; get_report_answer_keys per-slot keys + non-owner + not-completed [3]; mig-144 trigger widening accept/reject across both answer tables [6]; +6 single-active cross-mode start guards #1018 [internal_exam block + discovery auto-clear ×2, vfr_rt block + same-subject resume + discovery auto-clear ×3] + emailed_at resend re-stamp #1035 [1]). Prior: Earlier 2026-06-29 — **Single-active-session invariant (#1011, PR A):** at most one active (`ended_at IS NULL AND deleted_at IS NULL`) `quiz_sessions` row per student across all modes — global partial unique index `uq_one_active_session_per_student` (mig 136) + each start RPC raising `another_session_active` (migs 137–141); Discovery/Study Mode is now a real ephemeral `mode='discovery'` row via `start_discovery_session` (mig 137, still non-resumable + nothing-scored), `get_study_questions` whitelists `discovery` (mig 142), plus app-layer `endDiscovery` teardown + `ActivePracticeBanner` server-backed Discard. Closes the Discovery/practice answer-key oracle on the shared MC pool (can't start a second session, so an answer-revealing session can't coexist with a live exam). Decision 49 + `docs/security.md` §11d documented. Integration suite 231 (+11 — `rpc-single-active-session.integration.test.ts`: `start_discovery_session` happy-path/idempotent-replace/validation, discovery auto-clear on practice + exam start, cross-mode `another_session_active` blocks, global `uq_one_active_session_per_student` 23505 backstop, plus Gap C positive control — `get_study_questions` serves answer keys while the student holds their own active discovery session, guarding mig 142's `discovery` whitelist). App-layer: `end-discovery.integration.test.ts` (endDiscovery soft-deletes the caller's own discovery row under real RLS WITH CHECK, no-op when none active, cross-user isolation). Red-team: +1 spec file `single-active-session.spec.ts` (Vectors EP/EQ/ER/ES/ET). Prior: 2026-06-26 — **Study Mode UI relocation:** flashcard feature surfaced as **Discovery** (first/default segment of the New Quiz `ModeToggle`); `quiz-tabs` reduced to 2 tabs; `study-section.tsx` deleted; `quiz-config-form.tsx` short-circuits to `StudyConfigForm` sibling layout when `mode === 'discovery'`. Internal identifiers unchanged (`get_study_questions`, `startStudy`, `use-study-*`). Prior: **Study Mode (MC flashcards, feat/study-mode-mc):** new `get_study_questions(p_question_ids uuid[])` SECURITY DEFINER RPC (mig 135) delivering MC questions WITH the answer key + explanation for on-demand practice (no session, no score; exam-integrity preserved by the server-side active-exam-session guard — Study Mode is blocked mid-exam); `_filtered_question_pool` + `get_random_question_ids` extended with optional `p_question_type` parameter (mig 134, backward-compatible NULL default). Decision 48 documented. Prior: **VFR RT Training Phase 4 (report renders non-MC, #697):** study `batch_submit_quiz` `correct_count` unified to count correct items (blank-rows) matching exam `submit_vfr_rt_exam_answers` (mig 132); new `get_report_answer_keys` SECURITY DEFINER RPC delivers non-MC canonicals to the report (mig 133); the `/app/quiz` report renders short_answer/dialog_fill with 3-state partial display + an item-level header. Integration suite 220 (+1 — mixed-type MC+short_answer+dialog_fill session asserting correct_count sums correct items across types; +8 — `get_report_answer_keys` delivery [short_answer canonical, dialog_fill per-blank canonicals, all-MC→zero rows, mixed-type] + guards [unauth, soft-deleted caller, non-owner, not-completed]). Prior: **blank_index ⇔ dialog_fill write-time trigger (#828):** mig 131 adds a `BEFORE INSERT` trigger on both answer tables (`quiz_session_answers`, `student_responses`) enforcing `question_type = 'dialog_fill' ⇔ blank_index IS NOT NULL` via a cross-table question_type lookup the single-row `*_answer_shape_check` CHECK cannot do — closing the gap where a future inserter or admin-form/import bug could persist a malformed `blank_index`. SECURITY INVOKER (repo trigger convention), no `deleted_at` filter (§15 frozen-config carve-out). Integration suite 211 (+9 trigger tests). Prior: **submit RPC expired-replay flag (#839):** migs 129–130 re-add `expired:true` on the idempotent-replay branch of `submit_vfr_rt_exam_answers` (mig 129) and `batch_submit_quiz` (mig 130) — a retry of a timer-expired submit now returns the same `{expired:true}` payload as the original instead of looking like a 0-score completion, detected via the append-only `*.expired` audit event (also catches `complete_overdue_/empty_exam_session` expiry). Integration suite 202 (+2 replay tests). Prior: **dialog_fill delimiter guard + answer-key strip hardening (#951):** migs 125–127 add two CHECK constraints forbidding `{ } | ;` in dialog_fill answer values (primary leak guard) + delimiter-harden the student-facing strip regex in `get_quiz_questions`/`get_vfr_rt_exam_questions` (defense-in-depth); integration suite 200. Prior: **App-Layer DB Integration Tier (#925) COMPLETE:** Phases 0–4 add a real-DB app-layer integration test tier + mechanical schema-contract guards + binding rule promotions (code-style §5/§7, Decision 46); no migrations. Prior: **MC answer-key relocation #823 (P0 security):** migs 111–117 move the MC answer key out of `options[].correct` into the REVOKE-gated `questions.correct_option_id` column (sanitize trigger strips `correct` on write; biconditional CHECK; 7 scoring/report RPCs read the column incl. `check_quiz_answer` immediate-feedback RPC in mig 117; `get_question_authoring_fields` exposes it to admins only); integration suite now 200 (was 172; +25 VFR RT Phase 2 #697 tests + 3 #951 dialog_fill delimiter-guard CHECK rejection tests — get_quiz_questions non-MC delivery [EK], check_non_mc_answer guards/output [EL/EM], batch_submit_quiz non-MC dispatch + partial-credit + helper REVOKE; the 2.4 execution tests caught + fixed two deferred-validation runtime bugs that passed db-reset/tsc/biome/critics — mig 118 `get_quiz_questions` 42702 ambiguous-`id` and mig 120 helper 23502 NULL `is_correct`). Prior: **Internal Exam Code Email (Wave 2):** mig 110, `record_internal_exam_code_emailed()` SECURITY DEFINER RPC (audit writer), `sendInternalExamCodeEmail` Server Action (Resend provider, console-log fallback), new `lib/email` subsystem + `lib/email/templates/internal-exam-code.ts`, env vars RESEND_API_KEY + EMAIL_FROM (server-only). Tests: unit (Resend + template + query helper + Server Action + UI wiring) + 8 integration (RPC); integration suite now 169. Decision 44 documented (Resend transactional provider choice). Prior: **Quiz Question Filtering #864:** mig 109 adds `p_has_image` {all|only|exclude} AND-restriction to filtered-pool RPCs, mirrors calc-mode (#837); suite now 161. Prior: **Quiz Question Filtering #837:** migs 107–108 add `has_calculations` column + calc-mode AND-restriction to filtered-pool RPCs; integration suite now 154 (#842: +1 — concurrent-completion race asserting `complete_quiz_session` FOR UPDATE serializes to exactly one winner; #845: +2 — assert the `get_vfr_rt_exam_results` per-question answers array, passing + failing sessions). Prior: **Phase A follow-ups #833/#840:** mig 105 reshapes `get_vfr_rt_exam_questions` to a session-derived `(p_session_id)` signature (§15 carve-out now legitimate; explanations stripped from the in-flight path); mig 106 reveals explanations only via the `ended_at`-gated `get_vfr_rt_exam_results`; integration suite reached 145. Prior: **Phase A: VFR RT (Slovenia) Mock Exam COMPLETE** (squash commit 76a9b03e, PR #830). 15 migrations (094–104), 136 integration tests at delivery, 6 new RPCs, 3 new decisions documented. Phases B–E pending. Prior: **Red-team infra refactor** (issue #796): split 3 oversized red-team spec files into 5 files (now 41 spec files total, unchanged 271 tests), extracted shared cleanup/audit helpers into reusable modules. Prior sprint: **Security & Test-Hardening Sprint** (post-#668, see section below): DB-hardening migrations 085–093, OWASP A10:2025 error-disclosure coverage, auth audit logging (`record_auth_event`), activity-stamp trigger. Two batches landed that sprint — backend (#446/#684/#471/#532/#379, PRs #782/#783/#785/#787/#790) and red-team E2E coverage (#784/#786/#788/#781, PR #795). Prior milestone: **Umbrella #668 (PostgREST 1000-row truncation) CLOSED 2026-05-31** — all 25 sites addressed (24 fixed + 1 exempt) across instances #1–#9, plus the §5 cast-guard sweep (#677, PR #707) and red-team E2E coverage (#673, PR #709).
---

## App-Layer DB Integration Tier (#925) — 2026-06-21
...
## Quiz Question Filtering — Image-Presence Filter (Phase A follow-up, #864) — 2026-06-14

**COMPLETE** — DB migration 109 + RPC signature changes + student UI toggle. Mirrors the calc-mode pattern (#837): `p_has_image {all|only|exclude}` AND-restricts the filtered question pool on `question_image_url` presence (IS NOT NULL / IS NULL). Signature change requires DROP-before-recreate of `_filtered_question_pool`, `get_random_question_ids`, `get_filtered_question_counts` (all three, maintaining consistency).

**Feature:** Student quiz builder gains a second AND-restriction toggle alongside calc-mode: "Only questions with an image" and "Exclude questions with an image" (mutually exclusive; neither active = all by default). Count badge reflects both calc-mode and has-image filters.

**Migration 109 / 20260614000001:**
- DROP + recreate `_filtered_question_pool(p_subject_id, p_topic_ids, p_subtopic_ids, p_filters, p_calc_mode, p_has_image)` with new trailing `p_has_image TEXT DEFAULT 'all'` parameter. CASE WHEN branch filters on `question_image_url IS NOT NULL` (only) or `IS NULL` (exclude).
- DROP + recreate `get_random_question_ids`, `get_filtered_question_counts` with matching signature change.
- GRANT EXECUTE updated to reflect new signatures on all three.
- Function comments updated with #864 issue reference and parameter documentation.

**Integration tests:** 7 new tests in `rpc-has-image-pool.integration.test.ts` (all/only/exclude pool membership, count==quiz, omitted-filter default, only+unseen composition, null `p_count` → empty result guarding the 500-row cap). Suite now 161 (post-#837 suite 154 + 7 new).

**Docs updated:** docs/database.md (RPC summary lines 723–724, `get_random_question_ids` § Parameters new `p_has_image` line, `get_filtered_question_counts` § Parameters updated, `_filtered_question_pool` helper section updated, footer timestamp updated to 2026-06-14).
...
- **GitHub Actions CI** (cloud):
  ...
  - `dependabot.yml` — automated dependency updates, weekly schedule, `tech-debt` label, commits with `ci` or `chore` prefix. npm updates are grouped via a single pnpm-workspace-aware root entry (consolidated in #715; per-directory entries previously caused lockfile-sync failures — see #669/#670) into three groups: `npm-all-minor-patch`, `npm-all-major` (which **excludes `typescript`** via `exclude-patterns`), and `npm-typescript-major`. TypeScript majors are isolated because TS 7 is blocked upstream — Next.js cannot build under `typescript@7` (it dropped `lib/typescript.js` and moved the compiler API to `./unstable/*`) — and a grouped PR merges all-or-nothing, so leaving it in `npm-all-major` made every future majors PR unmergeable (#1128). This is isolation, not suppression: Dependabot still opens a PR for each new TypeScript major, so the signal that TS 7 has become viable is preserved. Remove `npm-typescript-major` **and** the `exclude-patterns` together once Next.js supports TypeScript 7
...
│   └── report/             ← VFR RT practice report ("VFR RT Practice …"; redirects non-RT sessions to /app/quiz/report)
├── internal-exam/          ← student internal-exam landing (Available + My Reports tabs)
│   └── report/             ← internal-exam report (mode-guarded; redirects non-internal_exam to /app/quiz/report)
...
- **#568 FIXED**: the 0-answer auto-submit hang on `/app/quiz/session`. The original read-after-write hypothesis was wrong — an instrumented e2e reproduction proved the report page rendered 200 OK (no redirect, `ended_at` set). Real root cause: the empty-exam path invoked `clearDeploymentPin()` (a Server Action) **after** `router.push()`; the Server Action response triggered an App Router revalidation that cancelled the pending soft navigation. Fix: fire `clearDeploymentPin()` **before** `router.push()` so push is the last statement, matching the `batch_submit_quiz` (1+-answer) path that always worked. `exam-flow.spec.ts:46` re-enabled (`test.fixme`→`test`). **Refined 2026-06-21 (#909):** the fix evolved from reordering the call to actually **awaiting** the Server Action (via `await clearDeploymentPin()` in both paths, and `await Promise.race([deleteDraft(...), 2500ms timeout])` in submitQuizSession). All paths now block on cleanup before navigating, eliminating the race entirely. Internal exams always carry a recovery draft, so the deleteDraft round-trip hit the race every time; #909 adds a hard-navigation fallback (`window.location.assign`) that fires after 4s without unmounting, as a safety net if soft navigation stalls.
...
- **New `checkNonMcAnswer` Server Action** (`quiz/actions/check-non-mc-answer.ts` + `-helpers.ts` + `-schema.ts`): Zod discriminated union (short_answer / dialog_fill / ordering) split across `-schema.ts` (input validators), session ownership + membership guard mirroring `check-answer.ts`, RPC-result runtime type guards paired with the cast (§5). Translates the client `{index,text}` blanks to the RPC's `{blank_index,response_text}` shape; ordering inputs validated for unique ids (a permutation).
...
*Last updated: 2026-06-25 — VFR RT Training Phase 5 CR hardening (c1e12317, PR #998): Zod input schemas hoisted from check-non-mc-answer-helpers.ts to new check-non-mc-answer-schema.ts (input guardrails; both files back under utility caps); ordering answer arrays validated for unique ids across all 4 validators (canonical order is a permutation); report-answer-body.tsx trailing return null replaced with compile-time exhaustiveness check. Earlier 2026-06-24 — VFR RT Training Phase 3 (Study runner renders + grades short_answer/dialog_fill; checkNonMcAnswer Server Action; no migration).*
...
- **Study `correct_count` → correct items (blank-rows)** (mig 132): `batch_submit_quiz`'s `v_correct_count` changes from "fully-correct questions" (`count(*) FILTER (… correct_rows >= total_blanks)`) to "correct items" (`sum(correct_rows)`), matching exam `submit_vfr_rt_exam_answers`. `score_percentage` (Decision 47 per-question partial credit) is unchanged — only the stored item count changes. The idempotent-replay branch returns the stored `quiz_sessions.correct_count` verbatim, so it stays consistent by construction. Sibling `complete_quiz_session` already writes an item-based `correct_count` (agrees, no change).
- **New `get_report_answer_keys` RPC** (mig 133, coming in commit 2): type-aware SECURITY DEFINER reader for the non-MC canonical answers (short_answer canonical, dialog_fill per-blank canonicals), `ended_at`-gated and owner-scoped, mirroring `get_report_correct_options`'s guard set (§15 frozen-config carve-out). MC keeps its existing `get_report_correct_options` path unchanged.
- **Report renders non-MC** (commit 3): the report question builder collapses per-blank dialog rows to one question per `questionId`; row renderers dispatch by `questionType` to short_answer / dialog_fill sub-renderers with a 3-state partial display (fully correct / partial "X/N blanks" / none). The **study** (non-exam) summary header becomes item-level (`Correct: {correct items} / {answered items}`) with a question-level `Skipped` (`totalQuestions − answeredQuestions`); the **exam** header initially kept `Correct: {correct items} / {totalQuestions}` with no Skipped — **reversed 2026-08-24 (Decision 60)**: that divided an item-level `correct_count` by a question-level total and can render fractions above 1 for live exams ("29 / 25" — reproduced locally, not observed in production), so BOTH headers are now item-level (`Correct: {correct items} / {answered items}`) and BOTH render `Skipped`, which is what now carries the paper-size signal.

*Last updated: 2026-06-24 — VFR RT Training Phase 4 (report renders short_answer/dialog_fill; study correct_count unified to correct items, mig 132; new get_report_answer_keys RPC mig 133; item-level report header). — amended 2026-08-24 (Decision 60: both headers item-level, both render Skipped)*
...
- **7 migrations (150–156):** `questions.diagram_config JSONB` column + `is_valid_diagram_config()` shape guard + 5-branch `question_type` widening (mig 150); `blank_index` write-invariant trigger widened to also admit `diagram_label` (mig 151); `get_quiz_questions` delivers `{image_ref, zones, labels(shuffled)}` with `answer` omitted, 17th RETURNS TABLE column `diagram_config_public` (mig 152); `check_non_mc_answer` widened to a 6-arg signature (+`p_mapping`) with a `diagram_label` grading branch — SET comparison, revealed `correct_mapping` (mig 153); new REVOKE-gated `_grade_record_diagram_label` per-zone helper — derives the zone ordinal server-side, never client-supplied (mig 154); `batch_submit_quiz` `diagram_label` dispatch + **INVERTED self-defence** (distinct zone_id/label_id is the integrity key; partial submission + unused distractor labels are explicitly allowed, unlike `ordering`'s forced complete permutation) + partial-credit rollup (mig 155); `get_report_answer_keys` per-zone canonical reveal via a 2-hop resolve — zone → `diagram_config.answer` entry → label text (mig 156).
- **App layer:** `DiagramLabelInput` (dnd-kit `DndContext` with `pointerWithin` collision detection, consume-on-place label pool, zone boxes positioned as `%` overlays over an inline SVG) + `DiagramLabelReport`; new `_components/diagrams/rwy-2709-lh-pattern.tsx` (SVG artwork) + `_components/diagrams/registry.ts` (`image_ref` → component); `diagram-validation.ts` (mirrors `ordering-validation.ts`); dispatch entries wired through the answer-input dispatcher, `use-answer-pipeline`, `use-quiz-state`, and the report row dispatcher; `DraftAnswer`/`AnswerFeedback`/`QuizReportQuestion` unions widened for the `diagram_label` variant; `load-draft-helpers.ts` `toFeedbackEntry` gains a `diagram_label` case so a resumed DB draft round-trips per-zone feedback.
- **Decision 52 documented:** inline SVG component registry keyed by `image_ref` (not a static PNG), distractor labels allowed, 9-zone RWY 27/09 left-hand pattern seed fixture (5 legs + 4 turns + ~2-3 distractors), general `diagram_config` schema (only the seed is pattern-specific).
- **Seed:** `apps/web/scripts/seed-vfr-rt-training-eval.ts` extended with the 27/09 fixture (9 zones, 9 correct answer entries, unrelated random zone/label id schemes, seed-time id-disjointness assertion per the security invariant in Decision 52).
- **Integration tests (CI-only, execute the RPCs):** `get_quiz_questions` diagram delivery (answer omitted, labels shuffled); `diagram_config` column REVOKE-by-omission; `is_valid_diagram_config` CHECK rejection suite (non-number coord, out-of-range coord, non-array zones/labels/answer, uncovered zone, unknown zone/label reference, duplicate zone, duplicate label [two zones sharing one label — the bijection guard]); `check_non_mc_answer` diagram correctness + full guard set (unauth, non-whitelisted mode, both-exam-mode, soft-deleted caller, non-owner, non-member, `answer_type_mismatch`, non-array `p_mapping`); `batch_submit_quiz` per-zone persistence + partial credit + self-defence rejection (duplicate zone, double-placed label) + non-vacuous acceptance of partial + distractor submissions; `get_report_answer_keys` diagram branch aligning with the derived `blank_index`; `_grade_record_diagram_label` REVOKE test (anon/authenticated cannot EXECUTE); trigger widening accept/reject. Integration suite count already bumped 311→362 in the header above by the implementing session.
...
Content pool: **50 questions / 71 blanks**, `"lifecycle": "pilot"` — so the importer refuses
`--force-remote` for this file and prod is untouched.

> The gate used to be a prefix match on the free-text `status` field (`status.startsWith('PILOT')`).
> It was **dead**: a later commit rewrote that prose to begin `"51 questions across…"`, so the check
> evaluated false and `--force-remote` would have carried all 50 un-evaluated questions into the
> production bank as `status: 'active'`, immediately exam-eligible. Three surfaces claimed it worked
> — the status string's own self-referential clause, the code comment, and this line. Replaced with
> a structured `lifecycle: "pilot" | "released"` field checked by `assertReleasedForRemote`, which
> fails closed on absent, non-string, wrong-case or any other value. Prose is no longer load-bearing.
