...
**Integration tests:** 132 new SQL tests (admin-questions.spec.ts + admin-exam-configs.spec.ts expanded; new vfr-rt-exam.spec.ts suite). Fixtures: VFR RT questions (part1=short_answer, part2=dialog_fill, part3=MC with per-blank variants), exam sessions (fresh start, idempotent resume, timer expiry, per-part grading breakdown).
