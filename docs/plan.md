...
- **#568 FIXED** (commit `68216d56`): the 0-answer auto-submit hang on `/app/quiz/session`. The original read-after-write hypothesis was wrong — an instrumented e2e reproduction proved the report page rendered 200 OK (no redirect, `ended_at` set). Real root cause: the empty-exam path invoked `clearDeploymentPin()` (a Server Action) **after** `router.push()`; the Server Action response triggered an App Router revalidation that cancelled the pending soft navigation. Fix: fire `clearDeploymentPin()` **before** `router.push()` so push is the last statement, matching the `batch_submit_quiz` (1+-answer) path that always worked. `exam-flow.spec.ts:46` re-enabled (`test.fixme`→`test`). **Refined 2026-06-21 (commit `f1333974`, #909):** the fix evolved from reordering the call to actually **awaiting** the Server Action (via `await clearDeploymentPin()` in both paths, and `await Promise.race([deleteDraft(...), 2500ms timeout])` in submitQuizSession). All paths now block on cleanup before navigating, eliminating the race entirely. Internal exams always carry a recovery draft, so the deleteDraft round-trip hit the race every time; #909 adds a hard-navigation fallback (`window.location.assign`) that fires after 4s without unmounting, as a safety net if soft navigation stalls.
