...
**What it MISSED — this paragraph previously said the sweep had covered "every SECURITY DEFINER RPC by family" and was CLOSED. That was false.** A mechanical re-derivation on 2026-09-01 found 4 more, all with only the `auth.uid()` null-check: `list_my_internal_exam_history` and `list_my_active_internal_exam_codes` (gated by mig `20260824000200`), and `get_daily_activity` and `get_subject_scores` (gated by mig `20260824000300`; the same migration added the rule-9 `deleted_at IS NULL` filter `get_subject_scores` never had on `quiz_sessions`).
