...
**Do not trust that list either — RE-DERIVE it.** The set is OPEN, so this rule states the derivation rather than an enumeration (`code-style.md` §10 rule 2). Take each function's LATEST definition across every supersession form (`agent-workflow.md` § "name EVERY supersession form"); keep those that are SECURITY DEFINER, not `is_admin()`-gated, and `GRANT`ed to `authenticated`; then check each for a `users` lookup filtered on `deleted_at IS NULL` whose miss raises. Two exempt classes: trigger functions, which no caller invokes directly (`stamp_last_active_on_session_complete`), and helpers `REVOKE`d from `authenticated`/`anon`/`public` (`answer_matches`, the `_grade_record_*` family).
