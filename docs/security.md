...
- Immutable append-only table. Stores one row per accepted (document_type, version) — `record_consent()` has been idempotent since `20260606000001_record_consent_idempotency.sql`, so a repeated acceptance of the same pair is deliberately not re-stored. Covers: Terms of Service and Privacy Policy — with version, acceptance flag, timestamp, IP, and user agent. (Cookie Analytics was removed from the `document_type` CHECK by `20260327000058_remove_cookie_analytics.sql`.)
