...
- `WITH CHECK` is required only when the write predicate must DIFFER from `USING`, and only on the two commands that accept both. On a `FOR ALL` or `FOR UPDATE` policy, omitting it is safe: PostgreSQL reuses `USING` as the write check (verified — `pg_policies.with_check` reads NULL, yet a row violating `USING` is still rejected). This inheritance does NOT extend to `FOR INSERT`, which has no `USING` to inherit from. What actually lets a student write another org's rows is a policy whose `USING`/`WITH CHECK` predicate is too broad — not a missing clause. Two consequences worth stating, because "safe" is easy to over-read: (a) safe means *security*-safe, not behaviour-neutral — the reused predicate also TIGHTENS writes, which is why `students_update_sessions` (mig `20260313000023`) spells out a deliberately BROADER `WITH CHECK (student_id = auth.uid())`: its `USING (… AND ended_at IS NULL)` would otherwise reject the completing write. That particular path is now privilege-gated as well — mig `20260605000001` removed `ended_at` from `authenticated`'s UPDATE grant, so the broader `WITH CHECK` no longer enables any student-reachable write. (It is a hole-punch, not a guard: it PERMITS a write the reused `USING` would reject. The guard is the column grant, which acts in the opposite direction.) Never delete a broader `WITH CHECK` as redundant. And (b) a reused `USING` constrains only the columns it names: on a hypothetical `FOR ALL USING (student_id = auth.uid())` session policy with no `WITH CHECK`, the owner could still write `score_percentage`, `mode` or `config`. That is a too-broad *predicate* and IS a finding — distinct from a merely absent clause, which is not. (On the real `quiz_sessions` those columns are separately defended by the mig `20260605000001` column grant and `trg_quiz_sessions_immutable_columns`.)
...
⚠️ **No `AS RESTRICTIVE` policy existed anywhere in the `public` schema as of 2026-08-19** (0 of 62 policies at the time the figure was first recorded; a later local count read 58, so treat the total as indicative and the zero as the load-bearing claim). Both figures move with any policy migration, so re-derive before relying on the zero — `SELECT count(*) FILTER (WHERE permissive = 'RESTRICTIVE'), count(*) FROM pg_policies WHERE schemaname = 'public'`. Every denial above rests on the *absence* of a permitting permissive policy. Do not describe a `USING (false)` / `WITH CHECK (false)` permissive policy as "blocking" or "overriding" — it does neither, and it would not stop a permitting policy added beside it. Only `AS RESTRICTIVE` AND-combines.
...
PostgreSQL combines multiple **permissive** RLS policies with **OR**. When a table carries more than one permissive `SELECT` policy — typically a narrow per-user policy plus a broader role policy — RLS alone does **not** restrict a read to the calling user. A `SECURITY INVOKER` or `SECURITY DEFINER` function meant to return the **caller's own** rows from such a table MUST add an explicit predicate: `WHERE <owner_col> = auth.uid()`, or `WHERE <owner_col> = p_student_id` PAIRED WITH an `auth.uid() = p_student_id` identity guard. The identity guard on its own only validates the parameter — a query keyed on a session id or record id still reads whatever RLS allows, which is the whole defect. The explicit filter is load-bearing, not redundant.
...
**Tables with multiple permissive SELECT policies** (audit when adding an RPC that reads one) — as of 2026-05-26: `student_responses`, `quiz_sessions`, `exam_configs`, `audit_events`. This set GROWS with any migration adding a second permissive SELECT policy, so re-derive it rather than trusting this line:

```sql
-- cmd IN ('SELECT','ALL'): an unqualified CREATE POLICY is FOR ALL and permits SELECT as well,
-- so filtering on cmd='SELECT' alone silently omits tables whose second read path is a FOR ALL policy.
...
**Edge Middleware responses (3xx redirects, 4xx/5xx errors).** Next.js `headers()` only applies to routed responses; middleware-emitted responses bypass it. `apps/web/proxy.ts` re-emits the static headers on every response it BUILDS ITSELF, via `applySecurityHeaders` (the `res.headers.set` block at the top of the file; read it rather than trusting a count here, since a new header joins it). Coverage is complete and rests on a funnel, not on repetition: every redirect exit returns through the shared `redirectWithCookies` helper, which applies them once, and the 503 applies them directly. The invariant to preserve is therefore the funnel — a hand-built `NextResponse` that bypasses `redirectWithCookies` must call `applySecurityHeaders` itself or it ships with no CSP and no HSTS. Those responses get a **reduced CSP** of `default-src 'none'; frame-ancestors 'none'` (not the full routed-response CSP). The reduction is intentional: no scripts execute on a 3xx/4xx/5xx, so locking `default-src` to `'none'` is at least as strict as the routed policy and prevents accidental subresource loading on error pages. `frame-ancestors 'none'` matches the routed CSP so legacy browsers ignoring `default-src` still see the framing deny intent. The red-team spec `apps/web/e2e/redteam/header-validation.spec.ts` asserts both response classes; do not assume routed-response CSP rules apply to middleware responses when reviewing security changes.
...
- the pre-push security-auditor (`.claude/hooks/run-security-auditor.sh`), fail-closed. An LLM review, not
  ...
  runs only when the LLM call times out or fails: `grep -n 's[k]_live_' .claude/hooks/run-security-auditor.sh`.
...
| **Active-user / soft-deleted-caller gate** | `PERFORM 1 FROM users WHERE id = <uid> AND deleted_at IS NULL; IF NOT FOUND THEN RAISE 'user not found or inactive'` — or fold `AND deleted_at IS NULL` into a `SELECT … INTO` on `users` with a `NOT FOUND` guard. Progressively added by the mig-076 family (076/078/085/087/088/095b/095c/099/100/103/104/105/106 — an as-of list, not a census; the set is OPEN and later migrations keep joining it, e.g. `20260824000200`/`20260824000300`, so re-derive per the sweep paragraph below rather than reading this as complete). It closes the "deactivated account with a still-valid JWT" window. Note the token spelling splits by RPC family and BOTH are correct. Each family is an OPEN set — over twenty migrations apiece and still growing — so DERIVE membership rather than reading any list as complete: `grep -rln "RAISE EXCEPTION 'user not found or inactive'" supabase/migrations/` for the space-separated form, and the same grep with `'user_not_found_or_inactive'` for the underscore form. As of 2026-09-01 the space-separated form covers the exam-session / report / batch-submit family plus the internal-exam student reads and the analytics readers (`20260824000200` / `20260824000300`); the underscore form covers the VFR-RT / quiz-question-serving family. Match the family you are editing. |
| Ownership / identity scope | `WHERE <owner_col> = auth.uid()`, or `WHERE <owner_col> = p_student_id` PAIRED WITH an `auth.uid() = p_student_id` guard — the guard alone validates only the parameter (".claude/rules/security.md" rule 11 "Multiple Permissive RLS SELECT Policies"; docs/security.md §3 Row Level Security) |
...
**Promotion sweep (#883) — what it fixed.** The count=3 promotion fixed 4 legacy read-RPCs old enough to predate the mig-076 gate family: `check_quiz_answer` (mig 117, #823), `get_report_correct_options` (mig 114, #856), `get_quiz_questions` (mig 118, folded into the org-scope `SELECT … INTO`), and `get_session_reports` (mig 122). Each of those four is covered by a soft-deleted-caller rejection test. mig 122 aliases `users u` (`u.id`) because `get_session_reports`'s `RETURNS TABLE` declares an `id` OUT param — an unqualified `id` in the gate would be ambiguous (42702 at execution, code-style.md §5(c)).

**What it MISSED — this paragraph previously said the sweep had covered "every SECURITY DEFINER RPC by family" and was CLOSED. That was false.** A mechanical re-derivation on 2026-09-01 found 4 more, none carrying the active-user check: `list_my_internal_exam_history` and `list_my_active_internal_exam_codes`, which had only the `auth.uid()` null-check (gated by mig `20260824000200`), and `get_daily_activity` and `get_subject_scores`, which had that plus the pre-existing Decision-24 `IS DISTINCT FROM p_student_id` identity guard (gated by mig `20260824000300`; the same migration added the rule-9 `deleted_at IS NULL` filter `get_subject_scores` never had on `quiz_sessions`).

**Do not trust that list either — RE-DERIVE it.** The set is OPEN, so this rule states the derivation rather than an enumeration (`code-style.md` §10 rule 2). Take each function's LATEST definition across every supersession form (`agent-workflow.md` § "name EVERY supersession form"); keep those that are SECURITY DEFINER, not `is_admin()`-gated, and `GRANT`ed to `authenticated`; then check each for a `users` lookup filtered on `deleted_at IS NULL` whose miss raises. Exempt classes include: trigger functions, which no caller invokes directly (`stamp_last_active_on_session_complete`), and helpers `REVOKE`d from `authenticated`/`anon`/`public` (`answer_matches`, the `_grade_record_*` family).

**Scope bound — this rule covers SECURITY DEFINER only, and a SECOND exposure of the same threat is still OPEN.** A SECURITY INVOKER function has RLS evaluated for it, so it needs no manual gate — but only where the RLS predicate itself excludes a deactivated account, and today's do not. `students_select_sessions` (`20260313000023`) and `students_read_responses` (`20260311000005`) are ownership-only (`student_id = auth.uid()`), never consulting `users.deleted_at`. So as of 2026-09-01 eight SECURITY INVOKER RPCs granted to `authenticated` — `get_student_mastery_stats`, `get_student_streak`, `get_student_last_practiced`, `get_student_profile_stats`, `get_question_counts`, `get_filtered_question_counts`, `get_random_question_ids`, `_filtered_question_pool` — still serve a soft-deleted caller holding a live JWT. Not fixed here; sequenced as the next PR alongside the app-layer gap (neither `requireAuthUser()` nor `proxy.ts` reads `users.deleted_at`, so these RPC gates are currently the only enforcement).
...
- `/consent` page: two required checkboxes (TOS, Privacy). Server Action calls `record_consent()` twice and sets the cookie with the version tokens and the caller's user id; the form then navigates client-side to the validated `next` destination (`safeNextPath`), else `/app/dashboard` (`consent-form.tsx` — the action itself does not redirect).
- Proxy check: the `__consent_u` cookie must equal `<tos>:<privacy>:<signed-in user id>` (`buildConsentCookieValue`), so a cookie left by another user on a shared browser never matches. No DB hit in the proxy. On a mismatch the proxy redirects (307) to `/auth/consent-refresh` (GET and POST, so a Server Action is replayed), which calls `check_consent_status()` and either reissues the cookie (consented) or redirects to `/consent`. The pre-#1377 cookie `__consent` (`<tos>:<privacy>`) is neither read nor written, so a deployment still serving the old code keeps working during rollout.
- Re-consent trigger: bump `CURRENT_TOS_VERSION` or `CURRENT_PRIVACY_VERSION` in `lib/consent/versions.ts` → cookie mismatch → `/auth/consent-refresh` → `/consent` (the DB holds no consent for the new version).
...
- **SECURITY DEFINER soft-delete rule:** Every SELECT inside a SECURITY DEFINER function must explicitly filter `AND deleted_at IS NULL` on all soft-deletable tables. These functions are owned by `postgres`, which holds `BYPASSRLS`, so RLS is not evaluated at all — soft-delete policies are not applied automatically and must be replicated manually in every query. (`SECURITY DEFINER` *alone* does not bypass RLS: it re-runs as the owner, and under `FORCE ROW LEVEL SECURITY` not even the owner is exempt. The bypass here comes from the owner's `BYPASSRLS` attribute.) **Narrow exception:** SELECTs that retrieve records by IDs stored in an immutable, write-once column may omit the filter, because the accessible ID set is bounded by the immutable prior write rather than by the deleted-at predicate. Examples include: (a) `batch_submit_quiz` in both its initial-submit and idempotent-replay paths reads `questions` — initial via `quiz_sessions.config.question_ids`, replay via `quiz_session_answers.question_id` (both immutable write-once boundaries); `submit_vfr_rt_exam_answers`, `check_quiz_answer` reading `questions` via `quiz_sessions.config.question_ids` (written once at session start; `check_quiz_answer` verifies `p_question_id = ANY(config.question_ids)` before the read, so a question soft-deleted mid-session is still answerable for immediate feedback — mig 117 / #823; `submit_quiz_answer` verifies the same `p_question_id = ANY(config.question_ids)` membership before its questions read, so a question soft-deleted mid-session stays submittable for a fresh graded answer — aligned with `check_quiz_answer` for consistency, mig 123 / #855; `check_non_mc_answer` (the short_answer + dialog_fill + ordering + diagram_label immediate-feedback grader) reads `questions` via the same frozen `config.question_ids` after a membership check, so a question soft-deleted mid-session stays gradeable — mig 119 / #697, widened for `ordering` mig 146 and `diagram_label` mig 153; batch_submit_quiz replay removes the deleted_at filter on the JOIN for consistency — mig 20260619000250 / PR #856); (b) `get_vfr_rt_exam_questions`, `get_vfr_rt_exam_results` reading `questions` via the same frozen `config.question_ids` — both derive the question IDs server-side from the caller-owned session row, never from client input (`get_vfr_rt_exam_questions` since migration `20260611000100` / mig 105, which dropped its caller-supplied `p_question_ids uuid[]` signature for `(p_session_id uuid)`; `get_vfr_rt_exam_results` since its creation in mig 103); (c) `get_report_correct_options`, `get_admin_report_correct_options`, `get_report_answer_keys`, `get_admin_report_answer_keys` reading `questions` via `quiz_session_answers.question_id` — a write-once FK on the immutable, append-only `quiz_session_answers` table (no UPDATE/DELETE policies; resubmits are `ON CONFLICT DO NOTHING`), so a completed-session report still reveals the key for a question soft-deleted after it was answered (`get_report_correct_options`/`get_admin_report_correct_options`: MC key, mig 114 / #823; `get_report_answer_keys`: the non-MC sibling delivering short_answer `canonical_answer` + dialog_fill per-blank `blanks_config` canonicals + ordering per-slot `ordering_items` canonical text [mig 149] + diagram_label per-zone canonical label text via a 2-hop resolve on `diagram_config` [mig 156], mig 133 / #697; `get_admin_report_answer_keys`: the ADMIN, org-scoped sibling of `get_report_answer_keys`, whose four `RETURN QUERY` branches it copies verbatim and whose carve-out it therefore inherits unchanged — same `quiz_session_answers.question_id` write-once FK boundary, scoped by the caller's `organization_id` instead of by `student_id`, migration `20260824000100` / #991). Historical scoring and review then need access to records that may have been soft-deleted after the session was sampled. Any new instance of this exception must (a) cite the immutable, write-once column it relies on, (b) include an inline comment at the call site, and (c) cross-reference `docs/database.md` §3 "Scoring Soft-Deleted Questions". The `config.question_ids` write-once guarantee is enforced by trigger `trg_quiz_sessions_immutable_columns` (migration 079) — see `docs/database.md` §1 column-level immutability table.
