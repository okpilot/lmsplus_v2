...
- **AL — Concurrent duplicate `start_exam_session` race is constraint-enforced; only the error message is unmapped.** `start_exam_session` checks for an existing active session with an `IF EXISTS … RAISE` guard (migration `20260411000005`, persisted in the latest definition `20260428000004`) before inserting. Defense-in-depth backs this with the partial unique index `uq_active_exam_session` on `quiz_sessions (student_id, subject_id) WHERE mode = 'mock_exam' AND ended_at IS NULL AND deleted_at IS NULL` (migration `20260411000006`), so two concurrent active mock sessions for the same (student, subject) are **impossible** at the DB layer — a racer that slips past the EXISTS check has its INSERT rejected by the index. **Severity LOW:** the data-integrity invariant (≤1 active mock session per student+subject) is DB-enforced; the only residual gap is cosmetic — a concurrent loser receives a raw `unique_violation` (`23505`) rather than the friendly "an exam session is already in progress for this subject" message, and no score forgery or cross-student exposure is possible. Hardening tracked in **#754**: map the `uq_active_exam_session` `unique_violation` to that domain error inside `start_exam_session`, and optionally align the index's column list with `uq_internal_exam_session_active` (which additionally scopes `organization_id`).
