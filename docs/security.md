...
- Proxy check: the `__consent` cookie must equal `<tos>:<privacy>:<signed-in user id>` (`buildConsentCookieValue`), so a cookie left by another user on a shared browser never matches. No DB hit in the proxy. On a mismatch the proxy redirects to `/auth/consent-refresh`, which calls `check_consent_status()` and either reissues the cookie (consented) or redirects to `/consent`.
