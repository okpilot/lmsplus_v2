...
These two vectors from the exam-mode red-team review (issue #517) are documented rather than enforced by an automated red-team test: AJ is an admin-only trust boundary and AL is a low-probability race with no current DB constraint. Both are tracked in #517 and documented here so a future migration can close them if they recur.

- **AJ — Admin clearing `deleted_at` bypasses the partial unique index.** `exam_configs` enforces one active config per (org, subject) via a partial unique index `WHERE deleted_at IS NULL` (migration `20260411000007`). An admin who directly `UPDATE`s a soft-deleted config to clear `deleted_at` — instead of going through `upsert_exam_config` — re-enters that partial index without the RPC's de-duplication, allowing two active configs for the same (org, subject). This is an admin-only action (admins are trusted within their own org) and is unreachable by students. Mitigation if it recurs: a trigger on `exam_configs` that raises when `OLD.deleted_at IS NOT NULL AND NEW.deleted_at IS NULL`. Reactivation should always flow through `upsert_exam_config`.
- **AL — Concurrent duplicate `start_exam_session` is guarded sequentially, not by a constraint.** `start_exam_session` checks for an existing active session with an `IF EXISTS … RAISE` guard (migration `20260411000005`) before inserting; there is no DB-level unique constraint on (student, subject, active). Two requests racing within the same row-visibility window could both pass the check and create two active sessions. The window is a single round-trip and the client serializes start calls, so this is document-only. Mitigation if it recurs: a partial unique index on active sessions per (student_id, subject_id) `WHERE ended_at IS NULL`.
