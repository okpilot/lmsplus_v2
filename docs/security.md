...
### Known residual exam-mode vectors (red-team AJ / AL — document-only)

These two vectors from the exam-mode red-team review (issue #517) are **LOW-severity** residual risks documented rather than enforced by an automated red-team test, because both are bounded away from the high-value student-facing threats in §1: AJ is an admin-only action inside the admin's own org (a trusted boundary, not student-reachable, no answer exposure or privilege escalation), and AL is a sub-round-trip race the client already serializes. DB-level hardening for each is **tracked and prioritized** (AJ → #755, AL → #754) rather than deferred-until-incident; the entries below record the current state and the concrete mitigation each tracking issue will apply.

- **AJ — Admin clearing `deleted_at` bypasses the partial unique index.** `exam_configs` enforces one active config per (org, subject) via a partial unique index `WHERE deleted_at IS NULL` (migration `20260411000007`). An admin who directly `UPDATE`s a soft-deleted config to clear `deleted_at` — instead of going through `upsert_exam_config` — re-enters that partial index without the RPC's de-duplication, allowing two active configs for the same (org, subject). **Severity LOW:** admin-only within the admin's own org, unreachable by students; worst-case impact is a self-inflicted data-quality issue (two active configs for one subject), recoverable by re-soft-deleting one — not a security boundary breach. Hardening tracked in **#755**: a trigger on `exam_configs` that raises when `OLD.deleted_at IS NOT NULL AND NEW.deleted_at IS NULL` so reactivation must flow through `upsert_exam_config`.
- **AL — Concurrent duplicate `start_exam_session` is guarded sequentially, not by a constraint.** `start_exam_session` checks for an existing active session with an `IF EXISTS … RAISE` guard (migration `20260411000005`, persisted in the latest definition `20260428000004`) before inserting; there is no DB-level unique constraint on (student, subject, active). Two requests racing within the same row-visibility window could both pass the check and create two active sessions. **Severity LOW:** the check-to-insert window is a single round-trip and the client serializes start calls; worst-case impact is two concurrent active sessions for one subject, recoverable on completion — no score forgery or cross-student exposure. Hardening tracked in **#754**: a partial unique index on active sessions per (student_id, organization_id, subject_id) `WHERE mode = 'mock_exam' AND ended_at IS NULL AND deleted_at IS NULL`, mirroring `uq_internal_exam_session_active`, with `start_exam_session` mapping `unique_violation` to the existing "already in progress" error.
