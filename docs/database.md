...
`login_instructions_sent_at` — last time an admin sent this user their login details; NULL = never sent. `temp_password_expires_at` — NOT NULL = a temporary password's forced change is pending; `< now()` = expired. The only writer of the "arm" direction (stamping both columns on a send) is the SECURITY DEFINER RPC `record_login_instructions_sent(p_user_id)` (see RPC section below); the "clear" direction (setting `temp_password_expires_at` back to NULL after the user sets their own password) is written by the set-password/change-password Server Actions via `adminClient`, added in the login-instructions-email feature PR. No self-callable clear RPC exists — a caller clearing the flag without an actual password change would leave a stale "password set" UI state.
...
**Column UPDATE GRANT (mig 090, #773):** Defense-in-depth at the Postgres privilege layer on top of the RLS + trigger defenses. `authenticated` holds `UPDATE (full_name)` only; UPDATE on `role`, `organization_id`, `deleted_at`, and the two login-instructions columns above is revoked at the privilege layer (`REVOKE UPDATE ON users FROM authenticated` + `GRANT UPDATE (full_name) ON users TO authenticated`). A direct `UPDATE users SET role='admin'` now returns `42501` (permission denied for column) before the trigger or RLS even fires. Mirrors the `quiz_sessions` column-GRANT pattern (mig `20260605000001`).
...
CREATE TABLE questions (
  ...
  question_type   TEXT NOT NULL DEFAULT 'multiple_choice'
                    CHECK (question_type IN ('multiple_choice', 'short_answer', 'dialog_fill', 'ordering', 'diagram_label')),  -- ordering added mig 143 (#697 Phase 5); diagram_label added mig 150 (#697 Phase 6)
  ...
  diagram_config  JSONB NULL DEFAULT NULL,     -- diagram_label: {image_ref, zones:[{id,x,y,w,h}], labels:[{id,text}], answer:[{zone_id,label_id}]} (mig 150); answer is the zone->label key — REVOKE-gated by omission from mig 094 grant (N6, mirrors ordering_items); delivered as {image_ref,zones,labels(shuffled)} via get_quiz_questions, answer omitted
  created_by      UUID NOT NULL REFERENCES users(id),
  deleted_at      TIMESTAMPTZ NULL,            -- soft delete = question retired from bank
  deleted_by      UUID REFERENCES users(id) NULL,
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
  -- 5-branch type<->column discriminator (mig 143 added the ordering branch, mig 150
  -- added the diagram_label branch; #998 CR made it TOTAL — exact
  -- `ordering_items = '[]'::jsonb` / `diagram_config IS NULL` on every branch that
  -- doesn't own that column + a CASE-wrapped length check on the ordering/diagram_label
  -- branches, so a non-array ordering_items/diagram_config fails cleanly with 23514
  -- instead of raising a raw 22023).
...
**diagram_label shape guard (mig 150, #697 Phase 6):** The `is_valid_diagram_config()` CHECK (`IMMUTABLE PARALLEL SAFE`, same TOTAL/CASE-guarded-cast discipline as `is_valid_ordering_items`) validates `diagram_config = {image_ref, zones, labels, answer}`: `image_ref` a non-blank string; `zones`/`labels`/`answer` each arrays (CASE-wrapped to `'[]'` before `jsonb_array_elements` so a non-array degrades to 0 rows, never a 22023 abort); every zone `{id: non-blank string, x/y/w/h: number in [0,1]}` (each numeric cast CASE-guarded on `jsonb_typeof(...) = 'number'` first — Postgres does not guarantee OR-chain short-circuit, so the cast must be inside its own CASE, not after an `OR`), zone ids distinct, **at least one zone and at most 50 zones** (MAX_ZONES = 50, mirrors `apps/web/app/app/quiz/actions/diagram-validation.ts`); every label `{id: non-blank string, text: non-blank string}`, label ids distinct, **at least one label and at most 60 labels** (MAX_LABELS = 60, mirrors `apps/web/app/app/quiz/actions/diagram-validation.ts` for DB/client parity); every answer entry `{zone_id, label_id}` both non-blank strings referencing a real zone/label; **answer covers every zone EXACTLY once** (`answer` count = zone count AND distinct `answer.zone_id` count = zone count, which combined with the referential + distinctness guards forms a bijection zones → answer entries), and each `label_id` appears at most once (a one-to-one, *injective* zone→label assignment — NOT a full zone↔label bijection, since distractor labels may be unused; a repeated label would be unwinnable under consume-on-place). Labels MAY be unused (distractors are allowed by design — Decision 52); there is no requirement that every label appears in `answer`. **Reject unknown top-level keys:** the config shape is strictly `{image_ref, zones, labels, answer}` — any stray key (e.g., `answer_key`) is rejected by the CHECK (defence in depth, since `get_quiz_questions` re-projects on delivery). **Security invariant (seed-enforced, not DB-enforced):** zone ids and label ids must use unrelated random id schemes — otherwise the zone_id/label_id pairing in the answer-stripped delivery payload (mig 152) could leak the answer via naming correlation even with `answer` omitted.
...
to populate the handoff payload for cold-start exam resume. See Decision 36 in `docs/decisions.md`.
...
**`blank_index` ⇔ multi-row-answer-type write-time trigger (mig 131, #828; widened mig 144 for `ordering`, mig 151 for `diagram_label`):** A `BEFORE INSERT` trigger `trg_enforce_blank_index_shape_qsa` (mirrored on `student_responses` as `trg_enforce_blank_index_shape_sr`) enforces the biconditional `question_type IN ('dialog_fill', 'ordering', 'diagram_label') ⇔ blank_index IS NOT NULL` by reading the answered question's type — something the single-row `*_answer_shape_check` CHECK structurally cannot do (a CHECK cannot read another table). It rejects a non-multi-row-type row carrying a `blank_index`, and a `dialog_fill`/`ordering`/`diagram_label` row missing one (`ordering`'s `blank_index` = slot; `diagram_label`'s `blank_index` = the zone's derived ordinal position, mig 154). Shared trigger function `enforce_answer_blank_index_shape()` is **SECURITY INVOKER** (repo trigger convention — every answer insert originates inside a postgres-owned SECURITY DEFINER grading RPC, so the `questions` read runs as postgres and bypasses RLS) with **no `deleted_at` filter** (the question is reached via the session's frozen write-once `config.question_ids`; soft-deleted questions are still scored — §15 carve-out, see §3). Closes the gap where a future inserter or admin-form/import bug could persist a malformed `blank_index`.
...
**Answer shape discriminator (mig 095):** Mirrors `quiz_session_answers` constraint. Legacy callers omit `blank_index` (lands as NULL); rows conflict on the old `(session_id, question_id)` constraint semantics unchanged. New dialog_fill/ordering/diagram_label paths supply per-blank/per-slot/per-zone rows. The `blank_index` ⇔ multi-row-answer-type write-time trigger (mig 131, #828; widened mig 144/151) applies here too via `trg_enforce_blank_index_shape_sr` — see the `quiz_session_answers` note above.
...
### quiz_drafts
```sql
-- Temporary storage for interrupted practice sessions (quick_quiz, smart_review). Up to 20 per student.
-- APPROVED EXCEPTION: uses real DELETE (not soft delete) — drafts are disposable temp storage.
-- Session lifecycle: save → soft-delete the practice quiz_sessions row (Decision 49, #1085);
--   resume → mint FRESH quiz_sessions row from the draft's exact questions.
...
| `user_consents` | No | Append-only GDPR consent audit log (mig `20260327000057_user_consents.sql` — same pattern as `audit_events`): no `deleted_at` column, RLS blocks all UPDATE/DELETE, inserts only via `record_consent()` SECURITY DEFINER RPC |
...
**Service-role integration-test teardown — sanctioned hard-delete exception.** The `packages/db/src/__integration__/**` cleanup helpers (`cleanupTestData` / `cleanupReferenceData`) hard-DELETE ephemeral per-suite fixtures — including the immutable tables above (`audit_events`, `student_responses`, `quiz_session_answers`) — via the service-role client, to reset the disposable local integration DB between runs (a soft-delete can't reset accumulating fixtures). This is **test-infra only**: the no-hard-DELETE + immutable-table rules still govern the production app (enforced via RLS) and all non-`__integration__` `packages/db/src` code. The local `.claude/hooks/check-soft-delete-guard.mjs` excludes `__integration__`, and `.coderabbit.yaml` carries the matching exception scoped to `packages/db/src/__integration__/` teardown helpers only (declared within its `packages/db/src/**` path-instruction block).
...
> **Storage: `question-images` bucket (policy migrations 053, 055; bucket-ensure 20260410000009 — fresh environments only):** Admin INSERT/UPDATE/DELETE policies check that the first path segment (`(storage.foldername(name))[1]`) matches the admin's org. Not every writer produces that shape — derive them before relying on it: `grep -rl "question-images" apps/web`. Authenticated SELECT (`question_images_public_read`) is **intentionally unscoped**: the bucket is `public = true` (verified on prod 2026-09-15 by probe — `20260410000009` is `ON CONFLICT DO NOTHING`, so it sets the flag only where the bucket did not already exist), so GET-by-path bypasses RLS and scoping that policy alone would change nothing. Cross-org image visibility is an accepted risk for the single-org deployment (`docs/decisions.md` Decision 69). **Org-private images are a P1 gate (#814) before a second org is onboarded** — it must migrate every non-org-prefixed path, not just add policies.

> **Admin cross-row reads on `users` (binding pattern):** When an admin Server Action reads or *embeds* the `users` table to fetch peer rows (not just the calling admin's own row), use `adminClient` from `@repo/db/admin` — never the user-scoped supabase client. `users` has no `tenant_isolation` policy — it was created in mig 001 and dropped TWICE (`20260311000004:6`, `20260312000012:11`) because its self-referential subquery raised `infinite recursion detected in policy for relation users`. The live SELECT policy is `users_select` (`id = auth.uid() AND deleted_at IS NULL`, `20260312000012:14`); the only other is `users_update_own` (`20260326000056`). So the mechanism is stricter and more deterministic than a planner quirk: the policy matches ONLY the caller's own row, so a cross-row read returns no rows every time, not intermittently. Self-row reads work; cross-row reads come back `null` / empty (the exact shape depends on whether the query is a top-level select or an embedded resource). PostgREST applies RLS to embedded resources too, so even joining `users(...)` from another table triggers the same failure mode. Defense in depth: gate the path with `requireAdmin()` first, then apply `.eq('organization_id', organizationId)` on every adminClient query. Including: `apps/web/app/app/admin/students/queries.ts`, `apps/web/app/app/admin/dashboard/queries.ts`, `apps/web/app/app/admin/internal-exams/queries.ts`, `apps/web/lib/queries/admin-quiz-report.ts`, `apps/web/app/app/admin/dashboard/students/[id]/queries.ts`.
...
```
verb_noun pattern:
  get_quiz_questions         ← read, strips correct answers; widened in mig 118 to 15 RETURNS TABLE columns (+ question_type, dialog_template [tokens stripped to {{n}}], blanks_safe [index-only]; mig 145 adds the 16th column ordering_items_shuffled — canonical order hidden via shuffle, {id,text}-projected, NULL for non-ordering; mig 152 adds the 17th column diagram_config_public — {image_ref,zones,labels(shuffled)}, `answer` OMITTED, NULL for non-diagram_label) + active-user gate (security.md rule 12); supports multiple_choice, short_answer, dialog_fill, ordering, diagram_label; dialog_fill strip delimiter-hardened (mig 126) behind the mig-125 delimiter CHECK (#951)
  ...
  get_report_answer_keys           ← read, type-aware non-MC answer-key delivery for completed-session reports (student-scoped; sibling of get_report_correct_options): short_answer per-question canonical, dialog_fill per-blank canonical, ordering per-slot canonical item text, diagram_label per-zone canonical label text (2-hop resolve: zone → answer entry → label text); §15 carve-out on the questions read; active-user + ended_at gate; SECURITY DEFINER (migs 133/149/156, #697 Phases 4/5/6)
...
Widened in **mig 118** (supabase `20260621000100`) to support non-MC question types (`short_answer`, `dialog_fill`) for the VFR RT training practice quiz. The previous body used `CROSS JOIN LATERAL jsonb_array_elements(q.options)`, which silently dropped every row whose `options` is `'[]'` (i.e. all non-MC rows). The correlated-subquery `CASE` form from mig 105 replaces it. Widened again in **mig 145** (VFR RT Phase 5) to add a 16th column `ordering_items_shuffled` for the `ordering` question type. Widened again in **mig 152** (VFR RT Phase 6) to add a 17th column `diagram_config_public` for the `diagram_label` question type.
...
- `diagram_label` — `diagram_config_public` delivers `{image_ref, zones, labels}`; `answer` (the zone_id→label_id mapping) is **OMITTED entirely** and `labels` are shuffled (`ORDER BY random()`, same primitive as MC/ordering) so label array position cannot correlate back to a zone. `diagram_config` itself is REVOKE-gated from `authenticated` by omission from mig 094's grant (mirrors `ordering_items`, N6). NULL for non-diagram_label types.

```sql
-- Migration 152 — DROP + CREATE (RETURNS TABLE widened, 17th column added)
...
RETURNS TABLE (
  ...
  question_type           text,     -- 'multiple_choice' | 'short_answer' | 'dialog_fill' | 'ordering' | 'diagram_label'
  ...
  diagram_config_public   jsonb     -- diagram_label only: {image_ref, zones, labels(shuffled)} (`answer` omitted); NULL otherwise
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT
    ...
    -- OMITTED entirely — it is the answer key. NULL for non-diagram_label.
...
Admin-only RPC arming the forced password-change state on a student/instructor account. Stamps `users.login_instructions_sent_at = now()` and `users.temp_password_expires_at = now() + interval '7 days'` on the target row, then inserts one `user.login_instructions_sent` audit event. Exists because `audit_events` blocks direct INSERTs (`audit_no_direct_insert` = `WITH CHECK false`), so the write must flow through a SECURITY DEFINER function, and because `authenticated` has no UPDATE grant on either column (see the `users` table section above). No client call exists yet — the sending Server Action is added in the feature PR that follows this migration.
...
#### `get_admin_report_answer_keys` — admin variant of `get_report_answer_keys` (org-scoped)

Type-aware non-MC answer-key delivery for the admin session report: the ADMIN, org-scoped sibling of `get_report_answer_keys` (below). Delivers the correct answers for the **non-MC** questions (`short_answer`, `dialog_fill`, `ordering`, `diagram_label`) answered in a completed session belonging to the caller's org — `get_admin_report_correct_options` (above) covers the MC key only, so before this RPC the admin report had no answer-key read path for non-MC question types. Added in migration `20260824000100`.

**Parameters:** `p_session_id uuid`.

**Returns:** `(question_id uuid, question_type text, blank_index int, answer_key text)` — same shape and per-type row semantics as `get_report_answer_keys` below (one row per question for `short_answer`; one row per blank/slot/zone for `dialog_fill`/`ordering`/`diagram_label`; no rows for `multiple_choice`). The four `RETURN QUERY` branches are copied verbatim from `get_report_answer_keys`'s body.
...
- `diagram_label`: one row **per zone** — `blank_index` = the zone's 0-based ordinal position within `diagram_config.zones` (from `WITH ORDINALITY`, `idx - 1` — the SAME index `_grade_record_diagram_label`, mig 154, derives, so report rows line up with `quiz_session_answers.blank_index` by construction), `answer_key` = the display TEXT of the label canonically assigned to that zone via a **2-hop resolve**: zone (at this ordinal) → the `diagram_config.answer` entry matching `zone_id` → its `label_id` → that label's `text` in `diagram_config.labels`. Not a single-expression projection like ordering's `elem->>'text'`, because the canonical answer for a zone is not embedded in the zone object itself. `diagram_config` is REVOKE-gated from `authenticated` by omission from mig 094's grant (mirrors `ordering_items`, N6); SECURITY DEFINER bypasses the gate, scoped to the owning student's own completed session.
...
**Paging (client contract):** rows are per-element, not per-question, so one session can exceed PostgREST's `max_rows` cap: a session holds up to 500 questions and each contributes one row per blank/slot/zone. The cap is `POSTGREST_MAX_ROWS` (`apps/web/lib/supabase-paginate.ts`), which tracks `max_rows` in `supabase/config.toml`; read it there rather than trusting a number quoted here. Every caller must route through `fetchAllRpcRows` (`lib/supabase-rpc.ts`) rather than reading this RPC unpaged; it re-fetches ordered by `(question_id, blank_index)` when an unpaged read returns exactly the cap. Derive the current call sites: `grep -rn "fn: '\(get_report_answer_keys\|get_admin_report_answer_keys\)'" apps/web/lib` (anchoring on the `fn:` key keeps comments and test mocks out of the result). Same truncation class as umbrella #668.
...
Added in **mig 119** (supabase `20260621000200`, #697 Phase 2). Extended in **mig 146** (VFR RT Phase 5) to add `ordering` grading and a 5th parameter `p_order`. Extended in **mig 153** (VFR RT Phase 6) to add `diagram_label` grading and a 6th parameter `p_mapping`. Companion to `check_quiz_answer` (mig 117): that RPC handles `multiple_choice`; this one handles `short_answer`, `dialog_fill`, `ordering`, and `diagram_label`. All are practice-mode-only immediate-feedback graders.

**Signature change (mig 153):** `DROP FUNCTION IF EXISTS check_non_mc_answer(uuid, uuid, text, jsonb, jsonb)` then `CREATE FUNCTION` with the 6-arg signature (CREATE OR REPLACE cannot change the argument list; the old 5-arg overload is removed to avoid ambiguous-call errors). Every existing branch (short_answer/dialog_fill/ordering) and the guard prologue is re-emitted **verbatim** from the latest definition (mig 146) — only the `diagram_label` branch + the `diagram_config` fetch + the `p_mapping` param are new. The 4-way `answer_type_mismatch` parity is maintained: every branch's mismatch guard now also rejects a non-NULL `p_mapping` when `p_mapping` is not that branch's own param.
...
{
  "is_correct": true,
  "correct_answer": "Wilco",          // short_answer: canonical text; null for dialog_fill, ordering, and diagram_label
  "blanks": [                          // dialog_fill: per-blank results; null for short_answer, ordering, and diagram_label
    { "index": 0, "is_correct": true, "canonical": "Wilco" },
    { "index": 1, "is_correct": false, "canonical": "Negative" }
  ],
  "correct_order": ["id-a", "id-b"],  // ordering: canonical item IDs in array order; null for short_answer, dialog_fill, and diagram_label
  "correct_mapping": [{ "zone_id": "z1", "label_id": "l3" }],  // diagram_label: raw canonical zone->label answer array; null otherwise
  "explanation_text": "...",
...
- `p_question_type TEXT DEFAULT NULL` — question-type filter. `NULL` (default) = unrestricted (all types); a specific value (e.g. `'multiple_choice'`) = only that `question_type`. Like calc-mode/has-image, it **AND-restricts** the pool. Added for Study Mode so the random-ID **fetch** is MC-only on mixed-type subjects; the pre-start **count badge** passes the same `p_question_type` to `get_filtered_question_counts` (mig 157 / `20260629000800`), so the fetch and count agree (count == quiz). This resolved #1003. Study/Discovery supplies `'multiple_choice'` via `getFilteredCount` (`apps/web/app/app/quiz/actions/lookup.ts`) and the seeding path (`apps/web/lib/queries/quiz-session-queries.ts`); existing quiz/exam callers omit it (DEFAULT NULL) and are unaffected (mig 134).
...
**Migration:** `20260528000001_filtered_question_pool_rpcs.sql`; `p_calc_mode` added in `20260611000400_calc_mode_filtered_question_pool.sql`, which DROPs + recreates the three functions (signature change) (#837); `p_question_type` added in `20260626000100_get_random_question_ids_question_type.sql`, which DROPs + recreates `get_random_question_ids` + `_filtered_question_pool` with the trailing optional param (Study Mode); the same `p_question_type` was then added to `get_filtered_question_counts` in `20260629000800_get_filtered_question_counts_question_type.sql` so the count badge is MC-aware too (#1003 / #1008).
...
**Internal helper:** `_filtered_question_pool(p_subject_id, p_topic_ids, p_subtopic_ids, p_filters, p_calc_mode, p_has_image, p_question_type)` — shared `STABLE SECURITY INVOKER` SQL function. Defines the filtered pool exactly once so `get_random_question_ids` and `get_filtered_question_counts` are structurally guaranteed to agree (count == quiz). The guarantee holds when both callers pass the SAME pool arguments — in particular the same `p_question_type` (DEFAULT NULL = unrestricted for quiz/exam callers). Discovery (Study Mode) passes `p_question_type='multiple_choice'` to BOTH `get_random_question_ids` (mig 134) and `get_filtered_question_counts` (mig 157), so the count badge and the MC-only fetch agree on mixed-type subjects (count == quiz; #1003 resolved). Both the calc-mode AND-clause (#837) and the has-image AND-clause (#864) live here (one place each), so both wrappers inherit them. Requires `GRANT SELECT (has_calculations)` to `authenticated` (mig 107) since it reads the column as the SECURITY INVOKER student. Prefer the wrapper RPCs over calling the helper directly: a direct call returns one row per pool member and can hit the 1000-row cap.
...
**Migration:** `20260528000001_filtered_question_pool_rpcs.sql`; `p_calc_mode` added in `20260611000400_calc_mode_filtered_question_pool.sql` (#837); `p_question_type` added in `20260629000800_get_filtered_question_counts_question_type.sql` so the count badge is MC-aware for Study Mode (#1003 / #1008).
...
6. Trim again (final trim, **mig 128 / #921**) — steps 4–5 can leave a stray edge space when punctuation was adjacent to a leading/trailing space (`". hello"` → `" hello"`); without this, grading penalizes a correct answer. (The comparison itself is no longer a bare `normalize_answer(response)` = `normalize_answer(canonical)` equality test — since mig 158 it goes through `answer_matches()`, which normalizes both sides and then allows a bounded spelling tolerance. The final trim still matters: `answer_matches` short-circuits on exact equality, and a stray edge space would defeat that.) Wraps the outermost `regexp_replace` in `trim()`; CREATE OR REPLACE, no signature change. The TS mirror gains the matching final `.trim()` in the same change.
...
Helper (migration 158, 2026-08-15). Decides whether a student's answer matches a candidate
(canonical or synonym). Pure function (IMMUTABLE, PARALLEL SAFE). Grants do **not** mirror
`normalize_answer`: EXECUTE is REVOKEd from `PUBLIC`, `anon` and `authenticated`, so no client role
may call it. Production grading never needs that — all four graders are SECURITY DEFINER and invoke
it as the `postgres` owner, whose implicit EXECUTE survives the REVOKE. `service_role` is then
GRANTed EXECUTE **explicitly**, which is how the integration tests reach it. That GRANT is
load-bearing, not belt-and-braces: measured on a clean `supabase db reset`, a freshly created
function in this schema lands with `proacl = {postgres=X/postgres}` alone, so without it
`has_function_privilege('service_role', …)` is FALSE and the integration tests fail on a permission
error. Do not "simplify" it away on the assumption that platform default privileges cover
`service_role` — they did not here. Omitting a GRANT would not have closed the function either:
Postgres grants EXECUTE to `PUBLIC` by default, so only the REVOKE does.

**Signature:** `answer_matches(p_norm_response text, p_candidate text) → boolean`. **Both**
arguments are normalized inside the function, so either may be passed raw or already-normalized —
`normalize_answer` is idempotent, so re-normalizing the left argument costs nothing. The parameter
name `p_norm_response` records where callers happen to pass an already-normalized value; it is not
a precondition, and passing a raw string there is not a bug.
...
**Logic:**
1. Exact match after normalization → true (unchanged pre-mig-158 behaviour).
...
3. Per word, in order: identical → next; **any token containing a digit must match EXACTLY**;
   otherwise allow a Levenshtein distance of **at most 1**, and only for words of **5 or more
   characters of the CANDIDATE** — the floor reads the candidate only, so a shorter student
   token can still match a 5+ candidate (`answer_matches('limb','climb')` is true). A single adjacent transposition
   counts as distance 1. A token longer than **255 characters** is never fuzzy-matched, because
   `levenshtein` raises above that length; it returns false rather than aborting the submission.
4. Total distance across the whole answer is capped at 2 — at most two single-edit words.
...
why a candidate under 5 characters is never fuzzy-matched; `sigth`/`sight` is distance 2 despite being a
single swap, which is why transposition is special-cased instead of raising the threshold (raising
it would also admit `depart`/`report`, distance 2 at length 6). There is deliberately **no wider
tier for long words**: prefix negation (`un-`/`in-`/`de-`/`dis-`) is exactly 2 edits at length ≥ 8,
so a 2-edit tier would accept `runway serviceable`/`runway unserviceable`, `northbound`/
`southbound` and `increase`/`decrease` — live ICAO phraseology, not typos. `airfiled`/`airfield`,
...
`to_regprocedure('extensions.levenshtein(text,text)')` DO-block immediately after `CREATE
EXTENSION`, because `WITH SCHEMA` is silently ignored when the extension already exists.
