...
**Mig 150 — append-only (student-facing); grader finalizes once**
...
Per-section audio/transcript records. NO `deleted_at` (no soft-delete). Append-only from the student's perspective — students have no UPDATE/DELETE (RLS `USING (false)`); the row is inserted at submit (`status='grading'`), then the postgres-owned grader `write_oral_section_grade` (bypasses RLS) performs a **one-time finalize** filling `transcript_text` / `transcript_meta` and flipping `status` to `graded` (or `failed`). One row per section (1–5) per session via UNIQUE constraint; never mutated again after finalize. Same write-once-then-immutable shape as `quiz_session_answers` / `student_responses`.
...
verb_noun pattern:
  ...
  write_oral_section_grade   ← write, grader-only (Class B): SECURITY DEFINER, REVOKE EXECUTE FROM PUBLIC/anon/authenticated, GRANT service_role only; no auth.uid() check; state-based replay guard (process only if section status='grading'); writes transcript + per-descriptor scores + metering ledger atomically; finalizes session when all planned sections graded (planned_sections = 1 practice / 5 mock, mig 153; computes per-descriptor aggregate MIN, sets session.status='graded' + total_final_level, writes `oral_exam.graded` audit event with total_final_level + planned_sections metadata); FOR UPDATE serialization on section + session (mig 154, Decision 52)
```
