...
verb_noun pattern:
  ...
  write_oral_section_grade   ← write, grader-only (Class B): SECURITY DEFINER, REVOKE EXECUTE FROM PUBLIC/anon/authenticated, GRANT service_role only; no auth.uid() check; state-based replay guard (process only if section status='grading'); writes transcript + per-descriptor scores + metering ledger atomically; finalizes session when all 5 sections graded (computes per-descriptor aggregate MIN, sets session.status='graded' + total_final_level, writes `oral_exam.graded` audit event with total_final_level + planned_sections metadata); FOR UPDATE serialization on section + session (mig 154, Decision 52)
```
