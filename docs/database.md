...
> **Storage: `question-images` bucket (policy migrations 053, 055; bucket-ensure 20260410000009 — fresh environments only):** Admin INSERT/UPDATE/DELETE policies check that `(storage.foldername(name))[1]` — the FIRST path segment — matches the admin's org. **Only the Server Action writes a path shaped that way:** `uploadQuestionImage` (`apps/web/app/app/admin/questions/actions/upload-question-image.ts`) resolves the admin's `organization_id` and writes `{org_id}/{uuid}.{ext}`. **The set of writers to this bucket is OPEN — derive it, never trust a count stated here:** `grep -rl "question-images" apps/web --include=*.ts` (match on the BUCKET NAME, which every writer carries even when it reaches the bucket through a constant — `import-questions.ts:56` is `const STORAGE_BUCKET = 'question-images'`. Do NOT anchor on `.upload(`: that misses any call whose `.from(...)` sits on its own line, which is how `seed-quiz-setup-eval.ts` is written and how it was missed once already). As ILLUSTRATIONS as of 2026-09-15 and not as a closed set, the non-org-prefixed writers are `apps/web/scripts/import-questions.ts` (`{subjectCode}/{filename}`) and `apps/web/scripts/seed-quiz-setup-eval.ts` (`eval/{label}.svg`), both under the SERVICE ROLE key — which bypasses RLS, so neither violates the policy, but objects in this bucket do NOT share a first-segment scheme. Authenticated SELECT (`question_images_public_read`) is **intentionally unscoped** — all authenticated users read all images (quiz display), and the bucket is `public = true` (verified on prod 2026-09-15; `20260410000009` sets that flag for fresh environments only — `ON CONFLICT DO NOTHING` makes it a no-op on the pre-existing prod bucket) so GET-by-path bypasses RLS entirely. Cross-org image visibility is an accepted risk for the current single-org deployment (see `docs/decisions.md` Decision 69). **Org-private images are a P1 gate (#814) that must land before a second org is onboarded** — org-scoping the SELECT policy alone would be theatre on a public bucket, and every non-org-prefixed path must be migrated (or its writer changed to org-prefix) in the same work, or org-scoped policies would orphan those objects. Enumerate the writers with the grep above at the time the work starts, rather than from the illustrations named here.
