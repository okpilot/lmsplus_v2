...
> **Storage: `question-images` bucket (policy migrations 053, 055; bucket-ensure 20260410000009 — fresh environments only):** Admin INSERT/UPDATE/DELETE policies enforce org-scoped path isolation — images are stored at `{org_id}/{filename}` and policies check `(storage.foldername(name))[1]` matches the admin's org. Authenticated SELECT (`question_images_public_read`) is **intentionally unscoped** — all authenticated users read all images (quiz display), and the bucket is `public = true` (verified on prod 2026-09-15; `20260410000009` sets that flag for fresh environments only — `ON CONFLICT DO NOTHING` makes it a no-op on the pre-existing prod bucket) so GET-by-path bypasses RLS entirely. Cross-org image visibility is an accepted risk for the current single-org deployment (see `docs/decisions.md` Decision 69). **Org-private images are a P1 gate (#814) that must land before a second org is onboarded** — org-scoping the SELECT policy alone would be theatre on a public bucket. The upload action (`uploadQuestionImage`) resolves the admin's org and prefixes the path automatically.
