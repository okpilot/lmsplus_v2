...
`login_instructions_sent_at` — last time an admin sent this user their login details; NULL = never sent. `temp_password_expires_at` — expiry of the temporary password; NULL = none recorded; `< now()` = expired. Writers: the SECURITY DEFINER RPC `record_login_instructions_sent(p_user_id)` (see RPC section below) stamps both on a send; `clearTempPassword()` sets `temp_password_expires_at` to NULL, called only after a successful self `auth.updateUser` password change (set-password, Settings, forgot-password completion); an admin-issued reset (`reset-student-password.ts`, orchestrated by `issueTempPassword()` in `issue-temp-password.ts`) calls `armTempPassword()` to arm it 7 days out BEFORE writing the new Auth password, so a write failure never leaves a live admin-typed password unforced; on a successful Auth write it calls `armTempPassword()` a second time, so a self-service clear racing between the first arm and the Auth write can't leave the new admin-typed password unforced either — the admin is told to reset again if only this second arm fails; if the Auth write itself fails, `restoreTempPasswordExpiry()` restores the value read before the first arm, compare-and-set on that arm's own value so it never overwrites a self-service clear landing after that arm (a clear landing between the pre-arm read and the arm is re-armed — accepted) — both writers in `apps/web/lib/auth/temp-password-admin.ts` (service role); `create-student.ts` sets it directly to 7 days out on the new profile row's insert (`TEMP_PASSWORD_TTL_MS`, same constant). Readers: every caller of `readTempPassword` or its wrapper `readTempPasswordState` (both `apps/web/lib/auth/temp-password.ts`; the self-service writers read through `refuseIfTempPasswordExpired`) — `git grep -nE 'readTempPassword(State)?\(|refuseIfTempPasswordExpired\(' -- apps/web ':!*.test.*'`.
...
`anon` holds no privilege on any public table; `authenticated` holds only the table commands a permitting RLS policy allows (migrations `20260925000200`, `20260925000300`, default privileges too). A new postgres-owned table defaults to `anon` nothing and `authenticated` SELECT only — a migration adding a write path must GRANT it explicitly. `anon` and PUBLIC hold EXECUTE on no public function (migration `20260925000400`); a new postgres-owned function defaults to `authenticated` EXECUTE and nothing for `anon`.
