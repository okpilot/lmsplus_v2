...
4. **Sibling tables are out of scope.** `organizations`, `question_banks`, `courses` and `lessons` carry the identical unqualified policy but have no role-gated write policies, so the carve-out does not apply; they are tracked separately under security rule 12 (sibling parity).
