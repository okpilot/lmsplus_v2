...
**Context**: VFR RT Training Part 3 (Phases 5–6) introduces two drag-and-drop question types — `ordering` (sequence radio/phraseology steps) and `diagram_label` (map zones → labels). The quiz Study runner reused for VFR RT has no drag primitive; React's native HTML5 drag-and-drop is unreliable on iOS Safari / iPad, the primary device for the EASA exam cohort.
