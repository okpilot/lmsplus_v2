...
- Detection is POSITIONAL: a hex token after a commit-context word, a possessive, a bare
  parenthetical `(<sha>)`, or bare at a line start. The word list is in the hook's `TRIGGER_WORDS`
...
- Two FOREIGN-SHA exclusions, both found by measurement against real history, not anticipated:
  ...
  parenthetical whose IMMEDIATELY-PRECEDING token is an `owner/repo@ref` action pin is skipped —
...
- A backtick requirement was tried and REJECTED: zero recall, because this repo writes SHAs bare
  ("added in 269667d7").
- Recall is **80%** — 280 of 352 — where the denominator is every distinct mixed-hex 7-40 token in
  the last 400 messages (URLs and trailers stripped) that `git rev-parse` resolves, and the
  numerator is those `extractRefs` returns. Stating the metric matters: a position-agnostic token
  scan gives a different denominator and so a different percentage for the same code. The only 4
