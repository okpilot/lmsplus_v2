...
- `/consent` page: two checkboxes (TOS required, privacy required). Continue button disabled until both boxes checked. Server Action calls `record_consent()` twice (one per document), sets cookie with current versions, redirects to `/app`.
