...
**Two known limits on WHEN it runs.** `gh pr merge --squash` is server-side, so no local hook gates
the message that lands on master. And `git commit --amend` re-validates prose that was TRUE when
written: if a cited commit was since rebased away and gc'd, the amend is blocked with no honest way
forward. Neither is fixed here; both are recorded so the next author is not surprised. Branch messages were each gated when authored; hand-edited squash prose is not.
