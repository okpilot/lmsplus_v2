...
4. **Edge Function** (`supabase/functions/score-oral-section`): triggered on `INSERT` to `oral_exam_section_responses` (webhook), calls ElevenLabs Scribe (→ transcript), calls Claude (Opus) with transcript + section-specific rubric (→ per-descriptor scores + rationale), calls `write_oral_section_grade` RPC with the results. Cost tracking: extracts token/char counts from provider responses.
