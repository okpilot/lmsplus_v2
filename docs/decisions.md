...
2. **Five RPCs** (migs 151–152, SECURITY DEFINER, full guard set per security.md rule 12):
   ...
   - **Class B (grader-only, mig 154):** `write_oral_section_grade(p_response_id, p_transcript, p_transcript_meta, p_descriptor_scores, p_usage)` — REVOKE EXECUTE FROM PUBLIC/anon/authenticated; GRANT service_role only. No auth.uid() check (grader runs as service role). State-based replay guard (process only if section.status='grading'). FOR UPDATE serialization on section + session (mutual exclusion between concurrent section webhooks). Atomically writes transcript + per-descriptor rows + metering ledger; when all 5 sections graded, computes per-descriptor MIN aggregate + sets session.status='graded' + final_level + writes `oral_exam.graded` audit event (documented rule-12 deviation: actor/role are session-derived, not re-gated on student.deleted_at, so service-role finalizer can complete even for soft-deleted students).
