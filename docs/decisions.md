...
**Divergence CLOSED by #990** (2026-09-01): the two list surfaces this decision left rendering
`correctCount / totalQuestions` — `admin/dashboard/students/[id]/_components/clickable-session-row.tsx`
and `admin/internal-exams/_components/attempts-table.tsx` — now render the same item/item fraction as
the report, so the "29 / 25" in a list and "29 / 29" one click later can no longer disagree. Re-derive
the set rather than trusting this list:
`grep -rn "correctCount}/{.*totalQuestions\|{correct}/{total}" apps/web/app --include="*.tsx"`.

The formatting rule itself is no longer duplicated per surface: `deriveStats` and both list cells call
`formatCorrectFraction(correctCount, answeredItems)` (`apps/web/lib/utils/format-correct-fraction.ts`),
so the em-dash-on-zero case and the spacing cannot drift apart again. Adopting it also changed the two
lists' punctuation from `15/20` to the report's spaced `15 / 18` — deliberate, so the two surfaces read
identically. The per-session item counts come from `fetchAnsweredItemCounts`
