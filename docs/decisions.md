...
**Decided:**
- Create red-team suite: Playwright attack specs executing exploit chains against local Supabase (same as production); 51 specs as of 2026-06-29 covering OWASP A01/A02/A03/A07/A09 and A10:2025 (exceptional-condition handling / error-path information disclosure)
...
### Decision 52: AI ICAO ELP (English Language Proficiency) async-graded oral exam (migs 150–152, Slice 0)

**Date**: 2026-07-02

**Context**: LMS Plus v2 serves EASA pilots in multiple languages. The ICAO ELP (English Language Proficiency) oral exam is a regulatory requirement for non-English-native pilots. The feature is a **student-facing mock oral exam** modeled on the ICAO standards: 5 sections (Interview, Picture Description, Communication, Listening, Video Task), each recorded via audio, with grades on 6 descriptors (Pronunciation, Structure, Vocabulary, Fluency, Comprehension, Interaction) at levels 1–6. **Key architectural choice**: grades are computed **asynchronously** — a student records all 5 sections, then a Supabase Edge Function transcribes (ElevenLabs Scribe) and grades (Claude Opus 4.8) off the request path, writing back per-descriptor scores and a metering ledger for V2 billing. Synchronous grading would block the UI indefinitely; async scoring decouples the student experience from LLM latency.

**Decision**: Build Slice 0 (foundation infrastructure) with:
1. **Four tables** (migs 150 — FORCE RLS via tenant-isolation + org policies): 
   - `oral_exam_sessions` — mutable state machine (status: in_progress/grading/graded/discarded), soft-deletable, one active per student (UNIQUE partial index independent of `quiz_sessions`). Stores the session config (section topics, maybe exam variant).
   - `oral_exam_section_responses` — append-only per-section audio records (5 per session, UNIQUE(session_id, section_no)). Status tracks: grading → graded/failed.
   - `oral_exam_descriptor_scores` — append-only per-descriptor 1–6 level rows. Stored both per-section and as a per-descriptor aggregate (section_no NULL = final). Idempotent finalization via partial unique indexes prevents dups on replay.
   - `elp_usage_events` — immutable metering ledger (no authenticated INSERT; service-role writes only via the Edge Function). Tracks STT seconds, TTS chars, LLM tokens, and cost estimates — input to V2 billing.
2. **Five RPCs** (migs 151–152, SECURITY DEFINER, full guard set per security.md rule 12):
   - **Class A (student-facing, mig 151):** `start_oral_exam_session()` (idempotent resume), `submit_oral_section_response(p_session_id, p_section_no, p_audio_path, p_duration_ms)` (records audio, advances session to grading when all 5 submitted), `discard_oral_exam_session(p_session_id)` (soft-delete), `get_oral_exam_report(p_session_id)` (ended_at-gated reveal).
   - **Class B (grader-only, mig 154):** `write_oral_section_grade(p_response_id, p_transcript, p_transcript_meta, p_descriptor_scores, p_usage)` — REVOKE EXECUTE FROM PUBLIC/anon/authenticated; GRANT service_role only. No auth.uid() check (grader runs as service role). State-based replay guard (process only if section.status='grading'). FOR UPDATE serialization on section + session (mutual exclusion between concurrent section webhooks). Atomically writes transcript + per-descriptor rows + metering ledger; when all planned sections graded (planned_sections = 1 practice / 5 mock, mig 153), computes per-descriptor MIN aggregate + sets session.status='graded' + final_level + writes `oral_exam.graded` audit event (documented rule-12 deviation: actor/role are session-derived, not re-gated on student.deleted_at, so service-role finalizer can complete even for soft-deleted students).
3. **Storage bucket** (supabase/migrations mirror only): `elp-recordings` (private, owner-prefix RLS — student prefix in audio_path prevents cross-student replay injection).
4. **Edge Function** (`supabase/functions/score-oral-section`): calls ElevenLabs Scribe (→ transcript), calls Claude (Opus) with transcript + section-specific rubric (→ per-descriptor scores + rationale), calls `write_oral_section_grade` RPC with the results. Cost tracking: extracts token/char counts from provider responses. **Trigger (Slice 1, supersedes the Slice-0 webhook-only description below): app-invoke** — `submitSectionResponse()` fires `triggerSectionScoring()` (`apps/web/lib/elp/trigger-scoring.ts`) via `next/server`'s `after()`, which POSTs directly to the function with the `x-webhook-secret` header. A Supabase Database Webhook on `INSERT` to `oral_exam_section_responses` remains a possible prod-ops alternative (the function still accepts the same `{record}` payload shape from either source) but must **not** be co-wired with the app-invoke — both firing for the same submission would double-transcribe and double-score (double STT + LLM cost). Exactly one trigger is active at a time; the choice is made at merge/deploy time, not left to both paths running simultaneously.

**Guard set** (Class A student RPCs): auth.uid() null-check, active-user gate (`deleted_at IS NULL` user read), org resolution, ownership scope (`student_id = auth.uid()`, explicit — not delegated to RLS, since these tables carry multiple permissive SELECT policies, rule 11), soft-delete filter on every session read, `SET search_path = public`. `start_oral_exam_session` + `discard_oral_exam_session` write `audit_events` (cached-role subquery per rule 10); the grader (`write_oral_section_grade`, service-role, no auth.uid()) also writes an `oral_exam.graded` audit event on finalization with metadata {total_final_level, planned_sections} (documented rule-12 deviation: actor/role are session-derived and NOT re-gated on soft-delete state). The oral exam has its own single-active partial unique index (`uq_one_active_oral_exam_per_student`), independent of `quiz_sessions` (it shares no MC pool, so the #1011 answer-oracle does not apply).

**Slice 0 Scope**: the async-scoring SPINE only — no student-facing section-runner UI yet (no MediaRecorder capture). Students cannot yet take the exam from the UI; the report shell + Server Actions + RPCs exist and are proven end-to-end via a seed script + integration tests. Metering is recorded but not enforced (V2 = Stripe + credits). Next slices (per the approved plan): §1 Interview student section UI → §2/§4/§5 → §3 live ATC (ElevenLabs ConvAI) → V2 payments. Grading is fully AI/automatic — there is no instructor-grading feature.

**Rationale**: Async grading is the load-bearing decision. Synchronous would require a spinner and a 30–60s wait per section (LLM latency); the user experience is unusable. Webhook-driven pipeline separates concerns: student records (synchronous, fast), Edge Function transcribes + grades (asynchronous, off-request-path), RPC writes back (atomic, per-section). No client-side polling. The RPC as the final writer (Class B) mirrors the `batch_submit_quiz` + internal-helper pattern from Decision 47 — single authz boundary, REVOKE-gated grader internals, state-based replay guard (idempotent on webhook retry). Per-descriptor scores + aggregate MIN is simpler than per-section weighted averages (rubric authority belongs to the descriptor, not the section). State machine (in_progress → grading → graded) simplifies webhook error handling (a failed transcription keeps the section in grading; the student can re-submit or abandon).

**Binding constraints**: the grader-grant model is the security linchpin — `write_oral_section_grade` is `REVOKE EXECUTE FROM PUBLIC, anon, authenticated` + `GRANT service_role` (Supabase auto-grants anon/authenticated, so the REVOKE is mandatory, not omittable). The caller-supplied `p_audio_path` is validated against the caller's own `{org}/{student}/{session}` prefix (the grader dereferences it with the service role, bypassing storage RLS). `elp_usage_events` has no authenticated INSERT policy + `CHECK quantity >= 0` (metering-forgery guard). Full multi-tenant isolation applies (rule 12 guard set above) — this is student-facing prep, not an internal-only prototype.

**Scope**: the async-scoring foundation — 4 tables, 5 RPCs, storage bucket, Edge Function, Server Actions, report shell. No section-runner UI, no billing enforcement yet. Metering ledger is the V2-billing foundation, not yet consumed by pricing logic.

---

*Last updated: 2026-07-02 — Decision 52: AI ICAO ELP (English Language Proficiency) async-graded oral exam (migs 150–152, Slice 0 foundation) | Earlier 2026-06-30 — Decision 51: `ordering` question type stores PER-SLOT answer rows (dialog_fill-clone) for partial credit, deviating from spec N7's single-JSON-row; mig-144 trigger widening; `_grade_record_ordering` REVOKE-gated per-slot helper (mig 147); get_quiz_questions shuffled delivery (mig 145); batch_submit_quiz ordering dispatch + partial-credit rollup (mig 148) (VFR RT Phase 5) | Earlier 2026-06-30 — Decision 50: dnd-kit (core/sortable/utilities) for drag-and-drop question types (ordering, diagram_label); sensors [Pointer, Touch(delay250/tol5), Keyboard] for iPad (VFR RT Phase 5) | Earlier 2026-06-29 — Decision 49: single active quiz_sessions row per account across all modes (#1011) — global partial unique index `uq_one_active_session_per_student` + per-start-RPC `another_session_active` guard + Discovery-as-real-row + `endDiscovery` + `ActivePracticeBanner` recovery; Decision 48 amended (Discovery now a real ephemeral `mode='discovery'` DB row, still non-resumable + nothing-scored). | Earlier 2026-06-27 — Decision 48 reworked: Discovery reuses the real quiz runner via an ephemeral pre-marked sessionStorage handoff (navigate to `/app/quiz/session`, correct option pre-marked, explanation behind its tab, Exit not Finish, nothing persisted; persisted `ActiveSession` typed resumable-only). | Earlier 2026-06-26 — Decision 48 UI label: Study Mode surfaced as Discovery (first/default segment of New Quiz ModeToggle; internal identifiers remain `study`) | Decision 48: Study Mode `get_study_questions` RPC returns MC answers on-demand (deliberate exposure; exam-integrity enforced by the active-exam-session guard — raises `active_exam_session` mid-exam, red-team EO6) | Earlier 2026-06-21 — Decision 47: batch_submit_quiz per-type dispatch via internal helpers gated by REVOKE EXECUTE FROM PUBLIC, anon, authenticated (single authz boundary in the dispatcher) + DISTINCT-question partial-credit scoring matching the exam (VFR RT Phase 2.3) | Earlier 2026-06-21 — Decision 46: app-layer DB integration test tier (`apps/web/vitest.integration.config.ts`, real-DB under RLS) + mechanical schema-contract guards (soft-delete column guard, test-helpers import ban) + HARD new-query-site integration-test policy (#925) | Earlier 2026-06-20 — Decision 45: VFR RT training reuses the quiz Study UI on a dedicated `/app/vfr-rt` route (training before exam; bespoke exam UI parked) | Earlier 2026-06-19 — Internal Exam code email feature (mig 110): Decision 44 on Resend transactional email provider + `record_internal_exam_code_emailed()` RPC | Earlier 2026-06-10 — Phase A (migs 094–104): Decisions 41–43 on column REVOKE/GRANT privilege gate, UNIQUE NULLS NOT DISTINCT per-blank answers, and per-part VFR RT grading (≥75% per part, immutable config.question_ids); 6 new RPCs documented*
