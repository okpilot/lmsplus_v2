...
in `scripts/dialog-fill-content.ts`: a recall blank needs an `[atc]` line directly above it, or an
`unanchored` declaration that NAMES the competing phrase and shows what visible text excludes it.
...
### Decision 56: grading tolerates spelling slips, never digits (2026-08-15, mig 142)
...
**Decision**: New `public.answer_matches(normalized_response, raw_candidate)` (mig 142) replaces the
equality test in all four text graders. Bounded spelling tolerance — Levenshtein ≤2 from 8
characters, ≤1 from 5, exact below 5, single adjacent transposition counted as one edit, whole-answer
budget 2. **Any token containing a digit must match exactly.** See `docs/database.md` for the full
rule and the measurements behind each threshold.

**Consequence accepted**: All four graders changed together (sibling-parity), so a typo cannot score
in practice and fail in the exam. Validated by replaying 23 closed-book eval failures through the new
