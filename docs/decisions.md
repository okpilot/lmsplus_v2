...
exactly: agent-set closure in both directions, each agent's `model`/`tools`/`memory` against its
frontmatter, the single-writer invariant, existence of every declared path, the four phase-ordering
constraints across five phases, and the model literal in each file that hardcodes one. It does **not** assert that a
path list is COMPLETE — dropping `apps/web/app/auth/**` would still pass. Nine mutations were run in
a throwaway worktree (agent model changed, read-only agent gains Write, `memory:` removed, rogue
write-capable agent added, agent dropped from the spec, phases reordered, pre-push hook model drifted,
an agent moved INTO `post-commit-core`, an unknown role value); all nine went red, baseline and restore
green. implementation-critic reproduced eight of them independently.
