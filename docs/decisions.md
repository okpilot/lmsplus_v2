...
**Still open, deliberately.** The four policy contradictions this entry left for a human call are
SETTLED in Decision 63. Still open: several
...
## Decision 63: five policy contradictions settled; `.claude/**` joins the lint (2026-09-08)

Each of these needed a human call because both sides were defensible. All five are the user's, and
are written into the rules here rather than proposed.
...
2. **Red-team coverage-gap filings still COUNT toward `filed >= closed`.** `agent-red-team.md`
   mandates filing them, so obeying it could fail the PR's own budget. They are real backlog, and
   exempting them by ORIGIN would key the exemption on the one field the filer controls. There are
   now exactly TWO accepted justifications — first-illumination and red-team gaps; the second
   requires listing each issue in `## Deferred` as `red-team-gap` with the spec or vector it covers.
   The prior wording called first-illumination "the only accepted justification"; retracted here.
