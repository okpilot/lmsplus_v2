...
- Detection is POSITIONAL: a hex token after a commit-context word, a possessive, a bare
  ...
  8-hex lookalikes (`3f55bc25` writes "714eec4f in plan-critic.md").
- Two FOREIGN-SHA exclusions, both found by measurement against real history, not anticipated:
  ...
  parenthetical on a line carrying an `owner/repo@ref` action pin is skipped — `48919941` pins six
  actions at once as `actions/checkout@v6 (de0fac2)`, and every one would otherwise block.
- A backtick requirement was tried and REJECTED: zero recall, because this repo writes SHAs bare
  ("added in 269667d7").
- Recall is **79%** — 276 of 350 — where the denominator is every distinct mixed-hex 7-40 token in
