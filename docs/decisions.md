...
- **Commit format:** Conventional Commits enforced via commitlint in the Lefthook commit-msg hook, alongside the commit-claims guard (a cited SHA must resolve — Decision 64).
...
- Both were missed by the #883 sweep, which `docs/security.md` had recorded as complete. Neither had a test at any tier before this change. Coverage is now split across two tiers that exercise DIFFERENT guards: `packages/db/src/__integration__/rpc-analytics-guards.integration.test.ts` authenticates as a student, so it covers the active-user gate and the `deleted_at` filter (both mutation-confirmed there); `apps/web/e2e/redteam/server-action-unauthenticated.spec.ts` calls anonymously, so it is the only tier covering the older bare `auth.uid() IS NULL` check; and `apps/web/e2e/redteam/rpc-analytics-active-user-gate.spec.ts` (Vector FN) covers the authenticated-but-deactivated caller and the Decision-24 identity guard at the red-team tier, mirroring Vector FM's treatment of the internal-exam pair.
...
**Divergence CLOSED by #990** (2026-09-02): every list surface rendering
`correctCount / totalQuestions` now renders the same item/item fraction as the report, so the
"29 / 25" in a list and "29 / 29" one click later can no longer disagree. There were THREE, not the
two this decision originally named:
`admin/dashboard/students/[id]/_components/clickable-session-row.tsx`,
`admin/internal-exams/_components/attempts-table.tsx`, and — student-facing, found by post-commit
review rather than by the audit command below — `reports/_components/session-table.tsx` and
`reports/_components/session-card.tsx` (one surface, two renderers), fed by `get_session_reports`,
which selects `qs.correct_count` and `qs.total_questions` straight off the session row.
...
The formatting rule itself is no longer duplicated per surface: `deriveStats` and every list cell call
`formatCorrectFraction(correctCount, answeredItems)` (`apps/web/lib/utils/format-correct-fraction.ts`),
so the em-dash-on-zero case and the spacing cannot drift apart again. Adopting it also changed the two ADMIN
lists' punctuation from `15/20` to the report's spaced `15 / 18` — deliberate, so every surface reads
identically; the student-facing list was already spaced. The per-session item counts come from `fetchAnsweredItemCounts`
(`apps/web/lib/queries/answered-item-counts.ts`), which pages `quiz_session_answers` through
`fetchAllRows`: a page of 25 sessions can exceed PostgREST's 1000-row cap, and a plain `.in()` select
would silently truncate and UNDER-state a denominator. Still no SQL `COUNT(DISTINCT ...)` — the count
is derived in the query helper, as above.

That helper takes its Supabase client as a REQUIRED parameter, deliberately with no default. The two
admin surfaces pass the service-role `adminClient` because they read across students the caller does
not own; `/app/reports` passes the request's RLS-scoped `createServerSupabaseClient()`, because it is
a student path and a service-role read there would bypass the very policy that scopes it
(`students_read_answers` on `quiz_session_answers`, mig `20260311000005`, already permits a student to
read their own answer rows). An implicit `adminClient` default would have made the unsafe choice the
quiet one.

Both list COLUMN HEADERS were deliberately left alone ("Questions" on the student-detail list,
"Answered" on the attempts table). #990's acceptance criteria scope the fix to the query, the type and
the cell; the student-detail header is a `SortableTableHead` whose `questions` field maps to
`total_questions`, so relabelling it would either desync the label from the sort or change a
URL-visible sort contract the issue never asked about. Consequence, recorded rather than fixed, on
...
ATTEMPTS TABLE: the gap did not close — it narrowed on one axis and widened on another, and the two
must not be conflated.

- HEADER vs CELL, narrowed: "Answered" does now name what the DENOMINATOR became, but the cell renders
  `correctCount / answeredItems`, and the NUMERATOR has been `correctCount` throughout — before this
  change as much as after. So under that header "1 / 3" still reads as "answered 1 of 3" for an attempt
  where the student answered 3 of 10. Two operands mismatched the header before, one does now.
- ADMIN vs STUDENT, widened: the student's own view of the same attempt (`/app/internal-exam` → My
  Reports, `my-reports-tab.tsx`) renders `{answeredCount}/{totalQuestions}` = "3/10" under an
  identically-named "Answered" header. The old admin form `correctCount/totalQuestions` = "1/10"
  disagreed on the numerator but AGREED on the denominator; the new "1 / 3" agrees on neither. One
  operand disagreed before, two do now.

Measured 2026-09-06 on a seeded 10-question internal exam, 3 answered, 1 correct.

## Decision 61: post-commit reviewer rounds use CR-local's extend-by-one, not a consecutive-clean counter (2026-09-06)

**Problem** (#1255). `agent-critic.md` reset the clean counter to 0 on any APPLY finding, while the
ceiling capped a run at 4 total rounds. Those two are arithmetically incompatible: a finding on
round 1 leaves exactly rounds 2-4 to produce three unbroken clean rounds, so any later finding — or
any coverage round, which consumes the ceiling without advancing the counter — makes N=3
unreachable. (N was the old variable name in `agent-critic.md` for the value this decision
renames to M; they are the same number, and the rename is part of the fix.) On PR #1248 it became unreachable in practice and forced an escalation that said
...
**Explicitly NOT decided:** no comment-only-diff exception. Option 1 in #1255 — let a security-path
file whose diff contains no changed non-comment line drop back to the normal floor — is rejected.
`agent-workflow.md` forbids deriving the floor from semantic intent, a comment-only detector fails
OPEN when it is wrong, and the issue itself warns the option reopens that hole. **The mechanical
path-glob derivation is unchanged.** This fixes the arithmetic, not the trigger — a comment-only
touch of a security path still raises the floor, and that is intended.
...
**Problem.** The always-injected corpus (`CLAUDE.md` + `.claude/rules/*.md`) is 3,317 lines that
control execution with no compiler, no tests, and the same fact restated by hand across many files —
the model-tier rule appears in 11 of the 16 corpus files (`grep -rl`, 2026-09-07). Three independent
audits found **23** contradictions; **11** are hand-verified and **1 is refuted** (below). Those three
figures are derived from a row-per-contradiction table, not counted by hand — see Decision 58 and the
rule that a count is stored as a list and computed, never written. Every one had been invisible to our review pipeline, because **every reviewer
we run is diff-scoped** — `CLAUDE.md:179-180` invoke `/project:review` and `/project:insights` with a
stale `project:` prefix that no other command reference in the corpus uses. The command FILES exist
(`.claude/commands/{review,insights}.md`); only the invocation syntax is wrong. No gate could have
caught it, because nobody changed that line.

**Decision.** Facts the pipeline keys on live in `.claude/pipeline.json` and are asserted against the
repo by `.claude/pipeline.test.mjs`. Prose explains WHY a rule exists; it does not restate a value
the spec owns. `.claude/agents/agent-tools.test.mjs` is DELETED — its access-control checks are a
...
reordered, an agent moved into `post-commit-core`, a role relabelled, an unchecked key reintroduced,
the pre-push hook's `--model` swapped to another KNOWN model, that flag broken into a shell variable
while a dead comment retains the old literal, a frontmatter closer deleted, and prose injected into a
frontmatter block. No tally is recorded here: a count in prose drifts, and this entry has already
carried two mutually inconsistent ones.

**The first cut of this test had two CRITICAL holes, both found by post-commit review.** The
model-literal check asserted that *a* known model appeared in `run-security-auditor.sh`, never that
the RIGHT one did — swapping that blocking pre-push gate from sonnet to haiku passed clean. And the
frontmatter parser scanned for the first `\n---` anywhere, so deleting an agent file's real closing
delimiter made it swallow ~190 lines of body and still pass, because the body never re-declares the
keys under test. Recorded from the session rather than from an artifact: my first attempt to fix the
parser did not work, and only the mutation revealed it — the test was green either way. Post-commit review then found a false "every agent file carries a third bare `---`" claim in the
fix's own comment (true of one file of ten) and a narrower recurrence of the model hole — a dead
comment masking a REMOVED flag. Both are closed and mutation-proven. The lesson is this decision's own:
a check that LOOKS like coverage is worse than none, because it silences the question.

**Nothing contested is encoded.** An earlier draft put two of my own proposals into the spec as
though they were established fact — a red-team exemption from the defer ratio, and the removal of
implementation-critic's timeout escape. plan-critic caught both as CRITICAL. A data file a test
certifies is the worst possible place to launder a proposal, so the spec now records only what the
corpus unambiguously already says. Fields whose value is genuinely disputed were left OUT rather than
resolved by me.
...
reworded the diagram. implementation-critic then read `git show 5b7e9c58` — the #1266 commit — and
found the line had been considered and KEPT deliberately: *"it sits inside the `otherwise — the FULL
cycle:` branch, where four genuinely launch"*, with that branch recorded as a documented exception.
`:224` is true in its scope; the auditor had read the two lines side by side without the branch
context. The edit was reverted. This is the 1 refutation counted above — treat audit output as leads
requiring verification, never as findings.


## Decision 63: the open policy contradictions settled; `.claude/**` joins the lint (2026-09-07)

Each of these needed a human call because both sides were defensible. All are the user's, and are
written into the rules here rather than proposed.

1. **The test-file size exemption is DERIVED, not enumerated.** A basename matching `*.test.*` or
   `*.spec.*` is exempt at any extension and any path. The old list named three extensions and had
   already drifted past `.mjs`, so every `.claude/**/*.test.mjs` was flagged against the 200-line
   utility cap. An extension list is an OPEN set (§10 clause 2).

2. **Red-team coverage-gap filings still COUNT toward `filed >= closed`.** `agent-red-team.md`
   mandates filing them, so obeying it could fail the PR's own budget. They are real backlog, and an
...
3. **test-writer's tests ride the round's ONE fixup commit.** Its header and DO list both said
   "separate commit", which costs a full review cycle per round. `code-style.md:713` (an author
   writing a test beside new code) and the fix-plus-test case are untouched — different actor, and
   already one commit.
...
package's `biome check .`; the repo root is in no package, so neither CI nor a local run reached
`.claude/`. Separately the lefthook `biome-check` glob omitted `.mjs`. Both fixed. The baseline was
one real warning — an O(n^2) spread on a `reduce` accumulator in `check-mirror-sync.mjs`, the helper
the Rule-Mirror Sync sweeps themselves depend on — fixed here.

Decision 5 does NOT close learner row 639 (a delta never staged before push). Different gap, still open.
...
- Detection is POSITIONAL: a hex token after a commit-context word, a possessive, a bare
  ...
  `BARE_START_RE`, not restated here. The word list is in the hook's `TRIGGER_WORDS`
  and is not restated here — it is an OPEN set and a copy goes stale (§10 cl.2). A token PRECEDING
  such a word is not a reference: that is what excludes mirror-sync content digests, which are
  8-hex lookalikes (`3f55bc25` writes "714eec4f in plan-critic.md"). That cancellation applies ONLY
  to the weakest position, a bare line start. Applied unconditionally it silently dropped the
  commonest citation shape in this history — "in <sha> of", "by <sha> to" — reporting success on a
  fabricated SHA; caught by the post-commit semantic review, not by either critic round.
- FOREIGN-SHA exclusions, each found by measurement against real history, not anticipated (named,
  not counted — the set already grew once inside this branch):
  URLs are stripped first (Dependabot embeds foreign SHAs in `compare/<sha>...<sha>` links), and a
  token whose IMMEDIATELY-PRECEDING token is an `owner/repo@ref` action pin is skipped, on the
  parenthetical path and — bounded to matches that came via the `(` marker — the bare-start path —
  `48919941` pins six actions at once as `actions/checkout@v6 (de0fac2)`, and every one would
  otherwise block. Scoped to that one token, never the whole line: a line-wide test let incidental
  `scope/pkg@ref` prose suppress a real citation, and the guard then exited 0 reporting success.
- A backtick requirement was tried and REJECTED: zero recall, because this repo writes SHAs bare
  ("added in 269667d7").
- Recall was **280 of 352** over a 400-message window measured while this branch was in flight. No
  commit is cited as the anchor: every SHA on this branch is discarded by the squash-merge, so a
  pin here would be a documentation citation that stops resolving the day it lands — the exact
  defect the guard blocks, in the one place the guard cannot see. Treat the fraction as a
  SNAPSHOT and re-derive before quoting it. The
  denominator being every distinct mixed-hex 7-40 token (URLs and trailers stripped) that
  `git rev-parse` resolves, the numerator those `extractRefs` returns. It is stated as a FRACTION
  pinned to a SHA, not a percentage, because the window MOVES: re-deriving over HEAD's last 400
  gives a different denominator, and the rounded figure drifted from 80% to 79% within this branch
  purely from its own later commits. A position-agnostic token scan gives a different denominator
  again. Re-derive before quoting; do not carry the number forward. The only 4
  tokens it would block are in two squash merges citing their own pre-squash commits, which
  resolved at commit-msg time. RE-DERIVE, do not trust the figure: import `extractRefs` from the hook,
  replay `git log -400 --format=%B`, and compare its output against every mixed-hex token that
  `git rev-parse --verify --quiet` resolves. The recall figure is stated at all because the first
  draft measured only PRECISION — learner row 649, "measurement correct for the tested scenario,
  which excludes the failure case".
- Fail-closed: `git rev-parse --verify` runs WITHOUT `--quiet`, which was verified to collapse an
  AMBIGUOUS prefix and an ABSENT one into the same exit-1-empty-stderr signature. Classified by
  exit + stderr; an unrecognised outcome aborts and never reads as resolved.

**Bound, stated because understating it would be this very defect:** the gate proves a cited commit
EXISTS, never that the claim about it is true. Row 42 therefore stays RULE CANDIDATE — its two most
...
once — the PR-level sweep on 2026-09-09 added positions the original entry missed — so it is named
...
- A multi-SHA parenthetical `(<sha>, <sha>)`, and HOW MUCH of it leaks depends on POSITION —
  re-derive per shape, do not generalise from one. The comma breaks `PAREN_AFTER_RE`, so the
  pair rule never fires; only `BARE_START_RE`'s `(` marker can still catch the FIRST token, and
  only where that `(` opens the line. Verified: `(<sha>, <sha>)` opening a line yields the first
  token; the same text mid-sentence, or behind another list marker (`- (<sha>, <sha>)`), yields
  NOTHING and the hook exits 0 on a fabricated pair. Recorded until 2026-09-09 as the flat
  "only the FIRST SHA is checked", which is true only for the line-start case and understates
  every other — carried unverified through several commits, then briefly OVERCORRECTED to
  "neither is checked", which is equally wrong. §10 cl.3: a claim re-typed unchanged is a NEW
  assertion, and cl.5: re-reading finds incoherence, only re-deriving finds a claim that is
  coherent and false.
...
`git commit --amend` re-validates prose that was TRUE when written: if a cited commit was since
rebased away and gc'd, the amend is blocked. And a SHA resolves only against the LOCAL object
store, so citing a just-merged commit before fetching it blocks a TRUE citation — hence the
remedy text names `git fetch origin` first. None is fixed here; all are recorded so the next
author is not surprised. This paragraph was headed "Two known limits" until a further limit arrived,
which is §10 cl.2 in the decision record for the guard against exactly that. Branch messages were each gated when authored; hand-edited squash prose is not.
...
still over the limit but under the old allowance — reported as nothing at all, not even a stale-entry
warning, because the path never left `liveViolators`. Requiring the number to stay exact turns silent
absorption into a visible edit.

**DELETE vs PIN — codification has two moves, not one.** A copy is deletable only when its consumer
can follow a pointer. `.coderabbit.yaml` cannot (`agent-workflow.md § Rule-Mirror Sync`): CodeRabbit
sees the diff plus the literal `path_instructions` text, so a reference there would not sync the rule,
it would delete the enforcement. Its caps are KEPT and pinned by a test asserting they match
`limits.json`. That test fired on its own author within a minute, on a cap added to the data file and
not the mirror. The rule, rather than a claim about the current contents of a set that can
gain members: DELETE a copy when its consumer can follow a pointer to the authoritative
source; PIN a literal mirror, under a test, when the consumer cannot dereference one.
...
**Coverage boundary, stated rather than left implicit.** `.mjs` matches no rule, so every NON-TEST
`.claude/hooks/*.mjs` guard — including this one — is ungraded. Their co-located `*.test.mjs` files
...
## Decision 66: §10 cl.3 becomes a gate — an incomplete retraction blocks the commit (2026-09-14)

Build-order item 1 of the corpus-codification programme. `code-style.md` §10 cl.3 has always told
the author to grep a retracted phrase repo-wide before claiming the class closed. The learner row
...
blocks 18% of commits, is almost entirely noise, and MISSES the instance it was written for. The
narrowings needed before it was shippable are NOT counted here — this entry said "four" while the
guard already had a fifth, and any corrected number goes stale the next time one is added (§10
cl.2; derive them from `CORPUS`, `ISSUE_PREFIX_RE`, `completedSpecDirs` and `candidatesFor`): a
rarity window (a token surviving in three or more
files is vocabulary, not a claim); exclusion of ticket references and migration numbers; exclusion
of `.claude/agent-memory/**` on all three sides (those files quote past false claims verbatim, so
counting them both invents survivors and exonerates real retractions); exclusion of any spec whose
tasks are all `[x]`, which `agent-workflow.md § Rule-Mirror Sync` designates a historical record;
and a correction gate at HUNK granularity — a removal fires only when the same hunk supplies a same-class replacement, which
...
author of the guard built to catch that class. It is recorded rather than quietly overwritten
because it is the clearest evidence in this entry that the rule needs a machine. Re-derive rather
than trusting the number: the replay harness is NOT committed, so this is a claim to re-measure
before relying on it.

**The spec's escape-hatch design could not work and was replaced.** It proposed an inline marker on
the removed line — text the commit is deleting, so nothing greppable survives. The waiver is a
commit-message trailer instead, `Retracted-ok: <token> — <reason>`, naming ONE token and requiring a
reason that is not a bare "false positive". That is why the guard runs at `commit-msg`: it is the
only stage holding both the message and the staged index. `/insights` now audits waiver frequency,
and the recorded revisit condition is falsifiable — above roughly one waiver per 25 commits the
detector is re-narrowed, never baselined.

**No baseline file, deliberately, unlike Decision 65.** R4 mandates a ratchet for a check adopted
onto an already-violating codebase. This one is diff-scoped by construction: a retraction is a
...
assertions for the first time refuted SEVEN of them across three files, plus one test that pinned
nothing and one mechanism nothing pinned:
...
refuted SEVEN across three files, plus one test that pinned nothing and one mechanism nothing
pinned — including inside §7's own promotion sweep, which had asserted it verified them all by
