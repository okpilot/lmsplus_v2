...
- `get_subject_scores` additionally gains `AND qs.deleted_at IS NULL` on its `quiz_sessions` aggregate (rule 9). SECURITY DEFINER is owned by `postgres` (`BYPASSRLS`), so RLS never ran for it and a discarded session still moved the student's average. This CHANGES returned averages. `get_subject_scores` is called by exactly one production helper (`getSubjectScores` in `apps/web/lib/queries/analytics.ts`), and nothing imports that helper — so no page or component reaches the RPC today and the reachable surface is a direct PostgREST call.
