...
3. **For each finding, classify it (read the source, do not trust labels):**
...
4. **Apply each "Apply" finding directly.** Don't batch — fix and move on. If a fix changes more than 10 lines or touches a 4th file, stop and present the plan first.

5. **For each "Skip" finding, briefly note the reason** in the round summary you give the user.

6. **Re-run the review** after your fix commit lands.

7. **Stop the loop** when ANY of these is true:
