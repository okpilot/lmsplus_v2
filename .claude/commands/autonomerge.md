...
All the same rules bind: never push to `master`, never `--no-verify`, never amend after a hook failure, implementation-critic skips only an agent-memory-only commit, no in-flight findings at push, default APPLY / DEFER-with-issue / SKIP-with-reason.
