...
5b. **Fetch and capture the changed-path list ONCE, fail-closed.** Before the conditionals below, run:
    ...
    ```
    Steps 6, 7 and 7b then match against `$CHANGED` — do NOT re-run the diff per step, and NEVER let an errored or empty diff be read as "no paths matched". 7b (Red Team) is MANDATORY: on a stale, unresolvable, or errored base an unguarded conditional silently evaluates false and the required gate is skipped (see `agent-workflow.md` § "Always diff against `origin/master`, never the bare local `master`").
