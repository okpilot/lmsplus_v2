...
7. **E2E (conditional)**: if `git diff master...HEAD --name-only` includes `apps/web/e2e/`, run the web Playwright suite (`pnpm --filter @repo/web e2e`). Skip when no e2e specs changed — the full suite is slow and CI runs it on push anyway.
