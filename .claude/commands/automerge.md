...
17. **`/crlocal`** — run CodeRabbit local; triage each finding per `.claude/rules/agent-coderabbit-local.md` (read source, classify, APPLY/SKIP/DEFER, re-run after fix commits, stop on the loop's stop conditions). The stop rule (2026-06-23) is **minimum-rounds-met + last-round-clean**: run ≥ M rounds (M=2 normal / 3 security-path), stop on the first round at/after M with no apply-worthy findings; an APPLY finding extends the loop by one round rather than resetting a counter; ceiling 4 fix commits → escalate. Cloud CR on the pushed PR stays the authoritative gate. **⛔ Rate limit = STOP and WAIT.** If CR-local prints `Review limit reached` (or any usage/rate-limit message), do NOT push or merge on a partial CR-local pass — the minimum-rounds rule must be met with real review rounds, never skipped because the quota was hit. Wait for the reset (CR prints the reset time; use `ScheduleWakeup`/sleep for that duration), then resume the rounds. A rate limit is a pause, not a pass.
...
> If the PR diff touches `supabase/migrations/**` (or its `packages/db/migrations/**` mirror), do NOT merge under this command — merging to master auto-deploys the migration to the **PRODUCTION database** via `db-deploy.yml`. Prepare the PR, verify CI is green, and leave it **OPEN** with an explicit handoff note: the user manually evals and merges. **No LLM judgment call overrides this — not "trivial migration", not "idempotent", not "CI proved it".** Mechanical backstop: `db-deploy.yml` deploys through the GitHub `production` environment, which requires the user's approval before `db push` runs — but the backstop is a safety net, never a substitute for this hard stop.
...
- **CodeRabbit (cloud) must have actually REVIEWED the current HEAD — not be rate-limited.** ⛔ If cloud CR has not posted a review on HEAD because it hit a usage/rate limit (no review for the current commit, or a "limit reached" comment/state), do NOT merge — **STOP and WAIT** for the limit to reset (`ScheduleWakeup`), then let CR re-review or re-trigger it (`@coderabbitai review`). An absent or partial CR review is not an approval; merging past it defeats the gate. The same applies to the pre-push `/crlocal` minimum-rounds rule (step 17): a rate limit mid-loop means wait, not proceed.
