...
- **CodeRabbit (cloud) must have actually REVIEWED the current HEAD — not be rate-limited.** ⛔ If cloud CR has not posted a review on HEAD because it hit a usage/rate limit (no review for the current commit, or a "limit reached" comment/state), do NOT merge — **STOP and WAIT** for the limit to reset (`ScheduleWakeup`), then let CR re-review or re-trigger it (`@coderabbitai review`). An absent or partial CR review is not an approval; merging past it defeats the gate. The same applies to the pre-push `/crlocal` floor (step 17): a rate limit mid-floor means wait, not proceed.
