...
5. **Long functions** — any function > 30 lines (EXCEPT React function-component / custom-hook render/return bodies of pure JSX composition, allowed up to 35 lines — see suppression #5 below and code-style.md §3)
   ...
   A function already over the cap at the merge-base that the diff makes longer counts: measure both (`MB=$(git merge-base origin/master HEAD) || abort`, then `git show "$MB":<path>`; for a renamed or moved file, the old path from `git diff -M --name-status origin/master...HEAD`).
