...
Read the findings from ALL agents that ran this cycle — the core post-commit agents (code-reviewer, semantic-reviewer, doc-updater, test-writer), plus red-team and coderabbit-sync when those conditional agents ran — identify patterns, and update project rules/memory so the same mistakes stop happening.
