...
Read the findings from ALL agents that ran this cycle — the core post-commit agents (code-reviewer, semantic-reviewer, doc-updater, test-writer), — red-team and coderabbit-sync run AFTER you, so their findings are NOT your input; they reach you on the branch's next full cycle — identify patterns, and update project rules/memory so the same mistakes stop happening.
