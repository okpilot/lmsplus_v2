...
16. **Missing soft-delete filter in a SECURITY DEFINER SELECT** (docs/security.md §15 "Soft-delete in RPCs")
    ...
    - Fires ONLY on genuinely soft-deletable tables — consult the `docs/database.md` §3 soft-delete matrix. Never flag a table that has no `deleted_at` column — derive that set from `packages/db/src/types.ts`, since a later migration can add or drop the column and a stale list here SUPPRESSES a real finding. As of 2026-08-19 it was: `easa_subjects`, `easa_topics`, `easa_subtopics`, `quiz_session_answers`, `student_responses`, `audit_events`, `quiz_drafts`, `exam_config_distributions`, `fsrs_cards`, `user_consents` (derived from `packages/db/src/types.ts` — update when a migration adds/drops `deleted_at`). Likewise, hard-delete-exception tables that RETAIN a `deleted_at` column (per `docs/database.md` §3: `question_comments` — the column is an unused safety net; hard DELETE via own/admin RLS policies is the design, see check 6) do NOT require the filter — do not raise HIGH for a missing `deleted_at` filter on `question_comments`.
