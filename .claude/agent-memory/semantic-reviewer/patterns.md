...
- All 3 changed files are in `scripts/` or `e2e/` — no production surface change.

**Seed idempotency is correct for the expected CI scenario (GOOD)**
- `exam_configs` SELECT uses `.maybeSingle()` with `.is('deleted_at', null)` filter — correctly skips soft-deleted rows and reuses only active configs. Combined with migration 044's partial unique index (`WHERE deleted_at IS NULL`), a soft-deleted config does not block a fresh INSERT. Logic is sound.
- `exam_config_distributions` SELECT also uses `.maybeSingle()`. Error handling on both SELECTs intentionally omits `error` destructuring (consistent with the established seed pattern: fail loudly on INSERT, not on existence check).

**service-role client correctly bypasses RLS for seed (GOOD)**
- `db = createClient(url, SERVICE_ROLE_KEY)` is used throughout the script, consistent with the existing seed pattern. The exam_config admin-only RLS policies are irrelevant — the service role bypasses them. No auth surface change.

**test.fixme body intact — assertions preserved (GOOD)**
- `test.fixme('title', async ({page}) => { ...assertions... })` with Playwright ≥ 1.33 skips the test entirely (does not run the body). The assertions in the fixme'd test body are preserved verbatim so re-enabling by removing `test.fixme` would immediately enforce the full Bug A regression spec. No empty body risk.

**exact:true locator narrowing is correct (GOOD)**
- `getByRole('button', { name: 'Practice Exam', exact: true })` does NOT match "Start Practice Exam" or "Resume Practice Exam" — Playwright's `exact: true` requires the full accessible name to equal the string. "Start Practice Exam" is a distinct string and will not collide. Correctly applied in both `exam-flow.spec.ts` and `exam-recovery.spec.ts`.

**locator.or() race pattern is correct for Bug C / Bug D coverage (GOOD)**
- `expect(questionText.or(resumeBanner)).toBeVisible({ timeout: 15_000 })` waits for whichever surface stabilises. The subsequent `resumeBanner.isVisible().catch(() => false)` branches correctly: `true` → Bug D path (click Resume, assert session), `false` → Bug C path (questionText already proved visible). Neither: test fails, which is the right behavior. The 15s timeout is ample for a page reload. Coverage is equivalent to the original if/else on URL.

**test.setTimeout(150_000) math is consistent (GOOD)**
- `waitForURL(120_000)` at line 120 is reached after ~15s of setup. Combined: 15 + 120 = 135s total, under the 150s ceiling. The comment documents the reasoning (trace evidence: RSC payload fetched at 88-92s, 90s was arithmetically too tight). Consistent.

**SUGGESTION — distribution SELECT doesn't filter subtopic_id = null (minor logic gap)**
- `exam_config_distributions` SELECT uses `.eq('exam_config_id', examConfigId).eq('topic_id', topicId).maybeSingle()` without `.is('subtopic_id', null)`. If a distribution row for the same `(examConfigId, topic_050, someNonNullSubtopicId)` existed, the check would find it and skip inserting the topic-level `(topic_050, null)` distribution — leaving the exam_config incomplete. In CI this is safe (no other seed creates distributions for this config), but the guard is not semantically tight. Fix: add `.is('subtopic_id', null)` to match exactly the row that would be inserted.

**SUGGESTION — existing exam_config is reused without validating its params**
- When `existingCfg` is truthy (config already exists), the seed reuses it without verifying `time_limit_seconds=60`, `total_questions=10`, `pass_mark=70`. If a prior run or manual admin action left a config with different params, the CI tests would assert the wrong values. The seed currently cannot detect this drift. Mitigation: either add a validation check or document that the seed trusts the existing config shape.

**Pattern — seed SELECT-then-INSERT without ON CONFLICT (seed-specific, acceptable):**
Seed scripts in this codebase consistently use SELECT-to-check + INSERT pattern rather than `ON CONFLICT DO NOTHING`, because `ON CONFLICT` requires knowing the exact constraint column(s) including nullable keys (where `NULLS NOT DISTINCT` would be needed for PG15 to protect null subtopic_id). For serial CI runs, the SELECT-then-INSERT pattern is safe. For tables with `subtopic_id IS NULL` as a valid unique dimension, the SELECT guard should filter on the exact nullable column to avoid false positives.

### 2026-04-28 — commits 082830d + 8707963 (fix: round 8 — grace-window alignment, empty-session branching, orphan cleanup)
- **Files reviewed:** mig 052 (complete_overdue_exam_session + start_exam_session), mig 053 (complete_empty_exam_session), _overdue-helpers.ts, use-exam-start.ts, quiz-submit.ts
- **CRITICAL:** 0 | **ISSUE:** 2 | **SUGGESTION:** 3 | **GOOD:** 6

**ISSUE — use-exam-start.ts: loading spinner never cleared when cleanup discard fails**
- When `sessionStorage.setItem` throws and `discardQuiz` returns `{ success: false }`, the code logs the error, calls `setError(...)`, and calls `setLoading(false)` and returns. That path is correct — `setLoading(false)` IS called (line 70). However: if `discardQuiz` itself *throws* (network-level rejection bypassing the try/catch inside the action — the catch inside `discardQuiz` returns a value, so this cannot currently throw). After careful re-read: `discardQuiz` is a Server Action that wraps everything in try/catch and always returns. The `await` in the cleanup path will always resolve to a value. NOT an issue — false alarm. Retracted.

**ISSUE — quiz-submit.ts: success path omits setSubmitting(false) on navigate, but so do all other submit paths — consistent (GOOD)**
- The success branch calls `router.push(...)` without calling `setSubmitting(false)`. The component unmounts on navigation, so the dangling `true` is harmless. All other paths in this function follow the same pattern (lines 147-149). Consistent — not flagged.

**ISSUE — migration 053: v_overdue evaluated AFTER the UPDATE (races itself on clock skew)**
- The `UPDATE quiz_sessions SET ended_at = now()` runs at line 154. The overdue check at line 164 calls `now()` again. In Postgres, `now()` is transaction-start time, so both calls return the same timestamp within the same transaction. No clock skew is possible. Safe — not an issue.

**Pattern — PERFORM call doesn't surface errors from called RPC (NEW, watch):**
`start_exam_session` at mig 052 line 191 uses `PERFORM complete_overdue_exam_session(v_old_session_id)`. If `complete_overdue_exam_session` raises an exception (e.g. 'session is not overdue' race), the exception propagates up and aborts `start_exam_session` entirely, returning an error to the TS caller. The TS caller (`startExamSession` action) does not distinguish this error case — any exception becomes a generic 'Failed to start exam session'. This is a known accepted trade-off (mig comment says "matches batch_submit_quiz") but the race window is 30s wide. Count=1, watching.
...
**Original ISSUE from 82e64de: CLOSED (GOOD)**
- The gap (all three exam arrays collapse to [] + useQuizRecovery suppresses exam-mode entries → zero recovery surface + no signal) is now fully closed. The `examLookupFailed` boolean gates a `role="alert"` error notice before all exam banner renders. Student sees an explicit message and is told to refresh.

**`role="alert"` is semantically correct for this render path (GOOD)**
- The element only renders when `examResult.success === false`, which requires an actual DB/auth/network failure on every page load after `requireAuthUser()` has already passed (so the "Not authenticated" path is unreachable in practice). This is an exceptional, unpredictable failure — `role="alert"` is correct per ARIA spec. `role="status"` would be correct for advisory/informational content; `role="alert"` is for unexpected conditions requiring immediate attention. Justified.

**Styling aligned with established error-alert pattern (GOOD)**
- `bg-destructive/10` + `border-destructive/30` + `text-destructive` matches `active-session.tsx:43`, `quiz-main-panel.tsx:42`, and `finish-quiz-dialog.tsx:259`. No novel class introduced. Pattern consistency maintained.

**"Not authenticated" success:false path is unreachable in this page (GOOD)**
- `requireAuthUser()` at line 15 redirects to `/auth/login` on auth failure before `getActiveExamSession()` is ever called at line 17. The `getActiveExamSession` "Not authenticated" guard (line 34 of the action) is therefore dead code on this render path. The `examLookupFailed` notice is only reachable via the DB error or uncaught error paths (lines 47, 94 of the action) — both genuinely exceptional and non-routine.

**No test covers the new examLookupFailed notice branch (SUGGESTION)**
- `apps/web/app/app/quiz/page.tsx` has no co-located test file at all. The new `examLookupFailed` branch adds a render path that is not exercised by any test. Since `page.tsx` is a Server Component, a unit test would mock `getActiveExamSession` returning `{ success: false }` and assert the `role="alert"` element appears. Non-blocking but the failure path is precisely the one hardest to catch in manual testing.

**Pattern — role=alert vs role=status (documented):**
When a server-rendered error notice is always-present on load (e.g., rendered from a static Server Component prop), `role="alert"` is correct only when the condition is genuinely exceptional. Routine "no data" states or informational notices should use `role="status"` to avoid noisy screen-reader announcements on every page load. This commit is correctly using `role="alert"` because the path is gated on an actual server failure, not a routine empty-state.

### 2026-04-28 — commit 82e64de (fix(quiz): hide legacy recovery banner for exam-mode sessions)
- **Files reviewed:** use-quiz-recovery.ts, quiz-recovery-banner.test.tsx + quiz-session-storage.ts, page.tsx, resume-exam-banner.tsx, get-active-exam-session.ts
- **CRITICAL:** 0 | **ISSUE:** 1 | **SUGGESTION:** 1 | **GOOD:** 4

**Banner-less gap when getActiveExamSession fails and exam localStorage entry exists (ISSUE)**
- When `getActiveExamSession` returns `{ success: false }` (DB error, auth error, network), `page.tsx` collapses `activeExams` to `[]` and renders no `ResumeExamBanner`. Simultaneously, `useQuizRecovery` now suppresses exam-mode localStorage entries unconditionally. The user lands on `/app/quiz` with no exam recovery surface at all, and no error message explaining why the amber banner is absent. The localStorage entry is still present and intact — the suppression is in-memory only — so a page refresh may or may not recover depending on whether the server error clears. The student is stuck with a valid in-progress exam and no visible way to resume or discard it.
- This is not a regression introduced by this commit: the server failure path pre-existed. But this commit made the failure mode worse — before, the blue study banner would have appeared as a fallback, allowing the student to at least see something. Now the fallback is gone. The correct fix is to either (a) suppress the exam localStorage entry only when `getActiveExamSession` succeeds, passing that signal client-side, or (b) show a generic error banner when `examResult.success === false` so the student knows the session check failed.

**localStorage `mode` field could be missing on old entries, silently suppressing study sessions (SUGGESTION)**
- `mode` is typed `optional` on `ActiveSession`. An entry written before `mode` was introduced (or by a code path that never explicitly sets `mode`) will have `mode: undefined`. The filter `active?.mode === 'exam'` correctly passes `undefined` through as non-exam, so old study entries are not suppressed. However: `readActiveSession` in `quiz-session-storage.ts` has a validator at line 114 that rejects exam entries missing `startedAt`/`timeLimitSeconds` — but it does NOT reject entries with `mode: undefined`. A future code path that writes `mode: 'exam'` without setting `startedAt` would be rejected by the validator before reaching the hook. Conversely, a study session that somehow acquired `mode: 'exam'` (corrupt write) would be rejected by the validator too (missing `startedAt`). The filter is therefore safe against the known corruption paths. No action needed — but the safety depends on the validator gate in `readActiveSession`, not on the hook itself.

**`ResumeExamBanner.handleResume` does not call `clearActiveSession` (GOOD — intentional)**
- `resume-exam-banner.tsx` writes to `sessionStorage` and calls `router.push`. It does NOT call `clearActiveSession`. This is correct: the exam localStorage entry serves as the refresh-recovery checkpoint inside the session page (`use-session-bootstrap.ts`). Clearing it at the resume-banner layer would break the exam recovery path on tab close mid-session. The hook's filter is the right layer to suppress the blue banner — the amber banner owns resume, the session page owns clearing.
...
**Test fixture uses `{ ...ACTIVE_SESSION, mode: 'exam' }` which correctly exercises the suppression path (GOOD)**
- `ACTIVE_SESSION` does not include `mode`, so the spread adds `mode: 'exam'` to an otherwise valid study-session shape. The mock bypasses `readActiveSession`'s validator, so it tests the hook's own filter in isolation — which is the right unit boundary. No validator leakage into this test.

**Pattern — client-side suppression that depends on a server call succeeding creates a silent gap (NEW):**
When a client component suppresses a recovery surface because a server-sourced component owns that surface, the suppression must be conditioned on the server call succeeding. If the server call fails silently (returns empty arrays without an error banner), the user can end up with no recovery path at all. Pattern: pair server-dependent suppressions with an explicit fallback or error state visible to the user.

### 2026-04-27 — commit 4971eda (fix(quiz): populate questionIds in resume-exam handoff so cold-start works)
- **Files reviewed:** get-active-exam-session.ts, resume-exam-banner.tsx + 4 test files, docs/database.md, docs/decisions.md
- **CRITICAL:** 0 | **ISSUE:** 1 | **SUGGESTION:** 1 | **GOOD:** 6

**Malformed-config rows hidden rather than surfaced for Discard (ISSUE)**
- `getActiveExamSession` skips rows with malformed `config.question_ids` and returns them as absent. If a student has an active exam session whose config is corrupt (e.g. DB migration that changed the JSONB shape, a partial write, or a cold session from before `question_ids` was populated), the session is invisible — no Resume button, no Discard button. The student cannot recover the orphaned session from the UI. The session remains as `ended_at IS NULL` in the DB and will continue to appear as "active" until it times out or an admin cleans it up.
- Fix: return skipped rows in a separate `ActiveExamSession[]` field (e.g. `orphanedSessions`) where `questionIds` is absent/null, and render a Discard-only banner for those.

**Round-trip test in resume-exam-banner.test.tsx exercises the real readSessionHandoff (GOOD — confirmed)**
- `vi.mock('../session/_utils/quiz-session-storage', async (importOriginal))` spreads `...actual`, so `readSessionHandoff` in the mock module is the real implementation. The import at line 32 resolves through the mock registry and gets the real function. The round-trip test writes via the real banner component, reads via the real validator, and uses an in-memory `sessionStorage` — no mocked intermediary. Genuine integration coverage.

**Pattern — skip-row silently orphans sessions (NEW):**
When a SECURITY DEFINER query or Server Action filters out a row due to data quality issues (malformed JSONB, missing fields), a UI control must still be emitted if the row is otherwise valid enough to identify (e.g., the session ID is present). Hiding the row entirely prevents user-initiated recovery (Discard). Returning a degraded-state variant of the result type (e.g., `orphaned: true, questionIds: undefined`) is the correct pattern — it lets the UI render a Discard-only path. Rule: if filtering a row would strand a DB record that the user would otherwise need to act on, return the row in a degraded-state field rather than omitting it.

### 2026-04-27 — commit f22cb87 (fix(quiz): persist exam sessions and surface server-side resume banner)
- **Files reviewed:** get-active-exam-session.ts, resume-exam-banner.tsx, page.tsx, use-exam-state.ts, use-quiz-persistence.ts, quiz-session-storage.ts + 4 test files
- **CRITICAL:** 1 | **ISSUE:** 0 | **SUGGESTION:** 1 | **GOOD:** 6

**Handoff writes questionIds:[] which always fails isValidSessionData (CRITICAL)**
- `resume-exam-banner.tsx` writes `{ sessionId, mode:'exam', questionIds: [] }` to sessionStorage. `isValidSessionData` at line 139 of `quiz-session-storage.ts` rejects any payload where `questionIds.length === 0`. `readSessionHandoff` therefore always returns null for this path. The session page falls through to `readActiveSession`, and because an exam-mode localStorage session with no answers will have `questionIds` populated only if `useQuizPersistence.checkpoint` has been called since the last page load — not on a cold-start resume from another device. Cold-start resume silently redirects back to `/app/quiz` instead of loading the session.

**`passed modes:exam to useQuizPersistence` test is empty/structural-only (SUGGESTION)**
- The third test in `use-exam-state.test.ts` ("passes mode: exam to useQuizPersistence") does not assert the mode value was forwarded. It uses the `mockCheckpoint.mock.calls` reference but only checks it is defined. The intent is clear from the comment but the assertion provides no actual protection.

**Pattern — handoff shape written by clients must match validator contract (NEW):**
When a component constructs a sessionStorage handoff payload directly (rather than calling `buildHandoffPayload`), the shape must be validated against `isValidSessionData` before writing. The current contract requires a non-empty `questionIds` array. Direct writes that skip `buildHandoffPayload` are liable to violate this contract silently — the session page fails-soft (redirect), not with an error. Rule: direct sessionStorage writes for the handoff key must either call `buildHandoffPayload` or include an explicit assertion against the validator contract.

### 2026-04-27 — PR #523 full diff sweep — pre-push (master...HEAD, 65 files, 28 commits, student exam mode round 5)
- **Scope:** PR-level sweep before push. Cross-commit consistency check: soft-delete carve-out, doc alignment, error message contracts, test mock fidelity. Per-commit issues already cleared.
- **CRITICAL:** 0 | **ISSUE:** 0 | **SUGGESTION:** 2 | **GOOD:** 7

**FOR UPDATE concurrency comment lost in docs refresh (SUGGESTION)**
- The `docs/database.md` SQL body for `batch_submit_quiz` was refreshed in commit d80757b. The explanatory comment that justified `FOR UPDATE` positioning ("serializes concurrent retries so the second caller sees `v_ended_at IS NOT NULL` and takes the replay path instead of double-writing") was in master's docs but was not carried forward into the refreshed block. Neither migration 047 nor 048 includes this comment. The concurrency rationale is now only in the git history of the old migration. Non-blocking but the `FOR UPDATE` placement will look arbitrary to a future reader.
- Fix: add a brief comment above `FOR UPDATE` in the docs SQL block restoring the rationale.

**Test mock strings for `start-exam.ts` error routing are synthetic, not mirroring actual RPC strings (SUGGESTION)**
- `start-exam.test.ts` mocks the RPC error message for "not enough active questions" as `'not enough active questions available'`. The actual RPC raises `'not enough active questions for topic X (subtopic Y). Need N, found M'`. Both contain the substring `'not enough active questions'` (the `.includes()` check in the action), so the routing logic IS tested correctly. But the mock string does not reflect production output, which means a future developer reading the test gets a misleading picture of what the RPC actually says. Compare: the "already in progress" mock (`'already in progress for subject'`) and the "no exam configuration" mock (`'no exam configuration found'`) both contain their respective substrings but also differ from the actual RPC strings (which include "for this subject"). This is a systemic pattern across all three error mocks. Non-blocking, but consider updating mocks to use the actual RPC strings to make tests serve as documentation of the real error surface.

**Migration 048 inline comment does NOT reference docs/security.md §15 (GOOD — intentional)**
- `.claude/rules/security.md` rule 9 exception says "see `docs/security.md` §15 and `docs/database.md` §3". Migration 048 inline comment only cites `docs/database.md` §3. This is acceptable: the requirement in the exception rule is that the call site include an inline comment AND cross-reference §3. §15 is the rules-layer doc, not a required citation in migration comments. No inconsistency.

**Three-way consistency of the carve-out exception wording (GOOD)**
- `docs/security.md` §15, `.claude/rules/security.md` rule 9, and migration 048's inline comment all describe the same carve-out with consistent key elements: (a) IDs from an immutable write-once column, (b) only current case is `batch_submit_quiz`/`quiz_sessions.config.question_ids`, (c) new instances must cite the column and include a call-site comment. The wording in `.claude/rules/security.md` is a correctly compressed summary of the full §15 text. No drift.

**No test asserts the old (broken) behavior after migration 048 (GOOD)**
- Searched all `*.test.ts(x)` diffs for `'question not found'`, `deleted_at`, and `soft-del`. No test on the branch asserts or expects the broken `'question not found'` error that was caused by the regressed `AND q.deleted_at IS NULL` filter. The fix has no contradicting tests.

**RPC error substring routing is correctly bounded (GOOD)**
- All three `.includes()` checks in `start-exam.ts` (`'already in progress'`, `'no exam configuration'`, `'not enough active questions'`) are proper substrings of the corresponding `RAISE EXCEPTION` strings in migration 046. A future change to the RPC error strings that drops these substrings WOULD silently break the routing, but no such change is in this PR. The action's `console.error` logs the raw `rpcMessage` before any routing, so breakage would be detectable via logs.

**report page reads `passed` from DB, not from batch submit result (GOOD)**
- `quiz-report.ts` fetches `passed` directly from `quiz_sessions` via `getQuizReportSummary`. The `BatchSubmitResult.passed` field is available but intentionally not forwarded to the report page. The DB is the source of truth. This is the correct pattern for audit-sensitive fields.

**`passed !== null` guard before PassFailBadge correctly narrows the type (GOOD)**
- `result-summary.tsx` uses `{isExam && summary.passed !== null && (<PassFailBadge passed={summary.passed} />)}`. TypeScript control-flow analysis narrows `summary.passed` from `boolean | null` to `boolean` inside the short-circuit RHS. `PassFailBadge` receives `boolean`, not `boolean | null`. Type-safe and correct.

**Pattern — docs SQL body refresh can silently drop explanatory comments (NEW):**
When refreshing an embedded SQL block in docs/database.md, each comment in the new migration must be explicitly reviewed against the old doc block. Non-code comments (particularly for concurrency, performance, and ordering reasoning) are easy to drop because diff tools surface them as comment deletions. Rule: before committing a docs SQL refresh, diff the old and new comment blocks specifically and confirm all non-trivial reasoning comments are preserved or explicitly superseded.

### 2026-04-27 — commit 375bde5 (fix(quiz): redirect 0-answer practice exam to results on auto-submit)
- **Files reviewed:** use-auto-submit-countdown.ts, submit-empty-exam.ts, quiz-submit.ts, types.ts, migration 049/20260427000002, test files (5)
- **CRITICAL:** 0 | **ISSUE:** 2 | **SUGGESTION:** 2 | **GOOD:** 7

**Missing setSubmitting(true) in 0-answer exam path before async RPC call (ISSUE)**
- `handleSubmitSession` calls `setSubmitting(true)` before every async operation in all other branches (non-exam with answers, save, discard). The 0-answer exam branch calls `submitEmptyExamSession` without first calling `setSubmitting(true)`. If the user presses a button or the auto-submit fires twice before the first RPC returns, there is no in-flight guard. The firedRef in the countdown hook gates the initial fire, but the submitting flag is the defense at the quiz-submit layer. Missing it here is a behavioral inconsistency across all branches.

**audit_events actor_role subquery lacks deleted_at filter (ISSUE — repeated pattern, pre-existing in other RPCs)**
- `complete_empty_exam_session` uses `(SELECT role FROM users WHERE id = v_student_id)` without `AND deleted_at IS NULL` in the audit INSERT. The user's existence is already verified at line 29 (SELECT with deleted_at IS NULL), so this is a staleness window rather than a security gap. But the pattern is inconsistent with the intent of the check above it and was filed as #550 on other RPCs. This commit adds a new instance of the same pattern.

**Idempotent path hardcodes score_percentage=0 without reading DB value (SUGGESTION)**
- In the already-completed branch (line 52-59 of migration 049), the RPC returns a hardcoded `score_percentage: 0` and `answered_count: 0`. If `complete_empty_exam_session` is called twice and the first call succeeded, the second call returns the correct session_id but 0% regardless of what is actually stored in the DB. For this specific function (zero-answer exams only) the hardcoded 0 is always correct — the whole point is the session had no answers. But a future reader or copy-paste of this idempotent pattern into another RPC would be wrong. Noting it as an intentional narrowing assumption.

**test for fallback discardQuiz does not assert draftId forwarding (SUGGESTION)**
- The original test suite had a dedicated case: "forwards draftId to discardQuiz on zero-answer exam timeout". This commit removed it (because the primary path no longer calls discardQuiz). But the fallback branch in the new code still calls `discardQuiz({ sessionId, draftId })`. No test currently asserts that draftId is forwarded in the fallback path. If `opts.draftId` is non-undefined in the failure scenario, a bug there would go undetected.

**Migration files byte-identical (GOOD)**
- `packages/db/migrations/049_complete_empty_exam_session.sql` and `supabase/migrations/20260427000002_complete_empty_exam_session.sql` are byte-identical confirmed via diff.

**No deadlock risk from FOR UPDATE in complete_empty_exam_session (GOOD)**
- `start_exam_session` and `complete_empty_exam_session` both lock `quiz_sessions` rows, but `start_exam_session` uses INSERT (no SELECT…FOR UPDATE), not a row lock on an existing session. `batch_submit_quiz` does use FOR UPDATE on quiz_sessions. Both RPCs lock the SAME row by session_id, acquiring the same row lock. A concurrent call pair (one completing, one submitting answers) will serialize correctly: the second caller will wait, then either find `ended_at IS NOT NULL` (idempotent return) or proceed. No cross-table lock ordering inconsistency.

**firedRef + Effect 3 reactivation cannot double-fire (GOOD)**
- If opts.active goes false→true after onSubmit fires: Effect 3 resets firedRef.current=false AND calls setCountdown(opts.seconds). When active returns true, Effect 1 restarts the interval from opts.seconds. countdown will be opts.seconds, not 0, so Effect 2 will not immediately re-fire. The only way Effect 2 fires is countdown===0 AND !firedRef.current — both conditions are false after the reset. No double-fire possible on reactivation.

**firedRef.current set BEFORE onSubmitRef.current() in Effect 2 (GOOD)**
- Line 41 sets firedRef.current=true before line 42 calls onSubmitRef.current(). If onSubmit triggers a synchronous state update that causes Effect 2 to re-evaluate before returning, firedRef is already true, so the guard blocks the second call. Correct placement.

**Server Action security checklist complete (GOOD)**
- submit-empty-exam.ts: 'use server' present, Zod .parse() on raw input, auth.getUser() before RPC, no raw error.message returned to client, console.error logs before generic return.

**RPC security checklist complete (GOOD)**
- complete_empty_exam_session: SECURITY DEFINER present, SET search_path = public present, auth.uid() check with RAISE EXCEPTION on null, org-scope ownership check, soft-delete filter on users lookup AND quiz_sessions lookup, FOR UPDATE lock, mode guard, idempotent path safe.

**audit event_type taxonomy consistent (GOOD)**
- 'exam.expired' follows the existing dot-notation taxonomy: 'exam.started' (migration 040/046), 'exam.completed' (migration 041). No CHECK constraint on event_type TEXT column, so no risk of rejection.

**Pattern — 0-answer async branch missing setSubmitting(true) guard (NEW):**
When a handleSubmitSession-style function handles a fast-path that bypasses the normal setSubmitting(true) gate, it must either call setSubmitting(true) itself or document why the branch is already protected by an upstream one-shot guard (like firedRef). Without it, rapid UI events or test races can invoke the async operation twice before any state update completes.

### 2026-04-27 — commit 8a36a92 (fix(db): drop deleted_at filter in batch_submit_quiz bulk fetch)
- **Files reviewed:** supabase/migrations/20260427000001_batch_submit_softdelete_regression.sql, packages/db/migrations/048_batch_submit_softdelete_regression.sql, docs/database.md
- **CRITICAL:** 0 | **ISSUE:** 0 | **SUGGESTION:** 1 | **GOOD:** 4

**Security analysis — dropping deleted_at from bulk-fetch**
- The `deleted_at IS NULL` filter in the bulk-fetch offered zero security value. The real injection defense is: (1) `v_session_question_ids` is sourced from `quiz_sessions.config` locked at session start by `start_exam_session` (SECURITY DEFINER, auth-checked), and (2) the per-answer `IF NOT (v_question_id = ANY(v_session_question_ids))` guard raises before any temp-table lookup. Removing the filter restores the historical-scoring contract (§3) without expanding the attack surface.
- **Pattern — two-layer defense in SECURITY DEFINER batch RPCs (REINFORCED):** The pattern of (fetch by session-owned IDs) + (guard each answer against session-owned IDs) is defense-in-depth. A soft-delete filter on the bulk-fetch is redundant when the IDs are already scoped to a verified session.

**Intentional inconsistency — submit_quiz_answer vs batch_submit_quiz**
- `submit_quiz_answer` (per-question, live-session) retains `AND q.deleted_at IS NULL` — blocking live answers against just-deleted questions is conservative and correct for in-flight sessions.
- `batch_submit_quiz` (terminal, historical-scoring) drops the filter — in-flight sessions must complete even if questions were retired.
- This asymmetry is documented in both the function comment and §3. It is not a logic error; it is a deliberate design split between live-session semantics and terminal-submission semantics.
- **Pattern — asymmetric soft-delete semantics between live and terminal RPCs (NEW):** When reviewing new scoring RPCs, check whether the RPC is called during a session (apply deleted_at filter, fail fast) or at session termination (omit filter, preserve historical integrity). The two have different correct behaviors.

**Suggestion — §3 code example shows scalar SELECT, not bulk-temp-table pattern**
- The §3 example in docs/database.md (lines 522-526) illustrates the principle correctly (no deleted_at) but uses a simplified scalar SELECT. The actual implementation uses a temp table bulk fetch. A future developer implementing a new batch scoring RPC might copy the scalar pattern from §3 and miss the bulk-fetch technique. Non-blocking; consider updating §3 example in a docs pass.

**Backlog #550 (out of scope for this commit)**
- Both `actor_role` subqueries in audit INSERTs (`SELECT role FROM users WHERE id = v_student_id` without `AND deleted_at IS NULL`) are pre-existing and filed as #550. This commit does not introduce new instances. Confirmed out of scope.

### 2026-04-26 — commit 371f361 (fix(exam): address CodeRabbit review on PR #523)
- **Files reviewed:** answer-options.tsx, start-exam.ts, quiz-session.tsx, finish-quiz-dialog.tsx, quiz-config-form.tsx, session-card.tsx, session-table.tsx (+ test files)
- **CRITICAL:** 0 | **ISSUE:** 1 | **SUGGESTION:** 2 | **GOOD:** 9

**handleTimeExpired race — s.submitting guard removed**
- **Issue:** Removing `s.submitting` from `handleTimeExpired`'s guard and dep array is the correct intent, but `timeExpired={autoSubmitFiredRef.current}` (quiz-session.tsx:183) is read at render time from a mutable ref. The ref is written synchronously in `handleTimeExpired`, but `FinishQuizDialog` only observes the new value on the next render. The old `s.submitting` guard was there to prevent `setShowFinishDialog(true)` when a manual submit was already in flight — but that was wrong because the dialog should still open and show the expired state. The new code is semantically correct: `autoSubmitFiredRef.current = true` sets the ref and `setShowFinishDialog(true)` causes a re-render that reads the updated ref. The `submitting` guard was masking the expired state, and removing it is right. No double-dialog risk: `autoSubmitFiredRef` gates the callback and `setShowFinishDialog` only opens (never re-opens) the dialog. No regression.
- **Suggestion 1:** `timeExpired={autoSubmitFiredRef.current}` at quiz-session.tsx:183 is a snapshot of a mutable ref read at each render. If `handleTimeExpired` fires while React batches a state update, the new render will read `true` correctly. This is safe today, but it is a subtle implicit dependency — the `FinishQuizDialog` visibility of `timeExpired` depends on the render triggered by `setShowFinishDialog(true)`. Worth a comment noting that `autoSubmitFiredRef.current` is always `true` when the dialog is in expired-exam mode.

**answer-options.tsx — double guard**
- **Issue:** `handleSelect` at line 58 checks `isExam && lockedSelection != null`, and the button `disabled` at line 83 checks `!!isExam && lockedSelection != null`. The `handleSelect` guard is already unreachable when the button is `disabled` — `onClick` does not fire on disabled HTML buttons. The guard in `handleSelect` is therefore redundant. This is not a bug (redundancy is harmless) but it means `handleSelect` can no longer be called from keyboard navigation or programmatically without hitting the disabled attribute first. Confirm there is no code path that calls `handleSelect` directly outside a click handler. There is not — no ref or callback exposes it. Verdict: the redundancy is safe. Flag as SUGGESTION to remove the guard from `handleSelect` in a cleanup pass.

**Zod schema vs RPC return shape**
- **Good 1:** `StartExamRpcResultSchema` fields `session_id`, `question_ids`, `time_limit_seconds`, `total_questions`, `pass_mark` exactly match the JSONB keys returned by `start_exam_session` RPC (migration 20260411000003, lines 133-139). No field mismatch.
- **Good 2:** `z.number().int().positive()` for `time_limit_seconds` and `total_questions` correctly rejects a `0` time limit (which would be a misconfiguration). `z.number().min(0).max(100)` for `pass_mark` permits `0` (valid: always passes). `z.array(z.uuid())` for `question_ids` correctly validates the array form — the RPC returns `to_jsonb(v_selected_ids)` which is a JSONB array. Shape is sound.
- **Good 3:** `total_questions` is validated by Zod but intentionally not forwarded in the `StartExamResult` success branch. This matches the prior pattern noted in the memory (2026-04-14 PR sweep) — the field is stored on `quiz_sessions.total_questions` at session creation time and read back at report time. The dead field in the response is harmless.
- **Good 4:** `safeParse` (not `parse`) is used on the RPC result, matching the pattern that server-side data should never throw on shape mismatch. The existing `.parse()` on user input is correct (throws on invalid input, which the outer try/catch handles). The two are semantically consistent.

**finish-quiz-dialog.tsx component split**
- **Good 5:** `DialogFooter` receives `timeExpired` as an optional prop and uses it only in the submit button's disabled condition `(answeredCount === 0 && !timeExpired)` — exactly the same expression as before the split. DOM structure, class names, and conditional rendering are byte-identical to the original inline block. No behavioral change.
- **Good 6:** `ExpiredNotice` is a pure render function with no state or effects. It exactly reproduces the original `<div>` with the same conditional countdown display `{!submitting && countdown > 0 && ...}`. No behavioral change.
- **Good 7:** Neither sub-component is exported. Both are file-private, so no risk of misuse outside `FinishQuizDialog`.

**session-card / session-table — MODE_LABELS.mock_exam**
- **Good 8:** `session-card.tsx` applies `uppercase` CSS class alongside `MODE_LABELS.mock_exam` (which returns `'Practice Exam'`). The rendered output `PRACTICE EXAM` is visually identical to the hardcoded `'PRACTICE EXAM'` it replaced. `session-table.tsx` `ExamBadge` already has `uppercase` on the `<span>` className, so `MODE_LABELS.mock_exam` renders identically there too. No visual regression.
- **Good 9:** Using `MODE_LABELS.mock_exam` as the single source of truth means if the label ever changes in `reports-utils.ts`, both badges update automatically. The prior hardcoded string was a silent drift risk.

**Patterns update**
- **Pattern — Zod safeParse vs parse discipline (reinforced):** `parse` on user-supplied input (throws, outer catch handles), `safeParse` on DB/RPC results (never throw on external data shape). This commit applies the pattern correctly. Consistent with the existing `batchSubmitQuiz` handling.
- **Pattern — redundant guard in handler vs disabled button (NEW, watching):** When a button is `disabled`, its `onClick` does not fire in HTML, making any guard in the click handler redundant. The guard is only useful if the handler can be invoked programmatically (no external caller here). Log for a second occurrence before adding to rules.

### 2026-04-26 — commits 74d557b + 0c49a09 + PR #523 pre-push sweep
- **Scope:** (A) reset-effect extension in finish-quiz-dialog.tsx + test; (B) rename Exam → Practice Exam ~20 files; (C) PR-level sweep master...HEAD (58 files).
- **CRITICAL:** 0 | **ISSUE:** 1 | **SUGGESTION:** 2 | **GOOD:** 8

**74d557b — reset discard confirm state when timeExpired flips**
- **Good 1:** Effect condition `if (!open || (timeExpired && isExam))` correctly preserves existing study-mode behavior. For study mode `isExam=false` so `(timeExpired && isExam) = false` — only `!open` triggers the reset, identical to the prior code. Zero regression risk.
- **Good 2:** Dependency array `[open, timeExpired, isExam]` is complete — all three are referenced in the condition. `setState` dispatchers (`setConfirmingDiscard`, `setConfirmingSubmit`) are stable and correctly omitted.
- **Good 3:** New test exercises the true→false rerender scenario (lines 292-341) and the forward flip (lines 343-375). Both are genuine behavioral contracts, not vacuous assertions.
- **Good 4:** The test correctly notes (comment: "stale state must NOT re-surface") the root concern — not just that the panel hides on the forward flip but that it stays hidden if timeExpired hypothetically reverts. This is the scenario the effect guards against.

**0c49a09 — rename Exam → Practice Exam**
- **Issue:** `use-exam-start.test.ts` lines 294 and 300 still assert `'Exam mode is not configured for this subject.'` but `start-exam.ts` now returns `'Practice Exam is not configured for this subject.'`. The test mocks `startExamSession` directly, so the test continues to pass (it echoes back its own mock value), but it no longer exercises the actual production error string. A future reader or contract test will not catch drift between the test's mock value and the real Server Action return. Fix: update lines 294 and 300 to `'Practice Exam is not configured for this subject.'` (the string now returned by `start-exam.ts:52`).
- **Good 5:** All seven other string updates in `start-exam.test.ts` (lines 100, 113, 124, 132, 140) correctly match the new production strings.
- **Good 6:** Every button-level test assertion in `finish-quiz-dialog.test.tsx` that interacts with a renamed button was updated at the same time the production string changed. No test fires the old button name and finds nothing.
- **Good 7:** Admin-facing strings (`Exam Configuration`, `Configure Exam`, `Exam mode enabled/disabled`) and semantic strings (`Exam Readiness` on the student dashboard) were intentionally left unrenamed. The commit is correctly scoped to interactive strings that directly convey the mode name to the student during a session.
- **Good 8:** `session-card.tsx` `EXAM` → `PRACTICE EXAM` and `ExamBadge` in `session-table.tsx` `Exam` → `Practice Exam` are consistent and match the `MODE_LABELS.mock_exam` update.

**PR-level sweep — master...HEAD**
- **Suggestion 1:** `start_exam_session` RPC returns a `total_questions` field in its jsonb result (migration 046 line 139). `StartExamRpcResult` in `start-exam.ts` declares `total_questions: number` but `StartExamResult` (the outward type) does not include it, and `use-exam-start.ts` does not read it from `result`. The field is effectively dead code in the TS layer. Not a bug — `total_questions` is stored on `quiz_sessions.total_questions` (line 113) and queried from there at report time. The unused field in the RPC response is harmless but worth noting.
- **Suggestion 2:** `handleDiscardSession` in `quiz-submit.ts` (lines 171-187) does not set `submitting: false` on the success path before `router.push` navigates away. This is safe because the component unmounts on navigation, but if navigation ever fails silently (rare Next.js edge case), the dialog would remain in a stuck `submitting: true` state with no recovery path. The submit path already handles this symmetrically via `opts.setSubmitting(false)` on error.
- **Pattern — test mock string drift after copy rename (NEW):** When a Server Action's user-visible error strings are renamed, tests that mock the Server Action directly (not the RPC beneath it) will not break — they echo their own mock value. But if the mock value is never updated, it diverges from the real return. Rule: when renaming strings in a Server Action, grep the corresponding `.test.ts` files for the old string and update mock values alongside assertions.
- **Pattern — canDismiss guard must gate both trigger and panel (RESOLVED — fix verified in 74d557b):** Pattern identified in 34194aa (the `confirmingDiscard && canDismiss &&` fix), then the useEffect extension in 74d557b closes the remaining gap. Both the static guard and the reactive reset are now in place. Marked resolved.

### 2026-04-26 — commit 34194aa (fix(exam): lift !isExam gate on discard, add exam-mode confirm copy)
- **Files reviewed:** finish-quiz-dialog.tsx (changed), finish-quiz-dialog.test.tsx (changed), quiz-session.tsx (caller), quiz-submit.ts (discard path), discard.ts (Server Action), use-auto-submit-countdown.ts (timer)
- **CRITICAL:** 0 | **ISSUE:** 1 | **SUGGESTION:** 2 | **GOOD:** 4
- **Issue:** `confirmingDiscard` can be true when `timeExpired` flips to true mid-confirm. The ConfirmPanel render condition `{confirmingDiscard && (` does not check `canDismiss`. Student opens discard confirm (timeExpired=false), auto-submit fires (timeExpired=true via autoSubmitFiredRef), `open` does not change so the reset effect does not run, ConfirmPanel stays visible. Student clicks "Yes, discard" in parallel with the auto-submit countdown firing `onSubmit`. Both `discardQuiz` (soft-delete) and `batchSubmitQuiz` race against the same `quiz_sessions` row. Winner routes to their respective page; loser sees zero-row error. Fix: `{confirmingDiscard && canDismiss && (` at line 121 — mirrors the trigger button condition at line 167.
- **Suggestion 1:** No test for confirmingDiscard-visible-when-timeExpired-flips. The dynamic race scenario (open panel while timeExpired=false, rerender with timeExpired=true, assert panel gone) is unverified.
- **Suggestion 2:** Exam-mode discard-while-submitting test only checks the trigger button, not the "Yes, discard" confirm button state when submitting flips to true after panel opens.
- **Good 1:** canDismiss derived from props (not state) — instantly correct on next render cycle.
- **Good 2:** open=false reset effect still correct — no change needed, none made.
- **Good 3:** discard.ts handles exam path identically to study path — auth, ownership, ended_at IS NULL, zero-row guard all correct.
- **Good 4:** handleSubmitClick clears confirmingDiscard before toggling submit confirmation — prevents dual-panel rendering.
- **Pattern — `canDismiss` guard must gate both the trigger and the confirm panel (NEW):** When a boolean guard prevents opening a confirmation flow (here: `canDismiss` gates the discard trigger button), the same guard must also close the ConfirmPanel if the flag changes while the panel is open. A `useEffect` reset only fires on specific prop changes; it will not fire if the parent re-renders with a new value without changing the `open` prop. Rule: every ConfirmPanel render condition should be `{confirmingX && canDoX && (` not just `{confirmingX && (`.
- **Pattern — dynamic prop flip while panel is open creates race (SECOND OCCURRENCE):** The first occurrence was fire-and-forget + redirect (b9829c0). Here the same class of bug: a prop transition creates a window between UI state and action state. When a prop transition can invalidate an in-progress user action (confirm panel open), the rendering must respond immediately to the new prop value — not rely on a useEffect that only runs on a different dependency.

### 2026-04-14 — commit b9829c0 (fix(exam): soft-delete DB session on zero-answer exam timeout)
- **Files reviewed:** quiz-submit.ts (changed), quiz-submit.test.ts (changed), discard.ts (Server Action), 20260413000001_fix_exam_distribution_order.sql (start_exam_session RPC)
- **CRITICAL:** 0 | **ISSUE:** 1 | **SUGGESTION:** 1 | **GOOD:** 4
- **Issue:** Fire-and-forget `discardQuiz(...).catch(() => {})` followed immediately by `router.push` creates a race window. If the student navigates back and clicks Start Exam before the background DB UPDATE completes, `start_exam_session` still finds `deleted_at IS NULL` and raises "already in progress." Self-healing in ~200–800ms but reproducible on fast networks. Additionally, `.catch(() => {})` suppresses all rejection silently — if the Server Action fails entirely, the orphan persists indefinitely with no server log. Fix: await before push (the existing `discardQuizSession` helper on line 42 already does this correctly).
- **Suggestion:** Test "always redirects even when discardQuiz fails" covers rejection path only. If fix option A (await) is applied, add a test for `{ success: false }` resolution path to confirm redirect still happens on non-throwing failures.
- **Good 1:** `discardQuiz` auth check order is correct: `getUser()` before `parse()` — unauthenticated callers are rejected before input is parsed.
- **Good 2:** Zero-row guard on the soft-delete UPDATE (`.select('id')` + `!sessionData?.length`) matches project code-style.md §5 pattern.
- **Good 3:** "always redirects even when discardQuiz fails" test is a genuine behavioral contract test, not a vacuous one.
- **Good 4:** Fire-and-forget is correct for the happy path — the fix addresses the root problem from the 2026-04-14 PR sweep.
- **Pattern — fire-and-forget + immediate redirect = race window (NEW):** When a client-side fire-and-forget Server Action modifies a DB guard condition (e.g., `deleted_at`), and the very next line navigates the user to a page where they can trigger that guard again, a race window opens. The race is proportional to Server Action latency (~200–800ms). Rule: if a fire-and-forget action clears a guard that the user could immediately re-trigger, await it instead. Fire-and-forget is safe only for cleanup that has no re-entry guard dependency (e.g., `clearDeploymentPin`, draft cleanup).
- **Pattern — .catch(() => {}) suppresses server-action failures silently (WATCHING):** Fire-and-forget `.catch(() => {})` discards both network errors and `{ success: false }` returns. When the action contains a `console.error` internally, those errors ARE logged on success-false paths (because the action returns, not throws). But if the action throws before the try/catch (unlikely here, but possible), the outer `.catch` swallows it. At minimum prefer `.catch((err) => console.error('[context] action failed:', err))`.

### 2026-04-14 — PR #523 full diff sweep (master...HEAD, 45 files, ~2689 lines, student exam mode final)
- **Scope:** PR-level semantic review for student exam mode after 4 fix commits (34070af, ea3d8d7, bdeafc3, 6954711, da5ba6c). Includes all 15 commits on the branch.
- **CRITICAL:** 0 | **ISSUE:** 1 | **SUGGESTION:** 2 | **GOOD:** 17
- **Issue:** Zero-answer exam timeout does NOT call `discardQuiz` Server Action, leaving the DB `quiz_sessions` row with `ended_at IS NULL`. When the student next attempts to start an exam for the same subject, `start_exam_session` RPC raises "an exam session is already in progress" and they are permanently blocked. The comment at `quiz-submit.ts:110` says "discard and redirect" but only redirects. Fix: replace the `clearActiveSession + router.push` block with a `discardQuizSession(...)` call (which already handles the redirect on success).
- **Suggestion 1:** `toSessionData` (quiz-session-storage.ts:174) converts `ActiveSession` to `SessionData` but does not map `timeLimitSeconds` or `passMark` fields. These fields exist on `SessionData` but `ActiveSession` lacks them entirely. The exam recovery path in `use-session-bootstrap.ts` already clears exam sessions from localStorage before calling `toSessionData`, so this gap is never reached in practice. But if the guard were ever relaxed, exam sessions would load without a timer or pass mark. Consider adding the fields to `ActiveSession` and propagating them through `toSessionData` as a hardening measure.
- **Suggestion 2:** Report page shows FAILED for expired exams (grace-period and zero-answer paths both result in `passed: false`) without any "expired" indicator. The `BatchSubmitResult` type and `batch_submit_quiz` RPC both surface `expired: true`, but the result is not passed through `handleSubmitSession` or the report query. A student who was mid-exam when their network dropped would see a FAILED score with no context. Non-blocking UX gap.
- **Good 1-12:** All positives from the 2026-04-13 PR #523 sweep (listed there) still hold in the final branch state.
- **Good 13:** `batchSubmitQuiz` returns `expired: data.expired ?? false` — correct null-safe default for the common (non-expired) path.
- **Good 14:** `BatchSubmitResult` type correctly uses optional `expired?: boolean` rather than a discriminated union — appropriate since the expired path is a sub-case of success, not a failure mode.
- **Good 15:** `answeredCount ?? session.total_questions` fallback in `quiz-report.ts:110` correctly handles the `COUNT(*) = 0` case — `0 ?? 50` is `0` (not the fallback), since `0` is not null/undefined. The fallback only triggers if the Supabase count query returns `null` (network error), in which case `total_questions` is a reasonable substitute.
- **Good 16:** `getExamEnabledSubjects()` uses `createServerSupabaseClient()` (RLS-scoped), not `adminClient`. The `student_select_exam_configs` RLS policy correctly enforces org-scoping. Query is auth-scoped at every layer.
- **Good 17:** Report questions query (`quiz-report-questions.ts`) uses `buildReportQuestions` which maps options to `{id, text}` only, stripping the `correct: boolean` DB field before returning. Options are never returned with `correct` to the client. The `QuestionRow` type defines `options: {id: string; text: string}[]` with no `correct` field, reinforcing the strip.
- **Pattern — zero-answer exam timeout = orphaned DB session (NEW — severity escalated from suggestion to issue):** The zero-answer exam timeout path (`handleSubmitSession` when `isExam && answers.size === 0`) redirects and clears localStorage but never calls the `discardQuiz` Server Action. The DB `quiz_sessions` row remains with `ended_at IS NULL`, and `start_exam_session` guards against exactly this condition. This is the third time this class of "client redirect without server cleanup" has been reviewed. Rule: every client-side terminal path (submit, discard, timeout) for a session that has a DB row MUST call a Server Action to end or delete the row.
- **Pattern — field absence vs field gap in storage converters (WATCHING):** `toSessionData` converts `ActiveSession` → `SessionData` but silently drops new fields when they're not added to the source type. This is the second time a storage converter has been found missing fields added to the target type. A pattern to watch: when adding fields to `SessionData`, always check all functions that create `SessionData` (currently: `readSessionHandoff`, `toSessionData`).
- **Pattern — test mock string drift after Server Action string rename (NEW, 2026-04-26):** When a Server Action's user-visible error strings are renamed, tests that mock the Server Action directly (not the RPC beneath it) will not break — they echo their own mock value. But if the mock value is never updated, it diverges from the real return. `use-exam-start.test.ts` lines 294/300 still assert `'Exam mode is not configured for this subject.'` but `start-exam.ts` now returns `'Practice Exam is not configured for this subject.'`. The test echoes its own mock, so it passes silently despite the mismatch. Rule: when renaming strings in a Server Action, grep the corresponding `.test.ts` files for the old string and update mock values alongside assertions. First occurrence.



### 2026-04-14 — commit ce67aee (fix(exam): await discardQuiz before redirect to close race window)
- **Files reviewed:** quiz-submit.ts (changed), quiz-submit.test.ts (changed)
- **CRITICAL:** 0 | **ISSUE:** 0 | **SUGGESTION:** 0 | **GOOD:** 3
- **Good 1:** Race window is closed. `await discardQuiz(...).catch((err) => console.error(...))` on line 116 ensures the DB soft-delete completes before `router.push` executes. `start_exam_session` can no longer observe the stale `deleted_at IS NULL` row.
- **Good 2:** `.catch((err) => console.error(...))` replaces the silent `.catch(() => {})` from the prior commit, directly addressing the second finding from the b9829c0 review. Server Action failures are now logged server-side with context ('[handleSubmitSession] discardQuiz failed:') while still allowing unconditional redirect.
- **Good 3:** New `{ success: false }` test (line 550-555) covers the resolution path the prior suggestion requested. Combined with the existing rejection test, both non-success outcomes (throw and soft failure) are now verified to not suppress the redirect.
- **Pattern — fire-and-forget + guard race (RESOLVED):** Pattern first identified in b9829c0, resolved here. Marked resolved — no further instances outstanding.

### 2026-04-14 — commit 34070af (test(exam): add auto-submit countdown tests and clarify effect ordering)
- **Files reviewed:** use-auto-submit-countdown.test.ts (changed), use-auto-submit-countdown.ts (hook source, read for verification)
- **CRITICAL:** 0 | **ISSUE:** 0 | **SUGGESTION:** 0 | **GOOD:** 1
- **Good:** The `seconds: 2 → 3` change in the "does not fire onSubmit twice" test is correct and meaningful. The previous test (seconds: 2) was a false positive: because `seconds` was already 2, the dep array `[true, false, 2]` did not change on rerender, so the effect never re-ran and the firedRef guard was never exercised. The new test (seconds: 3) causes a genuine dep change, actually re-runs the effect, and the `firedRef.current === true` early-return is what prevents the second fire. The test now validates the claimed behavior. Effect 2 (reset, lines 40-45) does not interfere — it checks `if (!opts.active)` which is false in this scenario. The `setCountdown(opts.seconds)` on line 21 is only reached if Effect 1 does NOT early-return, so no state corruption from Effect 2's secondary `setCountdown`.
- **Pattern — vacuous test via unchanged dep array:** When a test intends to verify an effect guard against re-run, it must change a dep array value that actually causes the effect to re-run. Passing the same value that was already in the dep array silently skips the effect entirely, making the guard assertion vacuous. Always use a different dep value than the initial one when testing re-run guards.

### 2026-04-13 — commit ea3d8d7 (fix(exam): stabilize auto-submit countdown with ref-capture pattern)
- **Files reviewed:** use-auto-submit-countdown.ts (changed), finish-quiz-dialog.tsx (caller), quiz-session.tsx (upstream), use-quiz-submit.ts (onSubmit source), exam-countdown-timer.tsx (pattern reference)
- **CRITICAL:** 0 | **ISSUE:** 1 | **SUGGESTION:** 1 | **GOOD:** 3
- **Issue:** The two useEffect hooks share firedRef through a load-bearing ordering dependency: Effect 2 (reset) must execute after Effect 1 (interval) for the cleanup + reset sequence to be correct. This ordering is not documented with a comment. A future refactor could silently swap the effect order, breaking the reset semantics. Fix: add a comment above Effect 2 stating the ordering is intentional and load-bearing.
- **Suggestion:** `use-auto-submit-countdown.ts` has no test file. The ref-capture change is the highest-risk line in this diff — a stale-ref bug here would be silent in tests. The hook's behavioral contract (fires once at t=0, does not restart on onSubmit reference changes, resets cleanly on deactivation) should be covered by a co-located test file, consistent with ExamCountdownTimer which has comprehensive tests for the same pattern.
- **Positive 1:** `onSubmitRef.current = opts.onSubmit` is correctly placed in the function body (not inside a useEffect), so it executes synchronously before effects on each render. The latest onSubmit is always in the ref when the interval fires. This is the canonical ref-capture pattern.
- **Positive 2:** The firedRef double-fire guard is more reliable after this fix. Previously, a new onSubmit reference would re-run the interval effect (clearing + restarting the interval), potentially while firedRef was still false. After the fix, the dep array is stable and the interval only restarts when active/submitting/seconds change.
- **Positive 3:** Pattern consistency with ExamCountdownTimer. Both now use identical ref-capture for callbacks passed in as props.
- **Pattern — load-bearing useEffect ordering needs comment:** When two sibling useEffect hooks have an implicit ordering dependency (e.g., one clears a ref that the other reads), that ordering must be documented with a comment. React guarantees effects run top-to-bottom in order of appearance, but this is a non-obvious invariant. Document it or rewrite to remove the coupling.
- **Pattern — ref-capture (resolved):** The `opts.onSubmit` dep array instability issue (first seen in PR #514 sweep) is now fixed with the ref-capture pattern in the hook itself. Both ExamCountdownTimer and useAutoSubmitCountdown now use this pattern. This pattern should be the standard for all future hooks that accept callback props passed into interval/timeout effects.

### 2026-04-13 — PR #523 full diff sweep (master...HEAD, 44 files, ~2494 lines, student exam mode PR2 + fixes)
- **Scope:** Full PR-level semantic review for student exam mode PR2. 12 commits: RPC fix (distribution ordering + graceful timeout), exam setup UI, countdown, answer buffer, auto-submit, results pass/fail, zero-answer timeout fix, CodeRabbit fixes, security (deleted_at filter).
- **CRITICAL:** 0 | **ISSUE:** 1 | **SUGGESTION:** 2 | **GOOD:** 12
- **Issue:** `useAutoSubmitCountdown` in `finish-quiz-dialog.tsx` lists `opts.onSubmit` in its dep array. `onSubmit` is `s.handleSubmit` from `useQuizSubmit`, which is a plain function (not `useCallback`) recreated on every render. The first render after dialog open (`s.setShowFinishDialog(true)`) triggers a re-render, which creates a new `handleSubmit` reference, which re-runs the countdown effect, resetting the displayed counter to 10s and restarting the interval. In practice only one re-render occurs at dialog open (submitting state is stable), so the countdown starts cleanly after one reset. This is tolerable, but the mechanism is fragile — any additional re-render (parent component state change, future feature addition) would reset the countdown again. Fix: wrap `handleSubmit` in `useCallback` in `useQuizSubmit` with `[opts.*]` stable deps, or capture the onSubmit in a `useRef` inside `useAutoSubmitCountdown` and exclude it from deps.
- **Suggestion 1:** `timerStartRef.current = Date.now()` at component mount still has timer drift (already noted in PR #514 sweep, still not addressed). For a 10-minute exam, 2-5s drift is meaningful. Low urgency but worth a follow-up issue.
- **Suggestion 2:** `getExamEnabledSubjects()` in `exam-subjects.ts` uses `as unknown as { id, code, name, short }` to cast the `easa_subjects` JOIN result without a runtime shape guard. Supabase returns array for 1-to-many or null for no match — the `.filter(row => row.easa_subjects !== null)` correctly excludes nulls, but if `easa_subjects` is an array (e.g., FK misconfigured), the code will silently use array fields as strings. Mitigated by FK constraint in DB, but a `typeof subject.id === 'string'` guard would make the type assumption explicit.
- **Positive 1:** The zero-answer exam timeout deadlock (flagged in PR #514 sweep as ISSUE) is correctly fixed. `handleSubmitSession` now redirects to `/app/quiz` when `isExam && answers.size === 0`, bypassing `batchSubmitQuiz` entirely.
- **Positive 2:** Both ExamCountdownTimer instances (mobile top-bar and desktop info-bar) share the same `autoSubmitFiredRef` guard via `handleTimeExpired`. Double-fire from two simultaneous interval expirations is prevented.
- **Positive 3:** Security — `correct_option_id` is never exposed during an active exam. Correct answers are only returned in `batchSubmitQuiz` response after session completion, then the user is routed to the report page. Exam answer buffer (`use-exam-answer-buffer.ts`) holds only `selectedOptionId + responseTimeMs`.
- **Positive 4:** Exam sessions are never persisted to localStorage. `useExamPipeline` uses `useExamAnswerBuffer` (local state only), not `useAnswerPipeline` (which calls `useQuizPersistence` → `writeActiveSession`). localStorage recovery path in `use-session-bootstrap.ts` correctly blocks exam mode recovery with toast + redirect.
- **Positive 5:** `start_exam_session` RPC: SECURITY DEFINER + `SET search_path = public` + explicit `auth.uid()` null check + `NULLS LAST` ordering fix. All four SECURITY DEFINER requirements met.
- **Positive 6:** `batch_submit_quiz` (migration 047): SECURITY DEFINER + `SET search_path = public` + auth check. Grace period logic (30s) handles race between client timer expiry and submission. Idempotent replay (ended_at IS NOT NULL) correctly returns existing results. Score denominator is `v_total` for exam mode (unanswered = wrong). All consistent.
- **Positive 7:** Both migration files are byte-identical between `packages/db/migrations/` and `supabase/migrations/`. Dual-directory discipline maintained for 046 and 047.
- **Positive 8:** `BatchSubmitInput` Zod schema enforces `.min(1)` on answers, which correctly validates that batchSubmitQuiz is never called with an empty array from the study path. The exam zero-answer path redirects before calling batchSubmitQuiz, so the Zod gate is never a conflict.
- **Positive 9:** `student_select_exam_configs` RLS policy correctly scopes to `organization_id = (SELECT organization_id FROM users WHERE id = auth.uid())`. `getExamEnabledSubjects()` uses `createServerSupabaseClient()` (auth-scoped) not adminClient, so RLS applies.
- **Positive 10:** `useNavigationGuard(!isExam && answers.size > initialSize.current && !submitted.current)` correctly disables the navigation guard in exam mode. Exam sessions have no localStorage checkpoint, so blocking navigation would trap students after answer loss.
- **Positive 11:** All test fixtures updated consistently — `QuizReportSummary` mock now includes `passed: null, timeLimitSeconds: null`, `QuizConfigForm` tests pass `examSubjects={[]}`, `ModeToggle` tests updated for new `examAvailable` prop. Cross-commit test/production consistency maintained.
- **Positive 12:** `startExamSession` Server Action: auth check is pre-RPC and explicit. Zod validation on `subjectId` (UUID format). RPC error messages are domain-specific (never leaks raw Postgres error). Outer try/catch with generic message. Matches project Server Action pattern exactly.
- **Pattern — unstable function reference in useEffect dep array:** Plain functions defined inside hooks (not wrapped in `useCallback`) create new references each render. When passed into a hook's dep array, this can cause effects to re-run unexpectedly. This is the second occurrence of this pattern (first: answer pipeline in PR #514). Rule to add: any function passed as a dep to `useEffect` or `useMemo` must be either a `useCallback` or captured in a ref.
- **Pattern — timer drift from mount vs server start:** Client timers initialized at component mount time will always show more time than the server has remaining (by the mount latency: navigation + questions fetch + render). The precise fix is to return `started_at` from the start-session RPC and thread it through `SessionData`. This is the second time this has been noted without being fixed. Should become an Issue on the next occurrence.

### 2026-04-13 — commit da5ba6c (fix(exam): address 3 valid CodeRabbit findings on session hooks)
- **Files reviewed:** quiz-submit.ts, use-exam-answer-buffer.ts, use-exam-state.ts (plus consumers: use-quiz-state.ts, quiz-session.tsx, use-answer-handler.ts)
- **CRITICAL:** 0 | **ISSUE:** 0 | **SUGGESTION:** 1 | **GOOD:** 3
- **Suggestion:** `use-exam-answer-buffer.test.ts` — test file still uses `await result.current.confirmAnswer(...)` after `confirmAnswer` was de-asyncified. `await` on a non-Promise is harmless in JS (resolves immediately), so tests pass. But the tests now misrepresent the actual signature. Cleanup: remove `async` from `act()` callbacks and `await` from `confirmAnswer` call sites. The same-tick `Promise.all([p1, p2])` test still works because `Promise.all([boolean, boolean])` resolves correctly.
- **Positive 1:** `clearDeploymentPin().catch(() => {})` added to zero-answer exam timeout path completes consistency across all 4 session-terminal paths in `quiz-submit.ts`. Fire-and-forget is appropriate for a best-effort cookie clear.
- **Positive 2:** Removing spurious `async` from `confirmAnswer` makes the type honest — the function had no `await` inside and is correctly synchronous.
- **Positive 3:** `feedback: emptyFeedbackRef.current` in `useExamPipeline` stabilizes the reference across renders (vs `new Map()` which created a fresh object each time), preventing false dependency changes in consumers. `handleSelectAnswer: (id) => Promise.resolve(confirmAnswer(id))` normalizes the exam pipeline's return type to `Promise<boolean>`, matching the study pipeline and making `QuizState = ReturnType<typeof useQuizState>` consistent across modes.
- **Pattern — test signature drift on de-asyncification:** When a function's `async` keyword is removed, its test file must be updated to remove `await` from call sites and `async` from wrapping lambdas. The tests will still pass (await on non-Promise is a no-op), but they misrepresent the type, creating reader confusion. This is a recurring pattern to watch.
- **Pattern — `new Map()` in render return vs stable ref:** Returning `new Map()` directly in a component/hook's return object creates a new reference on every render. If that value flows into a `useMemo` dep array or `useEffect` dep array downstream, it will trigger unnecessary re-computations. Always use a stable ref (`useRef(new Map())`) when returning an empty immutable structure.

### 2026-04-13 — PR #514 full diff sweep (master...HEAD, 35 files, ~1805 lines, student exam mode)
- **Scope:** Full PR-level semantic review for student exam mode (PR2). 7 commits: RPC fixes, types, startExamSession action, exam subjects query, setup UI, session behavior (countdown, answer buffer, auto-submit), results (pass/fail badge).
- **CRITICAL:** 0 | **ISSUE:** 1 | **SUGGESTION:** 1 | **GOOD:** 9
- **Issue:** Zero-answer exam timeout causes UX deadlock. When a student opens an exam, answers 0 questions, and the timer expires: (1) `handleTimeExpired` fires, `setShowFinishDialog(true)`. (2) `FinishQuizDialog` opens with `timeExpired=true`, `answeredCount=0`. (3) Auto-submit countdown starts (10s), calls `handleSubmit()`. (4) `handleSubmitSession` at `quiz-submit.ts:107` guards `answers.size === 0` → sets error "No answers to submit." and returns. (5) Dialog displays the error but `handleClose()` returns early when `timeExpired && isExam` — user cannot dismiss. The student is locked in the dialog with no valid action. Fix: for exam mode, bypass the `answers.size === 0` guard and call `batchSubmitQuiz` directly with an empty array; the RPC grace-period path will auto-end the session with 0 score and return `expired: true`, which navigates to the report page.
- **Suggestion:** `timerStartRef.current = Date.now()` at component mount in `quiz-session.tsx:45` introduces a timer drift of ~300–600ms from the server's `started_at` (RPC response + navigation latency). For typical exam durations (45–120 minutes) this is negligible, and the 30s RPC grace window covers it entirely. The precise fix would be to pass `session.startedAt` from the DB through `SessionData` → `QuizSessionLoader` props → `ExamCountdownTimer.startedAt`. Non-blocking.
- **Positive 1:** `correct_option_id` is not exposed during the exam. The per-question study path (`check-answer.ts`) is bypassed; the exam buffer (`use-exam-answer-buffer.ts`) holds only `selectedOptionId + responseTimeMs`. Correct answers only arrive in the `batchSubmitQuiz` result after session completion. Security boundary holds.
- **Positive 2:** Auth check in `startExamSession` is explicit and pre-RPC: `supabase.auth.getUser()` → null check → `return { success: false, error: 'Not authenticated' }` before the RPC call. Defense in depth is present.
- **Positive 3:** Score denominator is consistent across all three layers: `batch_submit_quiz` SQL uses `v_total` (not `v_answered`) for exam mode; `result-summary.tsx` uses `totalQuestions` (not `answeredCount`); `quiz-report-questions.ts` reads from `quiz_session_answers` which stores per-answered records. All three agree on semantics: unanswered = wrong.
- **Positive 4:** Auto-submit double-fire is prevented. Both `ExamCountdownTimer` instances (mobile top bar and desktop info bar) call the same `handleTimeExpired` callback, which checks `autoSubmitFiredRef.current` before firing — the second timer's expiry call is a no-op.
- **Positive 5:** Exam session data (`mode`, `timeLimitSeconds`, `passMark`) is correctly threaded: `sessionHandoff` → `isValidSessionData` → `hasValidOptionalFields` validator → `SessionData` type → `QuizSessionLoader` props → `QuizSession` props → `useQuizState` opts. The validator at `quiz-session-validators.ts` correctly guards the new fields with type and range checks.
- **Positive 6:** Navigation guard is correctly disabled for exam mode: `useNavigationGuard(!isExam && answers.size > initialSize.current && !submitted.current)`. Exam sessions have no localStorage checkpoint, so the guard would be meaningless and would incorrectly block navigation.
- **Positive 7:** `use-session-bootstrap.ts` correctly handles exam mode in the localStorage recovery path: `stored.mode === 'exam'` → `clearActiveSession` → toast → `router.replace('/app/quiz')`. Exam sessions cannot be recovered from localStorage (by design).
- **Positive 8:** Both migration directories (`packages/db/migrations/046-047`, `supabase/migrations/20260413000001-2`) are byte-identical. Dual-directory discipline maintained.
- **Positive 9:** RPC `batch_submit_quiz` graceful timeout path correctly handles the full lifecycle — ends the session with 0 score, logs `exam.expired` audit event, returns `expired: true`. The idempotent replay path (already-ended session) also correctly reads `v_passed` from the initial SELECT rather than a separate subquery.
- **Pattern — exam mode UX deadlock on 0-answer timeout:** When a client-side guard (e.g., `answers.size === 0`) exists for normal flow correctness, it must be bypassed or replaced for edge cases where the guard must not apply (e.g., exam timeout forcing submission of empty answer set). The RPC-side guard (empty array raises) is the wrong layer to catch this — the client must handle it by redirecting to the report or calling a dedicated expire endpoint.

### 2026-04-13 — commit 1395d92 (fix(exam-config): add try/catch to Server Action callers)
- **Files reviewed:** config-form-dialog.tsx, subject-config-card.tsx, plus sibling context (toggle-exam-config.ts, upsert-exam-config.ts, require-admin.ts, question-form-dialog.tsx, student-form-dialog.tsx, delete-button.tsx, rethrow-redirect.ts)
- **CRITICAL:** 0 | **ISSUE:** 1 | **SUGGESTION:** 1 | **GOOD:** 3
- **Issue:** Bare `catch { toast.error(...) }` blocks in both changed components (and in pre-existing sibling components: question-form-dialog, student-form-dialog, delete-button) do not call `rethrowRedirect(error)` before the fallback handler. `requireAdmin()` uses `redirect()` internally which throws a `NEXT_REDIRECT` error. Server Component try/catch blocks in this codebase consistently call `rethrowRedirect(error)` first (5 usages in admin/dashboard/). Client-component catch blocks wrapping Server Action calls do not. This is a behavioral inconsistency. Runtime impact is likely low (Next.js handles redirect errors at the Server Action transport layer), but the pattern should be uniform. Fix: add `rethrowRedirect(error)` as first statement in all 5 affected catch blocks.
- **Suggestion:** Catch-block toast messages are identical to the Server Action's structured `result.error` messages. There is no observable difference between a handled domain error and an unexpected throw. Consider making catch messages slightly distinct for production debugging.
- **Positive 1:** Both changed components follow exactly the same try/catch structure — symmetric behavior, no inconsistency between config-form-dialog and subject-config-card.
- **Positive 2:** Catch blocks are correctly placed inside the `startTransition` async callback, not around it — correctly scoped to catch async throws.
- **Positive 3:** Commit brings the two new components into alignment with the existing sibling pattern (question-form-dialog, student-form-dialog, delete-button) — consistent application within the established pattern.
- **Pattern — bare catch in client-component Server Action callers:** Client components that call Server Actions inside try/catch blocks must call `rethrowRedirect(error)` before any fallback handling. This is the same rule that applies to Server Component try/catch blocks. The project has `rethrow-redirect.ts` specifically for this. Gap: the 5 existing client-component action callers all have bare catch blocks. Should be fixed as a batch.
- **Pattern — catch message deduplication risk:** When a catch-block message is identical to a Server Action's `result.error` string, the two error paths are indistinguishable in the UI. Keep catch messages semantically distinct from domain error messages.

### 2026-04-11 — PR #516 full diff sweep (master...HEAD, 52 files, ~5680 lines)
- **Scope:** Full PR-level semantic review for exam-config admin feature (PR1). Covers all 10 commits: DB schema, RPCs, admin UI, Server Actions, Zod schemas, tests, docs.
- **CRITICAL:** 0 | **ISSUE:** 1 | **SUGGESTION:** 2 | **GOOD:** 8
- **Issue:** `upsert_exam_config` RPC (migration 045/20260411000008) accepts `p_distributions = '[]'::jsonb` without error. An empty distributions array deletes all existing distributions and saves the config without any distribution rows. The `total_questions` DB CHECK (> 0) protects the scalar, but no constraint enforces `sum(distribution.question_count) == total_questions` at the DB layer. The toggle-enable pre-flight check would later catch the mismatch, but the config is saved in an invalid state between upsert and toggle. Fix: add `IF jsonb_array_length(p_distributions) = 0 THEN RAISE EXCEPTION 'distributions must be non-empty'; END IF;` at the start of the distribution block, and add a sum validation matching `p_total_questions`.
- **Suggestion 1:** `041_batch_submit_exam_guard.sql` idempotent replay path (line ~95) returns `'passed', (SELECT qs.passed FROM quiz_sessions qs WHERE qs.id = p_session_id)` — a second SELECT on the same row. Migration 042 (`CREATE OR REPLACE`) supersedes this and correctly uses the already-fetched `v_passed` variable. The final live function is 042, so this is not a live bug. However, the intermediate function state (migration 041 alone) would execute two reads. Migrations should be clean end-states — migration 042 was correctly issued to fix it.
- **Suggestion 2:** `upsert_exam_config` RPC is not present in `packages/db/src/types.ts` Functions section. The `rpc()` wrapper in `lib/supabase-rpc.ts` works around this, but the RPC signature is untyped. After next `supabase gen types --linked`, the `types.ts` should be regenerated to include `upsert_exam_config` so it becomes type-checked.
- **Positive 1:** Both migration directories (packages/db/migrations/038-045, supabase/migrations/20260411000001-8) are byte-identical for all 8 PR migrations. Dual-directory maintenance discipline holds across all commits.
- **Positive 2:** Snake_case JSONB key fix (aac3e61) correctly updates SQL (both migration copies), TS action, and test assertions in a single commit. All three files are consistent: `topic_id`, `subtopic_id`, `question_count`.
- **Positive 3:** Zod `UpsertExamConfigSchema` cross-field `.refine()` enforces `sum(distributions.question_count) === totalQuestions` before the RPC is ever called. Client-side validation layer is thorough.
- **Positive 4:** Migration 042 (`exam_config_security_fixes`) correctly supersedes migration 041 (`batch_submit_exam_guard`) with three security improvements: server-side time limit enforcement, duplicate active exam session guard, and fetching `passed` from the main SELECT INTO instead of a secondary subquery.
- **Positive 5:** `toggle-exam-config.ts` — zero-row no-op check is present (`.select('id')` + `if (!data?.length)`) on the UPDATE, consistent with the project pattern from code-style.md Section 5.
- **Positive 6:** `queries.ts` error handling was fixed from the initial commit — all 6 parallel query errors now throw (`if (res.error) throw new Error(...)`) so the Suspense error boundary catches them rather than silently returning empty arrays.
- **Positive 7:** `exam_config_distributions` RLS correctly has no UPDATE policy — distributions are only replaced atomically via the SECURITY DEFINER RPC, never updated row-by-row from the application.
- **Positive 8:** `student_select_exam_configs` RLS policy correctly restricts students to `enabled = true AND deleted_at IS NULL` rows, while admin policies do not need these filters (the application-layer queries add them explicitly). Two-tier access control is consistent and intentional.
- **Pattern — SECURITY DEFINER RPCs should validate JSONB array inputs:** When a SECURITY DEFINER function receives a JSONB array parameter, it should validate array length > 0 and any business invariants (sum constraints) internally. Zod validates at the Server Action boundary, but the RPC is callable directly by any authenticated admin. Defense in depth requires the RPC to be self-defending.

### 2026-04-11 — commits edbfe3d, 8556c57, bd0d55a, 05609a7 (exam-config post-PR fixes: docs + tests)
- **Files reviewed:** docs/database.md, num-field.test.tsx (updated), 045_upsert_exam_config_rpc.sql comment, 20260411000008_upsert_exam_config_rpc.sql comment, queries.test.ts (new, 25 tests), distribution-editor.test.tsx (new, 11 tests), subject-config-card.test.tsx (new, 16 tests)
- **CRITICAL:** 0 | **ISSUE:** 0 | **SUGGESTION:** 1 | **GOOD:** 5
- **Suggestion:** `num-field.test.tsx` — the NaN-guard test still uses `userEvent.clear` (async) while the two clamping tests switched to `fireEvent.change` (sync). Inconsistency: the NaN guard test is async but the component's NaN guard fires synchronously on the change event. Using `fireEvent.change(input, { target: { value: '' } })` would be more consistent and faster. Non-blocking — the test correctly asserts the behavior.
- **Positive 1:** JSONB comment fix (bd0d55a) correctly updates both migration copies (packages/db and supabase/migrations) in the same commit. Dual-file maintenance discipline holds.
- **Positive 2:** soft-delete matrix entry for exam_config_distributions correctly documents the approved hard DELETE exception with rationale, distinguishing it from soft-deletable tables, and notes the ON DELETE CASCADE cascade path.
- **Positive 3:** num-field clamping tests now use exact `toBe(100)` / `toBe(5)` assertions instead of range assertions — catches regressions that would pass a `toBeLessThanOrEqual` check (e.g., returning 0 instead of 100 would pass `<= 100` but fails `=== 100`).
- **Positive 4:** queries.test.ts mock builder `buildTableMocks` uses per-table routing in `mockFrom.mockImplementation` and throws `new Error('Unexpected table in test: ...')` as a default case. This makes accidental extra DB calls immediately visible rather than silently returning undefined.
- **Positive 5 (CORRECTED):** `subject-config-card.test.tsx` asserts `onEdit` is NOT called when the toggle button is clicked, which is correct. However, the test comment says "stopPropagation prevents card click" — there is no `e.stopPropagation()` in the production code. The behavior works because the two buttons are DOM siblings (not parent/child), so click events on the toggle button never reach the `onEdit` button. The test passes for the right behavioral reason but the comment is misleading. SUGGESTION: update the test comment to say "sibling button layout prevents click propagation to card button" rather than attributing it to stopPropagation.
- **Pattern — fireEvent vs userEvent consistency:** When tests for controlled components are switched to `fireEvent.change` to bypass re-render cycling, all tests in the same describe block that share the same pattern should be migrated together for consistency. Mixed async/sync in the same file creates reader confusion.

### 2026-04-11 — commit edee5be (feat(exam-config): extract upsert_exam_config RPC for ACID compliance)
- **Files reviewed:** replace-distributions.ts (deleted), replace-distributions.test.ts (deleted), upsert-exam-config.ts, upsert-exam-config.test.ts, 045_upsert_exam_config_rpc.sql, supabase/migrations/20260411000008_upsert_exam_config_rpc.sql, docs/database.md, lib/supabase-rpc.ts
- **CRITICAL:** 0 | **ISSUE:** 1 | **SUGGESTION:** 2 | **GOOD:** 5
- **Issue:** `045_upsert_exam_config_rpc.sql:76-81` — JSONB key name mismatch. The RPC reads `(v_dist->>'topicId')`, `(v_dist->>'subtopicId')`, `(v_dist->>'questionCount')` (camelCase), but the Server Action passes camelCase object keys via `.rpc()` which Supabase wraps as a `jsonb` parameter. The TS→JSONB serialization preserves camelCase. So the key names match *today* — but this is a fragile implicit contract with no schema enforcement. Documented as SUGGESTION per analysis below.
- **Suggestion 1:** JSONB key naming convention — existing project RPCs use snake_case DB column names for JSONB (`topic_id`, `subtopic_id`, `question_count`). This RPC breaks convention by accepting camelCase JSONB keys (`topicId`, `subtopicId`, `questionCount`). There is no schema enforcement; a future refactor of the TS side could silently break the SQL. Consider standardizing to snake_case in the JSONB payload to match DB conventions.
- **Suggestion 2:** `upsert-exam-config.test.ts` — The `p_distributions` sent to the mock is `[{ topicId, subtopicId, questionCount }]` (camelCase) and the test asserts the same shape. The test does not verify the JSONB key names actually match what the SQL reads — it only tests that the TS→RPC argument is forwarded. No integration test exists for the SQL JSON key parsing. Any key-name divergence (camelCase TS vs. snake_case RPC) would not be caught by unit tests.
- **Positive 1:** SECURITY DEFINER checklist passes: `auth.uid()` guard present (line 36-38), `SET search_path = public` present, soft-delete filter on `users` (`deleted_at IS NULL`, line 42), role check present (line 46-48), org scoping via user's own row — not user-supplied org_id.
- **Positive 2:** True atomicity — lookup, upsert, delete distributions, insert distributions all occur in one plpgsql function body, within a single implicit Postgres transaction. Prior multi-step approach (4 separate Supabase queries in TypeScript) could leave distributions orphaned if an insert failed after a delete.
- **Positive 3:** Soft-delete guard on `exam_config_distributions` is correctly absent — `exam_config_distributions` is documented as ephemeral (no `deleted_at`), exempt from soft-delete rule per migration 043 comment. Hard DELETE is correct here.
- **Positive 4:** `NULLIF(v_dist->>'subtopicId', '')::uuid` handles both JSON `null` (which `->>'key'` returns as SQL `NULL`) and empty string (`''`), coercing both to `NULL`. This is more defensive than the prior TS-side `d.subtopicId ?? null`.
- **Positive 5:** Server Action correctly handles both error path (RPC error) and null-data path (should never happen but defensive). Two separate guards with distinct log messages — fixes the prior `Issue 2` from commit f5c1070 review.
- **Pattern — camelCase JSONB keys in SQL RPCs:** When a plpgsql function reads JSONB passed from TypeScript via Supabase `.rpc()`, the JSON key names must exactly match what the TS caller serializes. If the TS passes camelCase objects, the SQL must read camelCase keys. Prefer snake_case on both sides to match Postgres column naming conventions and avoid silent key-name divergence.
- **Pattern — unit test gap for JSONB key parsing:** Unit tests that mock `.rpc()` at the Supabase client level cannot detect mismatches between the TS-serialized JSONB keys and the SQL `->>'key'` reads. This category of bug requires an integration test (real Postgres) or an in-test SQL parse simulation.

### 2026-04-11 — commit f5c1070 (fix(exam-config): address CodeRabbit review findings for PR #516)
- **Files reviewed:** config-form-dialog.tsx, exam-config-page-shell.tsx, num-field.tsx, replace-distributions.ts, toggle-exam-config.ts, upsert-exam-config.ts, 044_exam_configs_partial_unique.sql, 20260411000007_exam_configs_partial_unique.sql, docs/database.md
- **CRITICAL:** 0 | **ISSUE:** 2 | **SUGGESTION:** 3 | **GOOD:** 6
- **Issue 1:** `replace-distributions.ts:1` — No `'use server'` or `import 'server-only'` directive. The file is a server-side helper in the actions/ folder, imports server-only dependencies, and receives an admin Supabase client. Without `import 'server-only'`, accidental import from a client component is not prevented at build time. Fix: add `import 'server-only'` as first line.
- **Issue 2:** `upsert-exam-config.ts:71-75` — `if (error || !data)` combined guard uses `error?.message` optional chaining. When `data` is null but `error` is null (RLS block returning 200 OK with zero rows), log emits `undefined` as the message, obscuring the failure mode. Split into two separate guards with distinct log messages, matching the pattern used in the update branch of the same file.
- **Pattern — server-only helpers in actions/ folder:** Any file in an actions/ directory that imports server-only modules or receives server clients must have `import 'server-only'` at the top if it is not itself a Server Action (no `'use server'`). This prevents accidental client bundle inclusion with a hard build error rather than a subtle runtime failure.
- **Pattern — conditional render vs open=false for Dialog close animation:** When using shadcn Dialog with `key` for remount-on-subject-switch, using `{editingSubject && <Dialog open={true}>}` causes immediate unmount on close, cancelling the close animation. The correct pattern is always-mounted with `open={!!editingSubject}` + `key={editingSubject?.id}`. The `key` handles remount-on-switch; `open={false}` handles the close animation.
- **Resolved from prior session:** Issue 1 from 2026-04-10 (table-level UNIQUE blocking soft-delete re-creation) is now correctly fixed via migration 044/20260411000007. Pattern confirmed and pattern note updated.
- **Positive — partial unique index migration:** The DROP CONSTRAINT + CREATE UNIQUE INDEX WHERE deleted_at IS NULL pattern is the correct fix for soft-delete + uniqueness on the same table. The constraint name `exam_configs_organization_id_subject_id_key` correctly matches the Postgres auto-generated name from `UNIQUE (organization_id, subject_id)` in the CREATE TABLE DDL.

### 2026-04-10 — commit b6dc271 (feat: exam mode admin config — exam_configs, start_exam_session RPC, batch_submit_quiz exam guard, admin UI)
- **Files reviewed:** 038_exam_configs.sql, 039_quiz_sessions_exam_columns.sql, 040_start_exam_session_rpc.sql, 041_batch_submit_exam_guard.sql, queries.ts, upsert-exam-config.ts, toggle-exam-config.ts, config-form-dialog.tsx, schema.ts
- **CRITICAL:** 0 | **ISSUE:** 3 | **SUGGESTION:** 3 | **GOOD:** 8
- **Issue 1:** `038_exam_configs.sql` — Table-level `UNIQUE (organization_id, subject_id)` will block re-creation of an exam config after soft-delete of the original. The partial index `WHERE deleted_at IS NULL` handles query perf but does not replace the uniqueness constraint. Fix: drop the table UNIQUE constraint and create `CREATE UNIQUE INDEX uq_exam_configs_active ON exam_configs (organization_id, subject_id) WHERE deleted_at IS NULL` in a follow-up migration before any delete UI is shipped.
- **Issue 2:** `queries.ts:25-26` — `exam_config_distributions` is fetched without any application-level org-scope filter. RLS correctly restricts the result, but the asymmetry vs. the `questions` query (which explicitly adds `eq('organization_id', organizationId)`) is confusing and would cause a data exposure bug if RLS is misconfigured. Also: no error checking on any of the 6 parallel queries — all silently fall back to `?? []`.
- **Issue 3:** `041_batch_submit_exam_guard.sql:64-90` — Idempotent replay path reads `passed` via a secondary SELECT subquery instead of including it in the main `FOR UPDATE` SELECT. The secondary read is consistent within the transaction but is a code smell and inconsistent with how `v_correct_count` and `v_score` are handled. Fix: add `qs.passed` to the initial SELECT at line 50.
- **Positive — SECURITY DEFINER checklist:** `start_exam_session` passes all checks: auth.uid() guard, SET search_path = public, soft-delete filter on users, soft-delete filter on exam_configs, status='active' AND deleted_at IS NULL on questions, org-scope via user's own row (not user input).
- **Positive — pass_mark from session config:** `batch_submit_quiz` reads `pass_mark` from `v_config->>'pass_mark'` (stored at session creation), not from live `exam_configs` lookup. This is correct — the pass mark at exam-start time is what counts.
- **Positive — duplicate prevention in RPC:** `q.id != ALL(v_selected_ids)` in `start_exam_session` prevents cross-distribution question duplication even for overlapping topic/subtopic selections.
- **Pattern — table-level UNIQUE vs. partial UNIQUE INDEX for soft-delete tables:** When a table uses soft-delete (`deleted_at`) and has a UNIQUE constraint on a business key, the table-level UNIQUE constraint covers ALL rows including soft-deleted ones. A re-insert after soft-delete will violate the constraint. Always use a partial unique index `WHERE deleted_at IS NULL` instead of a table-level UNIQUE constraint on soft-deletable tables.
- **Pattern — explicit org-scope in parallel Supabase queries:** In parallel `Promise.all` admin queries, always add explicit `.eq('organization_id', organizationId)` or an equivalent filter even when RLS provides the same guarantee. This makes the intent clear, protects against RLS misconfiguration, and matches the pattern used elsewhere. Document when RLS is the sole enforcement mechanism.
- **Pattern — error checking in RSC parallel queries:** All Supabase responses from `Promise.all` must destructure `error` and throw on non-null. Silently falling back to `?? []` makes the admin page render as empty (no subjects) when any query fails, with no user-visible error.
...
### 2026-04-27 — commit 1f3afe2 (fix(quiz): surface orphaned exam sessions with discard-only banner)
- **Files reviewed:** resume-exam-banner.tsx, resume-exam-banner.test.tsx, get-active-exam-session.ts, get-active-exam-session.test.ts, page.tsx
- **CRITICAL:** 0 | **ISSUE:** 1 | **SUGGESTION:** 1 | **GOOD:** 4

**ISSUE resolved (orphaned sessions surfaced) — this commit IS the fix for the ISSUE logged in 4971eda**
The prior session's ISSUE (skip-row silently orphans sessions) is now correctly addressed: `getActiveExamSession` returns `orphanedSessionIds: string[]` on success, and the page renders a discard-only banner for each one. ISSUE pattern is resolved.

**Residual ISSUE — `activeSessionId` inferred type is `string | undefined`**
`activeSessionId` is computed at line 42 before the discriminant is narrowed:
```ts
const activeSessionId = discardOnly ? sessionId : exam.sessionId
```
Because `exam` is destructured as `ActiveExamSession | undefined` (from `exam?: never` in `DiscardOnlyProps`), TypeScript infers `activeSessionId: string | undefined`. This is passed directly to `discardQuiz({ sessionId: activeSessionId })`. If `discardQuiz`'s Zod schema expects `z.string()`, this is a type error that `pnpm check-types` would catch. If the schema uses `z.string().optional()`, the ISSUE is silent. Full-monorepo type-check (not turbo cache) must be clean before merge. The subtitle at line 90 also uses `exam.subjectName` inside a ternary; TypeScript narrows correctly, but the pattern is fragile if extracted to a helper.

**Pattern — discriminated union destructuring loses narrowing in pre-discriminant lines**
When a discriminated union is destructured at the function signature, any computation before the discriminant is checked loses branch-specific type narrowing. TypeScript can still narrow through a ternary on the discriminant, but the inferred type of the whole expression becomes the union of both branches' types. Rule: pre-discriminant computed values derived from one branch's exclusive props should use inline access at narrowed sites (inside `if (!discardOnly)` or inside the ternary's relevant branch), not be pre-computed at the top of the function body.

**Discriminated union design is correct — `never` mutual exclusion pattern**
`exam?: never` / `sessionId?: never` in the two union branches is the idiomatic way to express mutual exclusion in TypeScript. Call sites in `page.tsx` correctly split across the two branches with no mixing. Pattern worth reinforcing.

**Orphan banner placement is correct**
Resumeable exam banners → orphan banners → QuizRecoveryBanner. Correct order: actionable (resume) items before degraded-state (discard-only) items before draft recovery.
