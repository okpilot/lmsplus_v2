...
| Commit message misattributes an overclaim to a PRIOR commit; it lived only in THIS commit's own draft. Detail → [durable-catches](topics/durable-catches.md#2026-09-08-relocated-rows) | 2026-09-08 (8867ccea) | 3 | 2026-09-15 (16382b62) | WATCHING — grep the PRIOR commit for the phrase first. Instance 2: ab310cc9's message described a tech.md deferral as its own change; tech.md is absent from ab310cc9's file stat. Instance 3: the pre-amend message of the RULE 0 carve-out commit claimed `.coderabbit.yaml` already carried the wider form; it was added in that same commit (`git show b611d8aa:.coderabbit.yaml` → no carve-out). Caught pre-push and fixed by amending, so no dangling SHA is cited here. |
...
| Unmeasured numeric claim in commit prose or rule promotion body, never grep'd, wrong every time. Detail → [durable-catches](topics/durable-catches.md#2026-09-08-relocated-rows) | 2026-09-06 (`2e4cf2a3`) | 5 | 2026-09-16 (`1ef2eabf`) | RULE CANDIDATE — `grep -c` / `git log -S` before writing a count. Instance 5: §10 cl.7 extension says "two of them committed" but `git log -S` on all three named quoted strings returns only `1ef2eabf` itself — 0 of 3 have prior artifacts. The committed false quantifier on the branch (`fa43f72c`) is not among the three. CORRECTED in `a4ecd165` — all claims verified clean (2026-09-16). |
...
| Commit message count wrong on the rule sweep being promoted — "65 MUTATION comments" while the actual is 63 (`grep -c` across the five suites returns 23+8+9+10+13=63) | 2026-09-14 (cb481470) | 1 | 2026-09-14 | RESOLVED (10139a66). Both numbers are correct for different questions: 65 = MUTATION claims (grep -c 'MUTATION:'); 63 = lines beginning with the marker (grep -c '// MUTATION:'). Two claims in check-file-size-guard.update.test.mjs sit mid-line after other prose. §7 updated to record the disambiguating grep and the reason both counts are valid. Finding was validated rather than applied — a correct finding would have been wrong. |
| MUTATION comment claims N flags independently redden a test when only 1 does — the others require global git config or different modes not exercised by the test (29b63d89 subdirectory test: claims `--full-tree`/`--no-relative` are also pinned; verified by patching each independently) | 2026-09-14 (29b63d89) | 1 | 2026-09-14 | WATCHING — verify mutation claims by patching a scratch copy |
| False count IN A NEW FILE introduced by the commit fixing false counts — testkit says "three suites that import it" but only 2 suites import it (`grep -rn "from.*testkit"` returns 2 hits) | 2026-09-14 (236a043d) | 2 | 2026-09-15 (65a165e5) | PROMOTED → code-style.md §10 cl.7. Instance 2 (65a165e5) fixed by c5a502ff — "Three independent mechanisms" + "All three verified" now correct; suite 12/12 green. |
| Decision record omits a scope narrowing the FIX COMMIT's own message explicitly discloses — Decision 70 claims `path_instructions` covers "every prose surface" while b944f177's message says chat and commit-message review are no longer covered | 2026-09-15 (b944f177 PR #1284) | 1 | 2026-09-15 | RESOLVED (21a33700) — `.coderabbit.yaml` comment now reads "every file this glob matches"; the retraction is complete across both surfaces |
