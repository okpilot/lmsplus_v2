...
- **Spec task premise incompleteness** — `corpus-codification/tasks.md` task "Delete the `MUTATION:` comment, or generate it" states "Each one is a SECOND copy of a claim already encoded in a `*.mutations.json` entry". PR chore/record-mechanical-lever added THREE `notEncoded` entries proving some claims cannot be encoded (config-file mutations, test-assertion vacuity). The task premise is incomplete; update needed to acknowledge unencodable claims and that `--list` now marks them with `(not encoded)`.

*Last updated: 2026-09-17*

Recent audits (2026-09-14 through 2026-09-17): Decision 66 calibration claims, commit-msg gate mirrors, false-positive lists, memory-only binding audits, test-count inventory checks, waiver-residue references, section-name drift, Decision 74 round-membership, and mutation-encoding premise (2026-09-17, chore/record-mechanical-lever). See git log for detail.
