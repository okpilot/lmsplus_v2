...
- **Mutation-encoding progression (corpus-codification) — ongoing (2026-09-17 through 2026-09-18):** The `.spec-workflow/specs/corpus-codification/tasks.md` task line 201 "Encode the `MUTATION:` comment, or declare it unencodable" documents mutation encoding progress per data file. Task says "Per mutation DATA FILE (`--coverage` reports and aggregates per data file, not per suite), drive the gap to zero". **MILESTONES REACHED 2026-09-18:** `chore/encode-file-size-guard-test-claims` brought `check-file-size-guard.mutations.json` gap to ZERO. NOT the first — `check-prose-claims.mutations.json` was already at 0, as this same bullet records below; the orchestrator stated "first" in error and corrected it in-session. `chore/encode-retracted-phrase-claims` encoded 12 additional `check-retracted-phrase` claims, reducing gap 32→20. Progression: `chore/record-mechanical-lever` (2026-09-17) added `notEncoded` entries; `chore/encode-file-size-guard-cli-claims` (2026-09-18) reduced gap 35→22; `chore/encode-file-size-guard-test-claims` reduced gap 22→0 by encoding 13 claims + rewriting 4 false comments; current branch reduced gap 32→20 for retracted-phrase. Task stays `[ ]` — other data files still carry gaps: `check-prose-claims.json` gap=0 (also done), `check-prose-paths.json` gap=−8 (overencoded), `check-retracted-phrase.json` gap=20, `run-mutations.json` gap=11 (command: `node .claude/hooks/run-mutations.mjs --coverage`). Test comment in retracted-phrase rewritten: "compare paths loosely…" → "decode git's `-z` output…", one false claim fix.

*Last updated: 2026-09-18*

Recent audits (2026-09-14 through 2026-09-18): Decision 66/73/74 calibration and documentation, commit-msg gate mirrors, false-positive lists, memory-only binding audits, test-count inventory checks, waiver-residue references, section-name drift, CR-local round-membership, mutation-encoding premise and gap progression. See git log for detail.
