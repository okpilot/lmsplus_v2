...
- Mutation harness progression: GROUP markers (2026-09-18), `--update-expected` mode (2026-09-19), `--staged` mode + staged-path scoping (2026-09-21, #1325). Hook header documents `--staged` mode, exit codes, and harness semantics; Decision 67 remains accurate. Commits `60037901` (R1) + `3162464a` (R2) + `3f609574` (R2 fixup). Round 3 (CEILING): ci.yml adds `.staged.test.mjs` and `.scope.test.mjs` to mutation harness unit tests in CI. **DRIFT FOUND:** CLAUDE.md §QA-pipeline (line 198) omits hook unit tests running in CI — ambiguity pre-existed, branch worsens it by adding more tests. Recommend clarification: mutation harness + pipeline tests run in CI alongside integration tests and Vitest. No new public decisions; no steering drift (tech.md mentions mechanical guards exist without enumeration, consistent with approach). No other doc edits needed.
- **Guard controls (2026-09-22, P2 guards/controls branch, Decision 79):** ci.yml adds pipeline.repo.test.mjs (pipeline spec's own controls), controls.test.mjs (validates guard controls registry), controls.repo.test.mjs (controls registry's own controls). CLAUDE.md §QA-pipeline now needs the same clarification the prior entry flagged — three NEW unit tests in CI validate the guard system alongside integration tests and Vitest. Edit required: add phrase to line 198 clarifying guard unit tests run in CI.

- **New gate member: deletion-reviewer (2026-09-22, docs/deletion-reviewer):** Seventh round-1 member (sonnet, read-only). Updated `.claude/agents/deletion-reviewer.md` (new), `.claude/rules/agent-deletion-reviewer.md` (new), `.claude/pipeline.json` (agent entry + role), `.claude/pipeline.test.mjs` (EXPECTED_CORE + roles assertion), all command docs (automerge, autonomerge, fullpush, wrapup), agent rule files (learner, code-review, agent-critic, agent-workflow, agent-memory), CLAUDE.md (gate list + numbering), docs/plan.md (round 1 list), docs/decisions.md (Decision 80), .spec-workflow/steering/tech.md (code-review section), .spec-workflow/specs/corpus-codification/tasks.md (size phrasing). All mirrors updated in same commit; no stale "six" references remaining; Agent-memory correctly excludes deletion-reviewer from project memory. **No changes needed** — all edits already applied.

- **CI reviewer on opus (Decision 78, 2026-09-22):** `.github/workflows/claude-review.yml` is a new automated PR reviewer that runs on GitHub Actions. Pattern: When a new CI reviewer/workflow lands, check `.spec-workflow/steering/tech.md` § Code review — it may list reviewers (CodeRabbit, in-session subagents) without mentioning the new one. Decision 78 added to `docs/decisions.md` and `agent-critic.md` § Model tier updated to name it. **DRIFT FOUND:** steering doc `tech.md` line 102-103 omits the new CI reviewer; recommend update to mention `.github/workflows/claude-review.yml` (opus, blocks via dismissable REQUEST_CHANGES, Decision 78) alongside CodeRabbit and in-session subagents.
...
- **Shared diff-parser refactor (2026-09-22, guards/shared-diff-parser):** `.claude/hooks/diff-parse.mjs` extracted — new module handling unified-diff parsing (header/path decoding, added-line extraction, DIFF_ARGS constants). Consumers: `check-test-title-leakage.mjs` and `run-security-auditor.sh` updated to use it. CI: new `.claude/hooks/check-test-title-leakage.repo.test.mjs` (textconv/ext-diff/noprefix/color configs). Spec-only: `.spec-workflow/specs/corpus-codification/tasks.md` P1/P2 marked done, P8 (this work) now active. **No binding doc edits** — internal refactor, no API/architecture change.
- **Secrets-guard branch — false claim correction pattern (2026-09-22):** Branch corrects a FALSE CLAIM ACROSS FOUR FILES that "pre-commit hook blocks `.env*` files". Actual secret protection: `.gitignore` ignores `.env*` (except `.env.example`) + `*.pem`/`*.key`; pushed content scanned by pre-push auditor (fail-closed), GitHub push protection, CodeRabbit. Files updated IN BRANCH: `docs/security.md` §8, `.claude/rules/security.md` rule 8, `CLAUDE.md` NEVER DO line, `.claude/rules/agent-security-auditor.md`. Binding docs audit: `docs/plan.md` (only mentions `.env.local`/`.env.remote` in local dev context — accurate), `docs/decisions.md` (line 63/139 mention "protect .env" re: guard-bash.js preventing display of `.env` files, not preventing commits — accurate), `docs/setup-audit.md` line 41 (says ".gitignore blocks `.env*`" — accurate wording). Steering docs: no false claims found. **NO DOC EDITS REQUIRED** — branch has already updated all binding docs that needed it.
