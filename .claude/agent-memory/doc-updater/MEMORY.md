...
- **Decision 74 documentation (2026-09-17, chore/record-mechanical-lever):** Decision 74 records CR-local's move to round 1 only. This required updates to all binding gate mechanics docs: `docs/plan.md` (round 2+ clause + learner input), `CLAUDE.md` (round 2+ + learner), `.spec-workflow/steering/tech.md` (code-review stage description), `.claude/rules/agent-workflow.md` (round 2+ diagram), `.claude/rules/agent-coderabbit-local.md` (trigger, handling, DO/NEVER), `.claude/rules/agent-learner.md` (learner input), `.claude/agents/learner.md` (mission, inputs, report format), `.claude/commands/crlocal.md` (description + steps). Decision 73's body carries an annotation `*(Round-2+ membership superseded 2026-09-17 by Decision 74: ...)*` in house style (compare other Decision-N annotations in the file — they use parentheses + italics). ALL docs previously checked and VERIFIED IN BRANCH as accurate; no edits needed.
...
- **Mutation-encoding progression (corpus-codification) — ongoing (2026-09-17 through 2026-09-18):** The `.spec-workflow/specs/corpus-codification/tasks.md` task line 201 "Encode the `MUTATION:` comment, or declare it unencodable" documents mutation encoding progress per data file. Task says "Per mutation DATA FILE (`--coverage` reports claim sites and encoded mutations separately per data file, not per suite), drive unencoded claims to zero". **MILESTONES REACHED 2026-09-18:** `chore/validate-mutation-group-refs` eliminated the gap measurement entirely — `--coverage` now prints: claim sites (total `// MUTATION:` comment occurrences in comment lines only), linked (those attached to a `// GROUP:` marker), encoded (mutations in the data file), and dangling ids (GROUP markers naming no mutation, which aborts `--run`). The gap subtraction was `claims − encoded − notEncoded`, arithmetic on non-commensurable units (comment tokens ≠ data rows); six `notEncoded` entries across two data files existed only to zero this false measurement and were deleted. `chore/encode-file-size-guard-test-claims` brought `check-file-size-guard.mutations.json` to zero unencoded claims. `check-prose-claims.mutations.json` was already at 0, `check-prose-paths.json` now at 0 (deletes removed the offsetting entries). Task stays `[ ]` — `check-retracted-phrase.json` and `run-mutations.json` still carry unencoded sites (derive with `node .claude/hooks/run-mutations.mjs --coverage`). Test comment in retracted-phrase rewritten: "compare paths loosely…" → "decode git's `-z` output…", one false claim fix.
...
- **Baseline policy amendment (2026-09-18, fix/db-integration-fixture-isolation):** Code-style.md §1 amended the limits.json baseline policy from "may only shrink" to "SHRINK must be RECORDED; GROWTH is not forbidden but demands a PR-body argument". Mirrors carrying the OLD policy (which claimed failure in EITHER direction): `docs/decisions.md` Decision 65 body (line 1683: "whose recorded size CHANGES in either direction"), `.spec-workflow/steering/structure.md` (line 71: "recorded size CHANGES — in either direction"). Both need updating to reflect the new policy. No update needed in `.coderabbit.yaml` (it mentions caps but not the shrink/growth policy itself). Spec-only cargo (corpus-codification tasks.md) mentions "may only shrink" about `.claude/prose-claims.json`, a separate artifact with its own policy — not a carrier of the limits.json policy.

*Last updated: 2026-09-18*

Recent audits (2026-09-14 through 2026-09-18): Decision 66/73/74 calibration and documentation, commit-msg gate mirrors, false-positive lists, memory-only binding audits, test-count inventory checks, waiver-residue references, section-name drift, CR-local round-membership, mutation-encoding premise and gap progression, GROUP marker linking, baseline-policy mirrors. See git log for detail.
