...
- **Spec task premise incompleteness (corpus-codification) — ongoing (2026-09-17, 2026-09-18):** The `.spec-workflow/specs/corpus-codification/tasks.md` task line 201 "Encode the `MUTATION:` comment, or declare it unencodable" documents mutation encoding progress for the guard suite. Line 125 of this memory noted the task premise was incomplete from `chore/record-mechanical-lever` (2026-09-17) which added `notEncoded` entries. Now branch `chore/encode-file-size-guard-cli-claims` (2026-09-18) encodes 13 additional `MUTATION:` claims from the CLI test suite, reducing `check-file-size-guard.mutations.json` gap from 35→22 (command: `node .claude/hooks/run-mutations.mjs --coverage`). CORRECTED 2026-09-18: the task does NOT assume every claim is encoded or a stale duplicate — it already directs `notEncoded` rows carrying both `claim` and `why`, and already says deleting the comments is not the remedy. The earlier premise here was retracted by PR #1304's rewrite; a finding raised from it was SKIPPED as false (`git grep -nF -- 'SECOND copy' -- :/` exits 1). Re-read the task before citing it. Task stays `[ ]` — the remaining work to zero the gap is a separate PR targeting only `.claude/hooks/check-file-size-guard.test.mjs`.
