...
- **Commit `714da90e` fix(pipeline): give --base mode the escape hatch it documents — IN PROGRESS** (2026-09-14). The fix changes `--base` mode to iterate commits (waivers now commit-scoped), switches `/insights` waiver count from `git log -S` to `git log --grep`, drops false count from Decision 66, and delegates three command lists to `.claude/pipeline.json`. Three documentation sites now have INCOMPLETE descriptions of where the guard runs: CLAUDE.md:252 says "the message is readable nowhere earlier" (true for pre-commit, but misleading now that `--base` reads messages at pre-push time); docs/decisions.md:1765 says guard "blocks at `commit-msg`" (incomplete — it now also blocks at pre-push); .coderabbit.yaml:273 says "blocks the exact VERBATIM case at commit-msg" (incomplete for same reason). Each needs updating to clarify the guard now runs in BOTH commit-msg (authoring) AND pre-push (CI, via `--base` mode).
