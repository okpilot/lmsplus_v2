...
Recent audits (2026-09-14 through 2026-09-16): Decision 66 calibration claims, commit-msg gate mirrors, false-positive lists, memory-only binding audits, test-count inventory checks, and waiver-residue references. See git log for detail. **Current finding: commit `4714a817` detection-pattern change (unpaired `<` or `>` now treated as placeholders) requires `.coderabbit.yaml` line 314 mirror update.** Edit: replace "`<placeholder>`" with "placeholder (paired `<name>` or unpaired `<` or `>`)" per `agent-coderabbit-sync.md` rule on guard pattern re-mirroring.
