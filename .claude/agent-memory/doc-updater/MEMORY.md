...
- **Commit `ab310cc9` fix(pipeline): close a substring fail-open** — Decision 66 paragraph asserts unverified calibration claim "unchanged at 2 of 120" without evidence replay harness was re-run. Decision 66's own preceding prose requires "Re-derive rather than trusting the number: the replay harness is NOT committed, so this is a claim to re-measure before relying on it." The new prose repeats the measurement figure without the required evidence. Recommend: remove the specific figure claim or add evidence of re-measurement.

- **Commit `9a79ec78` fix(rules): make the tech.md deferral real** — Decision 66 no longer restates the "2 of 120" figure (correct), BUT the paragraph still claims "the calibration was re-measured against the fixed guard and did not move" — asserting measurement without evidence, which violates Decision 66's own requirement to "Re-derive rather than trusting the number: the replay harness is NOT committed". The figure is removed but the re-measurement claim survives. Recommend: remove the re-measurement claim entirely OR provide evidence of re-measurement (command run + output).

- **Commit `69d03e69` docs(decisions): stop asserting a calibration a reader cannot check — DISCHARGED** (2026-09-14). The re-measurement assertion is removed. Slice-2 rule is correctly cited at decision.md:1809 (paraphrased, not direct quote). Harness-commit task remains unchecked in corpus-codification tasks.md:109 `- [ ]`. Calibration figure "2 blocks over 120 commits (3 hits)" appears once at decision.md:1775-1776 in the design-measurement section; NOT restated in the final amended paragraph (1807-1810). Phrase "NO calibration figure is restated here" is accurate in scope — the later section defers to slice-2, neither asserts re-measurement nor restates the figure.

- **Commit `714da90e` fix(pipeline): give --base mode the escape hatch it documents — FOLLOW-UP `e97ce5a4` ADDRESSED PROSE** (2026-09-14). The parent commit changed `--base` mode to iterate commits; the follow-up audited Decision 66 and downstream doc mirrors. The "blocks at commit-msg" incomplete claim at docs/decisions.md:1765 was revised to "blocks at `commit-msg` when a value or filename this commit corrected in one corpus file still stands in one or two others. CI re-runs the SAME per-commit check over the branch range (`--base`)" which now accurately describes both stages. CLAUDE.md:252 was already correct ("the message is readable nowhere earlier" describes pre-commit only, and the commit-msg para 254-255 correctly states "CI reads it too, later, via `--base`"). `.coderabbit.yaml:273` (now 270) correctly updated from "blocks ... at commit-msg and again in CI across the branch" to "blocks ... at commit-msg, and CI re-runs that same per-commit check to catch a bypassed hook". **RESIDUE documented (ACCEPTED):** commit 1807→1806 also documents the merge-commit gap that `--no-merges` leaves uncovered — a merge's CONFLICT RESOLUTION can introduce text in neither parent, and CI now never grades it. The commit-msg hook does (merge runs the hook), so this is reachable only via bypassed hook, which is the hole CI exists to backstop.

- **Commit `e97ce5a4` fix(pipeline): skip merge commits in --base — DOCTRINE DRIFT FIXED** (2026-09-14). CR-local round 2: Decision 66 no longer quotes a "18% block rate" calibration figure (replay harness not committed, unverifiable); "THREE mechanisms are NOT pinned" → "the mechanisms that are NOT mutation-pinned are named — not counted" (code-style.md §10 cl.2); `ci.yml` comment "PR-diff run catches ... ACROSS the branch" → "CI re-runs that same per-commit check to catch a bypassed hook" (not cross-branch analysis); `.coderabbit.yaml` comment updated identically. All three documentation sites now agree: the guard is per-commit at `commit-msg`, and CI re-runs it per-commit too, so its value is catching bypassed hooks, not wider analysis. Steering: tech.md correctly defers to Decisions 64/66 for gate mechanics and says "do not count the gates", no drift. Command list fully delegated to `.claude/pipeline.json` per the pattern established in Decision 62. **CLEAN: all doc mirrors aligned, no count-based claims, no cross-branch analysis claims, accepted residue documented.**

- **Commit `236a043d` fix(pipeline): sweep the count I corrected in one place and left in three — CLEAN** (2026-09-14). CR-local round 3. Five findings: (1) "three mechanisms" spelled-out count stayed in three places (corrected guard could not catch spelled-out words, only digits); (2) `.coderabbit.yaml` MUTATION rule was over-broad and would flag its own source; (3) code-style.md §7 had "two claims sit mid-line" count of open set, replaced with derivation command; (4) git harness reached third copy, extracted to testkit.mjs; (5) octopus merges named in accepted residue. Verification: Decision 66 new paragraph "the value detector requires three or more DIGITS" matches `NUM_RE = /(?<![0-9A-Za-z_$#.-])(\d{3,13})(?![0-9A-Za-z_.])/g` (line 90 of check-retracted-phrase.mjs). Only "three mechanisms" survivor is Decision 66 explaining why guard couldn't catch it. `.spec-workflow/specs/corpus-codification/tasks.md:71` uses `wc -l .claude/hooks/check-retracted-phrase*.mjs` derivation command, automatically includes new `.testkit.mjs`. code-style.md §7 now derives "two claims" difference via `diff` command instead of stating count. `.coderabbit.yaml` MUTATION rule restored carve-out referencing code-style.md §7 permission. No steering drift (tech.md makes no claims incompatible with adding mechanical guards). **All doc edits validated against code; no documentation updates needed.**

- **Commit `18448a34` chore(memory): record the fourth doc-updater unpasted-assertion instance — BINDING AUDIT CLEAN** (2026-09-16). The commit increments the learner tracker row "doc-updater makes assertions without pasting verification command" from count=3 to count=4. Audit scope: search binding docs for any literal citation of count=3 for this row. Commands run: `grep -rn "doc-updater\|unpasted\|pasting verification" docs/ .claude/rules/ CLAUDE.md .spec-workflow/steering/` (no hits for count=3), `grep -rn "137\|19332\|19969\|147"` (no binding doc quotes), `for file in docs/*.md .claude/rules/*.md CLAUDE.md .spec-workflow/steering/*.md; do if [ -f "$file" ]; then grep -n "3.*doc-updater\|doc-updater.*3" "$file"; fi; done` (no count quotes found). Tracker size: `wc -lc .claude/agent-memory/learner/MEMORY.md` = 147 lines, 20167 bytes (both within 200-line / 25 KB injection caps). **NO DOC EDIT NEEDED.**
...
- **Commit `f41d7ef4` chore(memory): carry the 4c335a60 cycle's agent-memory deltas — BINDING AUDIT CLEAN** (2026-09-16). Commit touches ONLY `.claude/agent-memory/**` under the docs-only exemption. Changes: (1) code-reviewer added 1 new tracking row for the `4c335a60` cycle; (2) doc-updater added audit notes confirming no binding doc edits needed for `4c335a60`; (3) learner incremented one row count from 3→4 ("commit message claims file 'already carries/has X' when X landed same commit"); (4) semantic-reviewer incremented one row count from 3→4 (misattribution pattern) and expanded explanation. Audit scope: search binding docs for any citation to the old count values (3) for these rows. Commands run: `grep -rn "already carries\|landed same commit" docs/ .claude/rules/ CLAUDE.md` (1 hit: code-style.md line 1030, unrelated pattern), `grep -rn "misattributes an overclaim\|already carries.*when.*same" docs/ .claude/rules/ CLAUDE.md` (no hits). Memory file caps: `wc -lc .claude/agent-memory/learner/MEMORY.md` = 151 lines, 20286 bytes (both within 200-line / 25 KB caps). **NO BINDING DOC EDITS NEEDED.** The commit carries forward agent-memory deltas only; no binding docs are affected.

- **Commit `63cf80c0` test(hooks): pin the ./-prefix strip in the prose-paths normaliser — BINDING AUDIT CLEAN** (2026-09-16). Commit adds 1 test case to `.claude/hooks/check-prose-paths.test.mjs` (26 → 27 tests) and carries agent-memory deltas (code-reviewer, doc-updater, implementation-critic/topics/commit-notes.md). Audit scope: no binding docs state test counts for prose-paths suites (check-prose-paths.test.mjs or .repo variant). Verified: `grep -rn "27\|49\|48\|26\|22" docs/ .claude/rules/ CLAUDE.md .spec-workflow/ --include='*.md'` matched 0 test-count references. Decision 71 and code-style.md §9 explicitly defer to `measure-prose-paths.mjs` output rather than stating numbers. `.spec-workflow/specs/corpus-codification/tasks.md` line 105 correctly states "no figure is stated here" and cites the derivation command. No coverage description changes. `.claude/pipeline.json` and `lefthook.yml` list "prose-paths" as a command name with no test-count assertions. Memory file caps: `wc -lc .claude/agent-memory/code-reviewer/MEMORY.md` = 38 lines (within 200-line cap), `wc -lc .claude/agent-memory/doc-updater/MEMORY.md` = before-edit baseline verified under current span. **NO BINDING DOC EDITS NEEDED.** The commit is a test-only addition; no inventory counts in binding docs require updating.
