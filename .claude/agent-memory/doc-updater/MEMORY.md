...
- **Mutation-encoding progression (corpus-codification) — ongoing (2026-09-17 through 2026-09-18):** The `.spec-workflow/specs/corpus-codification/tasks.md` task line 201 "Encode the `MUTATION:` comment, or declare it unencodable" documents mutation encoding progress per data file. Task says "Per mutation DATA FILE (`--coverage` reports and aggregates per data file, not per suite), drive the gap to zero". **MILESTONE REACHED 2026-09-18:** `chore/encode-file-size-guard-test-claims` brought `check-file-size-guard.mutations.json` gap to ZERO — the FIRST data file in the repo to reach this state.** Progression: `chore/record-mechanical-lever` (2026-09-17) added `notEncoded` entries; `chore/encode-file-size-guard-cli-claims` (2026-09-18) reduced gap 35→22; current branch (2026-09-18) reduced gap 22→0 by encoding 13 additional claims and rewriting 4 false comments. Task stays `[ ]` — other data files still carry gaps: `check-prose-claims.json` gap=0 (also done), `check-prose-paths.json` gap=−8 (overencoded), `check-retracted-phrase.json` gap=32, `run-mutations.json` gap=11 (command: `node .claude/hooks/run-mutations.mjs --coverage`). Test retitle: "an emptied baseline…" → "an over-limit file with no baseline row is reported…". Comments corrected: lines 265–268, 274–277, 283–291, 426–430 in test file (four rewrites including one false claim fix).
