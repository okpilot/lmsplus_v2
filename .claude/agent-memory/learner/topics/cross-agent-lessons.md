...
### MEMORY.md tracker overflow, compacted 2026-09-15 (full row text moved here)
...
- Agent terminal message self-referential with no prior report body delivered | 6 | 2026-09-14 | PROMOTED → agent-workflow.md § Delegation Protocol ("terminal message is the only channel back; restate everything"). First post-promotion cycle (9b06fa56) came back clean — all four agents self-contained. One cycle is weak evidence; falsified by the next report body that goes missing while the terminal says "as reported above".

### docs/recover-question-images-decision branch, 2026-09-15 (2 new WATCHING rows, full detail)

- **Orchestrator excludes a known-drift-prone rule-mirror from a sweep on an unverified runtime-access claim (count=1).** On `docs/recover-question-images-decision` commit `7afd5750`, the orchestrator's mirror sweep for Decision 69 skipped `.claude/agents/security-auditor.md` — the exact enforcer `agent-learner.md § Downstream-enforcer sync` names as the site that gets forgotten, and the one row-901-archive PROMOTED family already covers by INSTRUMENT (grep scope, under-applied output). This instance is a different mechanism: the orchestrator REASONED its way out, stating the auditor "has Read access to docs/security.md" so no separate mirror was needed. That premise is false at runtime: `run-security-auditor.sh:82` cats only the auditor's own agent definition plus the diff into the LLM call; `--allowedTools "Read"` grants a capability without feeding the content. semantic-reviewer caught it the same cycle. Log and watch — on a 2nd instance (any orchestrator reasoning, rather than grepping, its way out of a mirror site named in § Downstream-enforcer sync), propose a Finding-Validation-style addendum: before excluding a mirror site from a sweep on the grounds that a downstream agent 'already sees' the source, read that agent's OWN driver script/dispatch prompt and confirm the content is actually fed, not merely readable in principle.
...
## Durable-knowledge bullets relocated 2026-09-18 (learner MEMORY.md byte cap, #1297)
...
## Durable-knowledge bullets relocated 2026-09-18 (batch 6, from MEMORY.md lines 151-153)
