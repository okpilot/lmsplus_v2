...
| Fix commit correcting §10 violations introduces fresh §10 | 31 | 2026-09-08 | RULE CANDIDATE (31 — per-branch unit). 6TH BRANCH `chore/evidence-gate` (`4ec5b894`..`d728e389`, `check-commit-claims.mjs`) — 10 same-branch instances, SOME caught pre-commit across 5 impl-critic rounds and the rest by post-commit cycles that each produced a follow-up commit, not across separate commits; the branch's own final message: "Four of them are mine." Text exists (§10 cl.3), enforcement gap. STAYS OPEN — full mapping + dominant-pattern + mechanical-checkability answer in cross-agent-lessons.md §"Branch chore/evidence-gate" |
| Rules-file bullet closes an enumeration of a structurally OPEN set — falsified on each new discovery | 9 | 2026-09-08 | RULE CANDIDATE (9) — text exists (§10 cl.2). 9th: `371bfe49`, `.claude/commands/automerge.md` said "the two mechanical guards" in the very sentence that branch edited to add a THIRD; found by CR-local, not by any internal reviewer. 8th: `ee0186d9`, six files asserted "exactly TWO" accepted defer justifications, falsified by a third; named not counted in the fix. Chain in topic file |
...
| Mutation-check executed but doesn't falsify the claim — unisolated or untargeted mutation | 3 | 2026-09-08 | RULE CANDIDATE (3), THRESHOLD REACHED → addendum to test-writer.md § Mutation-check (per-branch coverage, not just per-file). `chore/pipeline-spec-as-data` (`a8f92eab`,`c7686957`); 3rd `chore/evidence-gate` `d728e389` (REMEDY.ambiguous branch never reached by any test). Draft text in cross-agent-lessons.md |
...
| Rules-file claim true in its hunk, false vs another section/mirror/arithmetic | 15 | 2026-09-08 | RULE CANDIDATE (15) — text still UNWRITTEN in §10 (new cl.6). 15th, FIRST in a COMMIT MESSAGE not a rules/doc file: `chore/evidence-gate` drafting `33c8ff79`, one measurement stated 3 inconsistent ways in one draft, caught pre-commit. Detail: cross-agent-lessons.md §"Branch chore/evidence-gate" |
