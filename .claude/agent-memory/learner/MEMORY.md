...
| CR local suggests wrong fix rejected on a documented architectural decision | 3 | 2026-06-24 | RULE CANDIDATE (3) — 3rd: §7 FK-order gate removal suggested during Phase 4 cleanup; prior 2: `correct_count` data-model suggestions. [full → topics/tracker-archive.md] |
...
| Partial schema mirror across sibling validation layers (grader tightens, save/load schemas drift looser) | 3 | 2026-06-24 | RULE CANDIDATE (3) — (1) 7259b1ba: save-draft mirrored array-cap+dedup but not text bounds; (2) 01236624: text bounds mirrored to SaveDraftInput (fix commit). (3) e3f3f800/3f569d2f: `toFeedbackEntry` DB-draft-load validator missed `length > 0` that the other 3 dialog_fill validators had — `[].every()` vacuously true, empty blanks loaded as valid; Opus semantic-reviewer ISSUE, fixed in 3f569d2f. 3rd occurrence across 2 distinct commits (different validator family, different mechanism: prior=text-bounds; this=presence constraint). Proposed rule target: code-style.md §5 or §7. [full → topics/tracker-archive.md] |
