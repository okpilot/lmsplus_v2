...
| Corrected claim partially retracted — old wording persists elsewhere | 12 | 2026-09-15 | RULE CANDIDATE (12) — +1 (16382b62): RULE 0 carve-out wider form existed in .coderabbit.yaml but not in CLAUDE.md banner or 49 agent files; impl-critic + CR-local caught it, all 50 surfaces widened. Mirror-lag direction reversed (mirror was MORE correct than canonical). +1 (`21a33700`): "applies to every prose surface" retracted from decisions.md in `b812d107` but survived in .coderabbit.yaml comment line 268; found by orchestrator manual grep, not by hook. Detail: topic |
| Mirror-sync grep misses a mirror on the wrong axis | 2 | 2026-08-24 | RULE CANDIDATE (2) |
| Subagent asserts a verification/write it did not perform — evidence invented, conclusion mostly true | 17 | 2026-09-15 | PROMOTED → agent-workflow.md § Finding Validation. b0ea0d58: doc-updater reported "CLAUDE.md — has RULE 0 banner ✓" (false). `b812d107`: doc-updater justified "no changes needed" by claiming commit "updates timestamp to 2026-09-15" — `git show b812d107 -- docs/decisions.md` contains no such footer. Detail: topic file |
| Rules-file claim true in its hunk, false vs another section/mirror/arithmetic | 29 | 2026-09-15 | RULE CANDIDATE (29) — +1 (c7805f83): Decision 70 "No hook measures prose" contradicted Decision 68. +1 (`b944f177`): code-reviewer called the 8-commit count "CORRECT" using range `b0ea0d58^..b944f177` (=9 commits), not the actual claim range `b0ea0d58^..68b03052`. Detail: topic file |
...
| MUTATION: comment overclaims which mechanisms the test pins | 7 | 2026-09-15 | RULE CANDIDATE (7). `b944f177`: MUTATION fixture dropped internal blank line from 16382b62's field; comment named length-cap assertion, blank-line assertion fired instead. Detail: cross-agent-lessons.md |
...
| Reviewer reinterprets a false claim into a nearby true one — reports clean | 2 | 2026-09-15 | RULE CANDIDATE (2) — b50353d2: semantic-reviewer found a true reading of false .upload(-grep claim. `b944f177`: code-reviewer judged "covers every prose surface" as "reasonable in context" — semantic-reviewer same cycle caught it false, confirmed false, retracted. |
...
| doc-updater makes assertions without pasting verification command despite dispatch requiring pasted output | 2 | 2026-09-15 | RULE CANDIDATE (2) — b0ea0d58: no artifact pasted for CLAUDE.md banner claim. `b812d107`: false "commit updates timestamp" assertion with no `git show` output — existing rule covers COUNTs; non-count qualitative assertions are outside current scope. |
...
| `check-retracted-phrase.mjs` trigger gap — pure deletion without replacement in hunk bypasses hook even when survivor exists | 1 | 2026-09-15 | WATCHING — `21a33700`: "applies to every prose surface" removed from decisions.md in `b812d107` without replacement; hook didn't fire; phrase survived in .coderabbit.yaml line 268; found by orchestrator manual §10 cl.3 grep. Distinct from #1285 (matching gap). |
| Config field content silently disables the entire config when schema-enforced maxLength exceeded — no error, no diagnostic | 1 | 2026-09-15 | WATCHING — `b0ea0d58` `tone_instructions` grew to 928 chars vs schema maxLength:250; CR rejected whole config, used defaults for 8 commits; all path_instructions inert. The "RULE 0 — NO PROSE" commit disabled the reviewer config by being too much prose. |
