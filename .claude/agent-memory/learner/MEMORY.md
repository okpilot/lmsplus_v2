...
| Correct advice with invented rationale (correct conclusion shields false WHY) | 5 | 2026-09-14 | RULE CANDIDATE (5). 5th: `feat/mutation-harness` CI comment claimed "re-grade what merge-base already proved"; worktree is built from HEAD so merge-base proves nothing. Propose: agent-critic.md verifies stated mechanism independently. Detail: topic file |
...
| Corrected claim partially retracted — old wording persists elsewhere | 6 | 2026-09-14 | RULE CANDIDATE (6) — cl.3 grep required but not performed on the full set. Detail: topic file |
...
| MUTATION: comment overclaims which mechanisms the test pins — sub-claim is unreachable; the comment is §10 prose against the test artifact | 5 | 2026-09-14 | RULE CANDIDATE (5) — `ff562c9f`/`ab310cc9`/`feat/mutation-harness`. New distinct mechanism (5th): comment names a REACHABLE mechanism but describes the WRONG FAILURE MODE ("passes silently" where code throws TypeError). §7 "A MUTATION: Comment Is a Prose Claim" covers both; consider extending to name the failure-mode variant explicitly. |
...
| Mutation-harness anchor orphaned by cosmetic reformat — exits 2 grading nothing | 1 | 2026-09-14 | WATCHING — `feat/mutation-harness` impl-critic: 3 mutation anchors orphaned at count=0 after reformat in the same commit; run would abort at exit 2 having graded nothing. Distinct from row 40 (which silently passes); this fails loudly. Log and watch. |
| Agent terminal message self-referential with no prior report body delivered | 1 | 2026-09-14 | WATCHING — code-reviewer reported "Memory updated. The review stands as reported above" with no preceding report body — delegation failure requiring re-ask. Candidate addition to `agent-workflow.md § Delegation Protocol`: "a terminal message must be self-contained." Log and watch. |
