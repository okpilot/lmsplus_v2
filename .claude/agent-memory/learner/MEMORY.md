...
| doc-updater proposal echoes an example from its OWN dispatch prompt as a literal citation | 5 | 2026-09-18 | RULE CANDIDATE (5). Latest (chore/encode-file-size-guard-cli-claims): memory-file variant — agent cited "line 125 of this agent's memory" as evidence for a claim about the current artifact; memory predated a task rewrite; `git grep` found no match. Same root: agent treats own context (memory OR dispatch prompt) as factual over grepping the artifact. Detail: cross-agent-lessons.md |
...
| MUTATION: comment overclaims which mechanisms the test pins | 8 | 2026-09-18 | PROMOTED → code-style.md §7 § "A `MUTATION:` Comment Is a Prose Claim". Rule text already present. Post-promotion recurrence (chore/encode-file-size-guard-cli-claims): 6 of 13 encoded entries needed wider expectRed than their source MUTATION: comment implied — hand-written comments systematically overstated specificity. Detail: cross-agent-lessons.md |
...
| Adding tests to mutation harness invalidates pre-existing exact-set `expectRed` specs | 2 | 2026-09-17 | RULE CANDIDATE (2). feat/prose-path-guard, then chore/encode-file-size-guard-claims — the coverage-gap test widened THREE run-mutations entries, one of them (`option-no-consume`) for the SECOND time. The interim "did NOT recur" was FALSE: it rested on a `--guard check-file-size-guard` run, which never reads `run-mutations.mutations.json`. A scoped run cannot clear a repo-wide claim. Any new test asserting a guard's OWN output joins every expectRed set for that guard — run the FULL harness before claiming no cascade. Non-recurrence chore/encode-file-size-guard-cli-claims (2026-09-18): test-writer added no new tests; full 232/232 harness green — first non-recurrence backed by the full run, not a scoped run. |
