...
| Fix commit correcting §10 violations introduces fresh §10 | 47 | 2026-09-15 | RULE CANDIDATE (47) — +1 (efe89098): fixing {org_id}/{filename} introduced "two different first-segment schemes" (there are three). 5th fix-commit on this branch introducing a fresh violation. Detail: topic file |
...
| Corrected claim partially retracted — old wording persists elsewhere | 10 | 2026-09-15 | RULE CANDIDATE (10) — +1: cloud CR on 08e1a6bb: suppression-12 expiry trigger added to security-auditor.md in 6753ad76 not propagated to .coderabbit.yaml. New-addition form, same mirror-lag mechanism. Detail: topic |
...
| Rules-file claim true in its hunk, false vs another section/mirror/arithmetic | 27 | 2026-09-15 | RULE CANDIDATE (27) — +2 this cycle: (c) cloud CR on 08e1a6bb: database.md {org_id}/{filename} stated as universal, true of SA only; (d) code-reviewer b50353d2: .upload(-anchored grep claim false — grep returns the file. Both in WHY clauses. Detail: topic file |
...
- `docs/recover-question-images-decision`: rows 40/42/62/66/69/92 + new reinterpret-row all fired on this branch. 5 consecutive false claims in one paragraph (database.md bucket section), every one in a WHY clause — each passed the check run, failed one not run. Semantic-reviewer defended claim 5 by reinterpreting it rather than falsifying it.
