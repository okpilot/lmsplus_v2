...
| MUTATION: comment overclaims which mechanisms the test pins | 8 | 2026-09-18 | PROMOTED → code-style.md §7 § "A `MUTATION:` Comment Is a Prose Claim". Rule text already present. Post-promotion recurrence TWICE: 6/13 entries (cli-claims) then 9/17 (test-claims, 2026-09-18) needed wider expectRed than their source comment implied. Rising rate; rule text present, so the gap is hand-authoring discipline. Next lever is a mechanical enforcer, not more prose. Detail: cross-agent-lessons.md |
...
| Adding tests to mutation harness invalidates pre-existing exact-set `expectRed` specs | 2 | 2026-09-17 | RULE CANDIDATE (2). Any new test asserting a guard's OWN output joins every expectRed set for that guard — run the FULL harness before claiming no cascade. Detail: cross-agent-lessons.md "expectRed cascade". Non-recurrence x2, both backed by a FULL run (a scoped run cannot clear it): cli-claims 232/232, test-claims 248/248 — no output-asserting test added either time. |
