...
| Rule-promotion sweep recorded closed/complete, later found incomplete | 3 | 2026-09-14 | RULE CANDIDATE (3) — Round 3: THREE mechanisms count, 1 of 4 fixed, sweep declared complete. agent-learner.md needs a re-derivable RECORD (row 688, topic file) |
...
| Verification gate's pass condition is empty result — fails open on malformed input | 5 | 2026-09-09 | RULE CANDIDATE (5), THRESHOLD-CLEARED. Detail: cross-agent-lessons.md |
...
| Correct advice with invented rationale (correct conclusion shields false WHY) | 4 | 2026-09-07 | RULE CANDIDATE (4). Propose: agent-critic.md verifies stated mechanism independently. Detail: topic file |
...
| doc-updater proposal echoes an illustrative example from its OWN dispatch prompt as a literal citation | 3 | 2026-09-14 | RULE CANDIDATE (3) — 3rd: after naming AND after rule promotion, doc-updater fabricated "five file-size-guard suites" + stale quote. Naming is insufficient; artifact-check (pasted grep) must be in agent DEFINITION. Propose: agent-doc-updater.md § DO extend pasted-output requirement from counts to ALL code-body citations. Distinct from row 68. |
...
| MUTATION: comment overclaims which mechanisms the test pins — sub-claim is unreachable; the comment is §10 prose against the test artifact | 4 | 2026-09-14 | RULE CANDIDATE (4) — `ff562c9f`/`ab310cc9`. Writing "MUTATION: removes X and Y" doesn't make Y reachable. Propose §7 addendum: a MUTATION: line is a prose claim, verify by reverting ONLY that mechanism. |
