...
| Pre-existing file-size violation surfaced when commit adds lines to over-limit file | 8 | 2026-06-27 | RULE CANDIDATE — file at/over limit must be split in same additive commit. 8th: Discovery UI (599b8bc9) worsened quiz-config-form.tsx (151→169, cap 150) + types.ts (246→252, cap 200), both pre-over-cap; extracted DiscoveryModePanel+StartButton+session-types.ts in cc5b7409. [full → topics/tracker-archive.md] |
...
- **Study Mode CR-local + fix (8c5d72e5+d8eae99c):** 2 new WATCHING (rows 465–466): error-posture change missed sibling integration test (impl-critic caught); unit test mock fabricates expected value (vacuous). POSITIVE: §7 lifecycle test + non-vacuous integration assertion applied on first pass.
- **Study Mode round-2 refactor + fix (cb150e26+17a32b60):** 1 new WATCHING (row 462): nesting depth 4 in fixture helper on restore-branch add. database.md footer count→2 RULE CANDIDATE. POSITIVE: semantic-reviewer 6 GOOD; sibling hook architecture applied first pass.
- **Study Mode Discovery UI (599b8bc9+cc5b7409):** file-size BLOCKING count 7→8 (quiz-config-form.tsx + types.ts both pre-over-cap, worsened then extracted in follow-up commit). 1 new WATCHING (row 469, archive): structural component shape mismatch in slot-reuse — plan-critic caught self-contained card stack about to be nested in fragment slot → double-border + trapped Start button. POSITIVE: orchestrator's own test title `forwards X to Component` tripped §7 impl-leakage hook (mechanical gate fires for all authors, not only agents); corrected before commit.
- **Study Mode exam-oracle PR-level sweep (feat/study-mode-mc):** All per-commit agents CLEAN (impl-critic ×5 + all post-commit). PR-level sweep found cross-surface oracle: shared MC pool + exam-exposed question IDs → study RPC = mid-exam answer oracle. Fix: active-exam-session deny-by-default guard (mig 117 mirror). Row 384 count→2 RULE CANDIDATE. 1 new WATCHING (row 468): per-commit review structurally blind to cross-surface interactions. POSITIVE: impl-critic deny-by-default suggestion applied; PR-level sweep working as designed.
