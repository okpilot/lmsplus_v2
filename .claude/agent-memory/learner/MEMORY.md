...
| CR (local OR cloud) fabricates a critical/major finding describing a code construct not present in source (hallucinated duplicate declaration, phantom import) | 2 | 2026-08-07 | PROMOTED → agent-coderabbit-local.md Common Pitfall #8 (verify claimed construct against source/tsc; broadened to cloud CR, mirroring pitfall #7). [full → archive row 585] |
...
| Code comment/JSDoc asserts DB/RPC guard behavior not verified against latest migration body (learner synthesis; mirrors impl-critic's own count=3 tracker) | 3 | 2026-08-07 | PROMOTED → code-style.md §10 (archive row 512) — corroborated same cycle by a distinct semantic-reviewer finding (replay-id used as teardown target). |
