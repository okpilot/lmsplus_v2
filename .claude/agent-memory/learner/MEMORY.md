...
| Plan prose states unverified content-item count that | 3 | 2026-09-08 | RULE CANDIDATE (3) → §10 cl.2 addendum needed (counts are enumerations in disguise — "280 lines" and "three command files" wrong within minutes of write: `chore/pipeline-spec-as-data`, item 5). Surface extends beyond plan.md (row 547) |
| Fix commit correcting §10 violations introduces fresh §10 | 29 | 2026-09-08 | RULE CANDIDATE (29) — text exists (§10 cl.3); enforcement gap. 4th arc: `7eaef31a` fixed one claim, left same phrase 21L below. Detail in topic file |
| Rules-file bullet closes an enumeration of a structurally OPEN set — falsified on each new discovery | 6 | 2026-09-08 | RULE CANDIDATE (6) — text exists (§10 cl.2). cloud-CR on `4cbaba0a`: "Three policy contradictions" hand-typed count over OPEN set. +2 prior `chore/pipeline-spec-as-data`. Chain in topic file |
...
| Subagent asserts a verification/write it did not perform — evidence invented, conclusion mostly true | 11 | 2026-09-06 | PROMOTED → agent-workflow.md § Finding Validation (`b177a3d2`). 11 instances through `bb82cb7b`; no 12th on f0eef243/12bc77f5 (execution-based, opposite pattern). Detail in topic file |
| Rules-file claim true in its hunk, false vs another section/mirror/arithmetic | 14 | 2026-09-08 | RULE CANDIDATE (14) (row 655) — +1 `chore/pipeline-spec-as-data` `807b6658`: semantic-reviewer MEMORY row asserted present-tense "test stays green" falsified by same staged diff. Still unwritten in §10 text — close the gap |
...
| Verification evidence answers a different proposition than the finding's claim — real check, wrong question | 2 | 2026-09-07 | RULE CANDIDATE (2) → agent-workflow.md § Finding Validation: state the proposition before treating command+output as validation. `43b1b9a5` (semantic-reviewer), `93df2283` (doc-updater). Distinct from rows 663/638 |
...
| Consistency check verifies A against B with no independent anchor — co-removing from both passes clean | 3 | 2026-09-08 | RULE CANDIDATE (count>=2 met 2026-09-08; rule TEXT drafted, NOT yet written). Routed to the rules PR, which owns the code-style.md surface. Do NOT mark PROMOTED until that heading exists in code-style.md §7 — verify with grep, not from this row. `4cbaba0a` (cloud CR: security-auditor deleted from both, 76 passed); `7eaef31a` (non-agent lefthook cmd deleted from spec+file); `807b6658` (fix iterated spec.hooks — spec defines its own coverage). Sweep: verify pipeline.test.mjs anchor is independent |
