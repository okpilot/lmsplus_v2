...
| Fix commit correcting §10 violations introduces fresh §10 | 46 | 2026-09-15 | RULE CANDIDATE (46) — finding (c): Decision 69 paragraph received its 4th §10 correction on this branch (§10 cl.2 qualifiers on the open-set enumeration of mirror locations). Each prior correction introduced a fresh violation. Detail: topic file |
...
| Rules-file claim true in its hunk, false vs another section/mirror/arithmetic | 25 | 2026-09-15 | RULE CANDIDATE (25) — cl.6 taken by SHA-citation gate; propose cl.8. +2 instances (cloud CR, e5344f7c): (a) security.md §13 cited migration 20260410000009 as proving public=true on prod — migration is a no-op INSERT ON CONFLICT that only establishes the flag in fresh envs; prod was manually configured; verdict was right, evidence was wrong; (b) tasks.md R0b-4 called a YAML block-scalar clause "byte-identical" — folding inserts newlines+indentation, so the claim was self-refuting within its own sentence. Detail: topic file |
...
- `docs/recover-question-images-decision`: 5 DISTINCT already-tracked patterns (rows 40/42/66/69/92) fired on this branch. Row 60 (PROMOTED: correct conclusion shields false WHY) also recurred — adding it to the promoted-but-not-yet-self-enforcing set alongside row 92. All instances "prose ABOUT a correct code change was itself wrong." CO-OCCURRENCE signal: documentation-fixup commits are where §10 defects concentrate, confirming row 53's cluster finding.
- POSITIVE (6753ad76): all four core post-commit agents CLEAN on a security-path suppression edit. CR-local ran M=3 rounds (security-path floor), round 3 applied a cheap fix, round 4 (ceiling) returned 2 trivial PROCESS findings (run red-team + e2e:redteam + coderabbit-sync) and zero apply-worthy code edits. NB: this bullet originally asserted round 4 was clean BEFORE round 4 had run — corrected after execution; the learner pass had inferred a result it could not have observed. First fully-clean full cycle on this branch after a sequence of §10-intensive commits.
