...
| Correct advice with invented rationale (correct conclusion shields false WHY) | 6 | 2026-09-14 | PROMOTED → agent-workflow.md § Finding Validation (new claim shape: "already covered by Y — open Y and verify"). Detail: topic file |
...
| Mutation-harness anchor orphaned by cosmetic reformat — exits 2 grading nothing | 2 | 2026-09-14 | PROMOTED → agents/test-writer.md § Mutation-check (write anchors AFTER a format pass). |
...
- POSITIVE (`9b06fa56`, terminal-message rule first post-promotion cycle): all four agents returned self-contained reports; preceding cycles lost five report bodies across four agents. One cycle is weak evidence — not yet proof. Falsified by the next cycle where a report body goes missing and the terminal says "as reported above".
