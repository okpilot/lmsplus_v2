...
| Pre-existing file-size violation surfaced when commit adds lines to over-limit file | 6 | 2026-06-24 | RULE CANDIDATE — file at/over limit must be split in same additive commit. 6th: draft.ts (SA) grew 134→148 lines as Zod schema (SaveDraftInput) added 14 lines — fix: extract schema to draft-schema.ts sibling; same extraction pattern as load-draft-helpers.ts earlier this session. [full → topics/tracker-archive.md] |
...
| Partial schema mirror across sibling validation layers (grader tightens, save/load schemas drift looser) | 2 | 2026-06-24 | RULE CANDIDATE — VFR RT Phase 3 CR-local rounds 3→4: (1) grader text bounds tightened; save-draft array-cap+dedup mirrored in r3/r4 but text bounds not; (2) extraction-fix commit mirrored text bounds to SaveDraftInput. Distinct from the broader "partial fix / sibling file group" row (count=11): this sub-type is specifically about multi-layer schema parity (grade→save→load). Proposed rule: "when tightening a Server Action input schema OR grader validator, audit sibling schemas that produce/consume the same payload (save/load/grade) for parity in the same commit." [full → topics/tracker-archive.md] |
...
- **VFR RT Phase 3 (dbcd3c9e/c9573351):** semantic-reviewer ISSUE (mobile submit button invisible for non-MC variants, fixed); code-reviewer 4 WARNINGS (3 fn-length, 1 missing co-located test — test-writer resolved); test-writer CLEAN + added buildAnswerHandlers/check-non-mc-answer-helpers/quiz-main-panel/use-answer-handler tests; doc-updater CLEAN; red-team 1 advisory gap (Vector EM deferred → #983). 2 new count=1 WATCHING: (1) responsive UI component wires submit/action button desktop-only via `hidden md:block` and the mobile footer button is variant-specific — non-MC variants have no mobile submit path; (2) test-writer impl-leakage hook catch: generated `it()` title containing `maps is_correct→isCorrect` flagged by `check-test-title-leakage.mjs` (distinct from the tsc-failure class). Commit-msg row incremented count=1→2 (RULE CANDIDATE, see live table).
- **VFR RT Phase 3 CR-local cycle (5a51345b/e87c75ff):** code-reviewer CLEAN; semantic-reviewer Opus CLEAN (1 SUGGESTION applied — load/rehydrate empty-string guard asymmetry); doc-updater CLEAN; test-writer +3 rejection-path tests (genuine gaps); red-team COVERED; impl-critic APPROVED. 2 count=1 WATCHING: (1) CR-local catches type-soundness/runtime-shape holes Opus critics passed (3 findings on this branch = 1 occurrence); (2) load/save validation asymmetry — load path accepts empty inputs, rehydrate path rejects. No live-table row incremented. POSITIVE SIGNAL: all internal agents clean post CR-local fixes.
- **VFR RT Phase 3 rounds 3–4 + extraction-fix (56678b99/7259b1ba/01236624):** round-3 fleet fully clean; round-4 fleet: code-reviewer 1 BLOCKING (draft.ts 134→148 lines, SA cap exceeded — file-size row count 5→6), semantic-reviewer 1 ISSUE (save-draft text bounds looser than grader — schema-mirror RULE CANDIDATE count=1→2); extraction-fix commit resolved both (draft-schema.ts split + text-bound parity). Fleet after extraction: all agents clean. POSITIVE SIGNAL: CR-local 4-fix-commit ceiling triggered and stopped the loop correctly — ceiling is working as designed. 1 new RULE CANDIDATE: partial schema mirror across sibling validation layers (count=2).
