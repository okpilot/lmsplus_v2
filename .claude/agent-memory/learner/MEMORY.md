...
| Fix commit correcting §10 violations introduces fresh §10 | 28 | 2026-09-02 | RULE CANDIDATE (28) → text exists (§10 cl.3); enforcement gap. 6th branch (chore/promote-agent-selfreport-rule): the row-663-promotion cycle itself shipped 2 fresh false claims + 2 more caught pre-commit (row 604 instance 24, topic file) |
...
| Subagent asserts a verification/write it did not perform — evidence invented, conclusion mostly true | 7 | 2026-09-02 | PROMOTED → agent-workflow.md § Finding Validation (`b177a3d2`, row 663). RECONCILED 6→7 at the rebase of this branch onto the squash-merged `12e79f5d` (2026-09-06): master and this branch each took the row 5→6 from a DIFFERENT doc-updater instance — master's is the fabricated "(6 total)" rules-file count on `dcad1d21`, this branch's is the stale footer line numbers `:746`/`:32` (true `:833`/`:110`) on `c787b0d2` — and `dcad1d21` is not an ancestor of this branch's base `20763be0`, so neither pass saw the other and the two +1s are independent. Earlier RECONCILED 7→6 (2026-09-02), unrelated: the claimed 7th instance (doc-updater, `18757ddf`) was a FALSE ATTRIBUTION BY THE ORCHESTRATOR, not an agent failure: doc-updater's write DID land, committed as `1c835391` at 16:05:43. The orchestrator checked `git diff`/`git status` against HEAD at 16:06 — HEAD already contained the commit, so "byte-identical to HEAD" meant COMMITTED, not never-written. `git log -1 -- <path>` shows it immediately, and row 663's own promoted remedy lists that command; two of its four artifact checks were run and the decisive one skipped. Instances span >=4 agent types: doc-updater (row-663's 2nd, 3rd + `dcad1d21`), learner (`560bf0ae`), code-reviewer (`20a14793`), one unnamed — so a doc-updater-only rule under-covers. NEW LESSON: an artifact check that picks the WRONG artifact manufactures a false instance of the very row it is checking |
