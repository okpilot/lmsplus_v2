...
| Claim-correction commit updates a count but leaves its | 9 | 2026-09-10 | RULE CANDIDATE (9) — 5th branch, first non-numeric instance. Propose §10. Detail: topic file |
...
| Verification gate's pass condition is empty result — fails open on malformed input | 5 | 2026-09-09 | RULE CANDIDATE (5), THRESHOLD-CLEARED. Detail: cross-agent-lessons.md (row 613) |
...
| Correct advice with invented rationale (correct conclusion shields false WHY) | 4 | 2026-09-07 | RULE CANDIDATE (4). Propose: agent-critic.md verifies stated mechanism independently. Detail: topic file (row 638) |
...
| Derivation query replacing an open-set enumeration is unverified before publish (false negative) | 2 | 2026-09-02 | RULE CANDIDATE (2) → code-style.md §10 cl.2 addendum. Detail: topic file (row 634) |
...
| Corrected claim partially retracted — old wording persists elsewhere | 5 | 2026-09-02 | RULE CANDIDATE (5) — a paraphrase survived cl.3's own grep. Propose: cl.3 also greps subject/keywords. Detail: topic file |
| Mirror-sync grep misses a mirror on the wrong axis | 2 | 2026-08-24 | RULE CANDIDATE (2) → grep by name/path too (row 653) |
| Subagent asserts a verification/write it did not perform — evidence invented, conclusion mostly true | 15 | 2026-09-10 | PROMOTED → agent-workflow.md § Finding Validation. Naming the failure didn't prevent recurrence; a pasted-output requirement did. Detail: topic file |
| Rules-file claim true in its hunk, false vs another section/mirror/arithmetic | 21 | 2026-09-10 | RULE CANDIDATE (21) — cl.6 taken by SHA-citation gate; propose cl.8. Detail: topic file |
...
| Orchestrator drafts its own unverified "because X"/attribution claim in comment prose | 4 | 2026-09-01 | RULE CANDIDATE (4) — enforcement gap: escaped impl-critic + full cycle. Detail: topic file (row 683) |
...
- CORROBORATION (row 663): reminders caught 0/3 recurrences; artifact re-check caught 3/3. Topic file.
