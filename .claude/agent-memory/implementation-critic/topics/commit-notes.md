...
## row detail 2026-08-09 (compaction spill from MEMORY.md)

**Behavior-change fix with no regression test for the NEW behavior** (count=2, both on #1124 — same PR, different commits, so held at WATCHING; do not promote off one PR).
1. `session-bootstrap-load.ts` Promise.all→await-questions-first: every existing test passed on the OLD code, so a revert is invisible. When a fix changes TIMING/ordering rather than output, demand a case that FAILS pre-fix (fake timers + a hung sibling promise).
2. CR-round-1 fixup added a `.catch` net to `use-session-bootstrap.ts:52` mirroring the one in `buildRecoveryResume` — but the SIBLING net is tested (`session-bootstrap-load.test.ts:290`, throwing-setter technique) and the new one is not.
**Generalized check:** when a fix is justified as "mirrors the sibling pattern", check whether the sibling's TEST was mirrored too — a defensive net no test can fail is a comment, not a guard. Mocked seams often make the new branch unreachable (here `loadSessionData` is real and never rejects, so only a throwing mocked setter — `mockClearSessionHandoff.mockImplementationOnce` — reaches it).

**Doc or code comment vs migration body** (PROMOTED → code-style.md §10; count=3, rule written in PR #1162, whose own footer records count=3, #1152). Applies to CODE COMMENTS, not just `docs/*.md`. 3rd instance: #1124 `study-start-handlers.ts` JSDoc claimed an orphan discovery row "would otherwise block the retry", but migs 137/141/138/139/140 — every start RPC — unconditionally soft-delete the caller's active `mode='discovery'` rows in step (1) → self-healing on all paths; `study.ts` says the opposite ("auto-cleared by the next start").

**Rules-file footer drift** (2026-08-08, PR #1162): `agent-coderabbit-local.md`'s `Last updated` footer was bumped for a one-clause wording change while `code-style.md`'s was left stale after that same commit added a normative §5 subsection and rewrote §10. Distinct from cross-file prose divergence (that is rule TEXT drifting; this is the CHANGELOG footer omitting a substantive change, so a later provenance audit finds no record). Caught pre-commit by impl-critic. On a 2nd occurrence: propose a rule that any commit touching a `.claude/rules/*.md` normative section updates that file's own footer.

**Playwright `getByRole('dialog')` on a Base UI AlertDialog** (2026-08-09, #815/#367 re-enabled delete spec): Base UI `AlertDialog.Root` → `useRenderDialogRoot('alert-dialog')` sets `role='alertdialog'`; Playwright `queryRole` is strict equality (`getAriaRole(el) !== options.role`, playwright-core 1.62 `coreBundle.js`) — no role-superclass matching, so the locator never resolves and the assertion fails deterministically. House pattern is `getByRole('alertdialog')` (`quiz-session-recovery.spec.ts:136`, `exam-recovery.spec.ts:42`). Check which primitive the component uses (`components/ui/alert-dialog.tsx` vs `dialog.tsx`) before approving a dialog-role locator.

**App-layer integration test reusing a REAL seeded reference code** (2026-08-09, `soft-delete-question.integration.test.ts`): passed `subjectCode: '050'`/Meteorology. `seedReferenceData` upserts `onConflict: 'code'`, so it MUTATES the seed-e2e row (`short` MET→'050') and, with no `cleanupReferenceData` in `afterAll`, leaves a global `easa_topics` row behind every run. All 20 sibling app-layer integration files use a unique suffixed code + `cleanupReferenceData` gated on `errors.length === 0`. Fix order matters: change the code FIRST — cleaning up while the code is still `'050'` would try to DELETE the real subject (FK is RESTRICT, so the teardown throws).
