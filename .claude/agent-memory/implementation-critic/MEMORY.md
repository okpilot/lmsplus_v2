...
| A test's "outside oracle" is co-modifiable with the value it checks — same `execFileSync('git',['ls-files',...])` shape, one edit narrows both, undetected | 2026-09-08 | 1 | 2026-09-08 | WATCHING. `pipeline.test.mjs` `tracked`-vs-`full`: co-narrowing both to one pathspec keeps them equal, 92/92 green. Comment's "cannot narrow" claim contradicts its own next sentence. **Oracle is only outside if a DIFFERENT mechanism produces it.** † |
...
| Doc **or code comment** describes behavior the authority (migration body, RLS POLICY, CHECK constraint, or CONTENT CORPUS) contradicts | 2026-06-06 | 17 | 2026-08-20 | PROMOTED → code-style §10. **A comment JUSTIFYING a gap is the highest-value claim to falsify. 17th: ADVICE was right (use `adminClient`), stated MECHANISM was invented (a "planner unreliable" quirk; policy had simply been dropped for recursion).** Instances 15-16 → tracker-archive. †
| Same-commit self-contradiction: a file restates the exact claim ANOTHER file in the SAME staged diff retracts | 2026-08-18 | 10 | 2026-09-07 | RULE CANDIDATE. Grep the diff for the OLD wording, not just the file being corrected. **10th: a tracker row said "test stays green" (present tense) for a case the SAME diff's `pipeline.test.mjs` CLOSES — when the fix is in the same commit, status must be RESOLVED and past-tense.** Grep for every restatement, INCLUDING within the same file. Instances 4–9 → archive. † |
...
| Rule REDEFINED in one surface, siblings left restating the old definition | 2026-07-23 | 8 | 2026-09-07 | RULE CANDIDATE. **Sharpest tell: the commit EDITED one clause of a sentence and left the adjacent clause stating the old rule.** **8th: a per-APPLY→per-ROUND sweep fixed two rules files but missed `.claude/agents/learner.md` — same text, agent-definition file.** Scope a mirror grep by CLAIM, never by extension. Instances 6-7 → tracker-archive. †
| RELATIVE commit reference ("the previous commit", "THIS commit", "this PR") in a DURABLE rules file — true at authoring, FALSE one commit later | 2026-08-19 | 3 | 2026-09-07 | RULE CANDIDATE. **Name the SHA or issue #; a rules file has no "now".** 2nd: the FIX kept "the previous commit on this branch". 3rd: agent-workflow.md added "this PR also introduced" when the SAME paragraph already correctly used "#1254" one sentence earlier — the relative form is the anomaly. †
| A GUARD/CEILING added to protect a claim makes that claim unreachable or unrunnable | 2026-08-19 | 3 | 2026-08-20 | RULE CANDIDATE. **Construct the case and RUN the check.** **3rd: `test.describe.configure({ mode: 'serial' })` pinned declaration order but ALSO skips every test after the first failure (Playwright types L3638), falsifying a mutation-check split recorded in the SAME diff. `mode: 'default'` pins order with no skip cascade.** Instances 1–2 → tracker-archive. †
| Claim-correction commit introduces a NEW wrong count/label/ENUMERATION | 2026-08-15 | 14 | 2026-08-20 | RULE CANDIDATE. Re-derive the SET; **de-quantify** rather than patch the numeral. **14th: cited `docs/security.md` §11 for multiple-permissive RLS — that's §3, a cross-file numbering leak. Cite the rule TITLE, never a cross-file number.** Instances 6–13 → tracker-archive. †
...
| Hook-addition commit updates ALL named registration surfaces EXCEPT `.spec-workflow/steering/tech.md`, which explicitly enumerates commit-msg gates by name | 2026-09-14 | 1 | 2026-09-14 | RESOLVED in the same commit (`ff562c9f`). The pattern DID occur — ten surfaces were updated and `tech.md` was missed — so the count stands; the finding itself is closed. Fixed by making the commit-msg entry DEFER to `.claude/pipeline.json`, the move the pre-commit entry had already made after going stale once, rather than adding a fourth hand-maintained enumeration. Rule-Mirror Sync table: `.spec-workflow/steering/**` is ALWAYS live. |
| A test file's per-test MUTATION comment claims the test pins a mechanism the SAME file's preamble lists as NOT PINNABLE — contradictory in the same commit | 2026-09-14 | 1 | 2026-09-14 | WATCHING. `check-retracted-phrase.repo.test.mjs`: preamble says "latin1 decode is NOT pinned (Node re-encodes to UTF-8)"; test body for `two paths differing only in an invalid byte stay distinct` says "This is the case that pins the decode" + mutation comment describing U+FFFD collapse that doesn't apply to valid-UTF-8 fixtures. Executed: changing latin1→utf8 does NOT cause path collapse. |
