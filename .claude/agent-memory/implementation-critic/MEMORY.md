...
| `git show --stat <sha> -- <path>` absolute claim in rules text is false for MERGE commits | 2026-09-06 | 1 | 2026-09-06 | WATCHING. A rules-doc fix prescribing "EMPTY stat means THAT commit did not touch the path" is unqualified — merge commits use combined diff (files only appear if differing from ALL parents), so `git show --stat <merge> -- <path>` is EMPTY even when the merge brought the file in. Demonstrated: real repo merge `6e968f37` added 41 lines to `patterns.md`; path-filtered stat = empty. Throwaway-repo confirmation: merge creating `branch.txt`; path-filtered stat = empty. APPLIED in `2b8adecf`: the rule now prescribes `--diff-merges=first-parent` for merge commits. Low practical impact (agents don't make merge commits), but the text makes an absolute claim. |
