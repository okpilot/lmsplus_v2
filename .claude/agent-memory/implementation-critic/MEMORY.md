...
| Behavior-change fix to an already-tested util lands with no regression test for the NEW behavior | 2026-08-07 | 1 | 2026-08-07 | WATCHING. #1124 `session-bootstrap-load.ts`: Promise.all→await-questions-first (so a known error no longer stalls 3s behind the flag timeout) — every existing test still passed on the OLD code, so a revert is invisible. When a fix changes TIMING/ordering rather than output, check the co-located test file has a case that fails pre-fix (fake timers + a hung sibling promise). |
