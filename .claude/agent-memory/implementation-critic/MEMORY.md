...
| Gate keyed on a PARTIAL identity — `topic_code` alone where pool identity is `subject_code`+`topic_code` | 2026-08-18 | 2 | 2026-08-18 | RULE CANDIDATE. 2nd: `--replace` orphan scope = bank+topic+question_type, but a POOL is one FILE — 3 P3_MC MC files share that scope, so one file's `--replace` soft-deletes the other two (16 rows, run reports success). Empirically confirmed. **When a new query defines a set to DELETE, enumerate the shipped content files that land in that set.** |
| Docstring rationale contradicted by an inline comment 10 lines below it, both written in the SAME commit | 2026-08-18 | 2 | 2026-08-18 | RULE CANDIDATE → §10. 2nd (r2, same file): `planReplaceAll`'s NEW docstring says "**Per-file** … diffs against **the file's own** nums via `planReplace`, soft-deletes ORPHANED" 10 lines above its own body comment "Planning **per FILE was the defect**". Read the whole block after retargeting a caller. |
...
| Doc **or code comment** describes behavior the authority (migration body, RLS POLICY, or CONTENT CORPUS) contradicts | 2026-06-06 | 8 | 2026-08-18 | PROMOTED → code-style §10. 7th widened the class beyond SQL (content corpus). 8th: the EXTRACTION-justifying docblock of a comment-accuracy commit's OWN new file — `parse-import-args.ts` says import-questions.ts "builds a Supabase client at module load, so importing it from Vitest opens a real connection"; the client is built inside `main()` AFTER argv+JSON validation, `main()` is merely INVOKED at module scope, and `parseArgs`'s `process.exit(1)` fires first, so no client is ever built. **A "why this exists" docblock is a claim like any other — trace it.** † |
| Same-commit self-contradiction: a comment restates the exact claim ANOTHER file in the SAME staged diff retracts | 2026-08-18 | 2 | 2026-08-18 | RULE CANDIDATE. Grep the diff for the OLD wording, not just the file being corrected. 2nd: code-reviewer MEMORY asserts "#1219 CLOSED" while the doc-updater MEMORY entry added in the SAME commit states the rule that `gh issue view N` reads OPEN until merge — and #1219 is OPEN (`Closes #1219` sits on unmerged `1335c73d`). **"Closes #N" in a local commit ≠ closed.** † |
