...
| Thin-wrapper page-error tests: mock-dependency form is valid when SUT is pure pass-through | 2026-06-01 | 2 | 2026-06-25 | FALSE POSITIVE. Do NOT flag mock-the-dependency page-error tests as "bypassing production logic" when the mocked helper is the ONLY fetch path (§7 permits it). † |
...
| Doc **or code comment** describes DB guard behavior that doesn't match the migration body | 2026-06-06 | 4 | 2026-08-09 | PROMOTED → code-style.md §10 (rule written in PR #1162, #1152). Verify guard-order/exemption/caller claims against the LATEST migration body — applies to CODE COMMENTS too, not just `docs/*.md`. **4th instance (#1167 fixup): the chain-trace obligation extends to RLS POLICY migrations, not just `CREATE OR REPLACE FUNCTION` bodies** — three comments asserted the proxy's `users` lookup does not filter `deleted_at`, but `users_select` (mig `20260312000012:14-16`, FORCE RLS table, anon-key client) already scopes it. Two of the three contradicted a filter added in their own commit. †
...
| plan.md integration-test count wrong: bad baseline propagated to a new "now N" claim | 2026-06-11 | 2 | 2026-06-11 | FALSE POSITIVE. The plan.md count literal is the VITEST RUNTIME total, NOT a static `it(` grep — verify via the run summary. † |
...
| Agent-memory curation: stub rows with archive pointer require the archive to hold the entry | 2026-06-22 | 1 | 2026-06-22 | WATCHING. Before flagging a stub-row-with-no-archive-match: fuzzy grep the archive + check other suffix forms (#948: 9 apparent misses all resolved). † |
...
| Integration fixture type changed to satisfy a new write-time trigger, keeping the original CHECK active | 2026-06-24 | 1 | 2026-06-24 | WATCHING. #828. When a new trigger rejects a constraint-regression fixture, retarget to a trigger-allowed type so the CHECK stays the constraint under test. † |
| Status/error-posture change updates some test tiers but leaves a sibling spec asserting the OLD value | 2026-06-26 | 2 | 2026-08-09 | RULE CANDIDATE. `study-queries.ts` return-[] vs throw (#CI fail); #1167 proxy 403→307 updated proxy.test.ts + both redteam specs but MISSED `e2e/admin-students.spec.ts:491` `toBe(403)`. Grep the WHOLE repo for the old literal, not just the plan's named files. † |
...
| Cross-file prose divergence: rule clarified in one doc, sibling doc left contradicting | 2026-07-23 | 1 | 2026-07-23 | RESOLVED (#1137). When clarifying a rule phrase, grep sibling command/rule files for it and fix in the SAME commit. |
| Rules-file edit bumps one rules file's `Last updated` footer but not a sibling's, same commit | 2026-08-08 | 1 | 2026-08-08 | WATCHING. #1162. Footer omits a substantive change → provenance audit finds no record. Distinct from cross-file prose divergence (that is rule TEXT drifting). † |
| Playwright `getByRole('dialog')` used on a Base UI **AlertDialog** (role=`alertdialog`) | 2026-08-09 | 1 | 2026-08-09 | WATCHING. #815/#367. Playwright `queryRole` is strict role equality — no superclass matching, so the locator never resolves. House pattern: `getByRole('alertdialog')`. Check which primitive the component uses before approving a dialog-role locator. † |
| Redirect target copied from a sibling helper without checking the route EXISTS (no `page.tsx`) | 2026-08-09 | 1 | 2026-08-09 | WATCHING. #1167 proxy admin-block 403→`redirect('/app')`, copied from `requireAdmin()` — but `app/app/` has only `layout.tsx`, no `page.tsx`, so `/app` renders Next's default 404. Swaps one bare page for another; objective unmet. Before approving any new redirect literal, `find app -path '*<seg>/page.tsx'`. |
| New app-layer integration test reuses a REAL seeded reference code + omits `cleanupReferenceData` | 2026-08-09 | 1 | 2026-08-09 | WATCHING. `soft-delete-question.integration.test.ts` used `subjectCode: '050'` — `seedReferenceData` upserts `onConflict: 'code'`, mutating the seed-e2e row + leaking a global `easa_topics` row. All 20 siblings use a unique suffixed code + `cleanupReferenceData`. † |

## Durable knowledge

- **CREATE OR REPLACE trace before flagging.** Trace to the LATEST migration definition before flagging a missing pattern on a Postgres function. Grep BOTH `CREATE OR REPLACE FUNCTION <fn>` AND bare `CREATE FUNCTION <fn>` (DROP+recreate form) — a CREATE-OR-REPLACE-only grep returns a superseded file as "latest". Required per `agent-critic.md`.
...
- **Every start RPC auto-clears the caller's active `discovery` row** (migs 137/141/138/139/140, unconditionally, before the single-active guard). An orphaned discovery row can never strand a user; client-side teardown is hygiene, not correctness. Reject any comment claiming an orphan "blocks the retry". †
...
- **Bounded-await helpers never reject.** The `Promise.race([p.then().catch(()=>fallback), timeoutResolve]).finally(clearTimeout)` shape (code-style §6, e.g. `fetchFlaggedIdsBounded`) cannot reject or leak its timer — `.finally` is attached at CREATION, not at the await site. Abandoning such a promise is safe; do NOT flag it.
...
- **Playwright project ordering = dependency-depth PHASES, not config order.** Adding ONE `dependencies:` edge re-partitions EVERY project's phase; projects inside a phase interleave with no defined order even at `workers: 1`. Check the resulting partition, not just the edge (#1143). †
