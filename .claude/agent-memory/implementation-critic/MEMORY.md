...
| A fix REPLACES the function a comment names, but the comment set still names the OLD one | 2026-08-18 | 1 | 2026-08-18 | WATCHING → §10. `import-vfr-rt-content.ts` has ZERO calls to `planReplace` (imports `planScope` only) yet 4 newly-ADDED `+` comment sites cite `planReplace` as the live decision path. **Grep the retired symbol across the diff's own added lines, not just the changed code.** |
| A behavioural gate is added but the operator-facing docblock/Usage block never learns the new flag | 2026-08-18 | 1 | 2026-08-18 | WATCHING. `--prune` (the abort that fixed the CRITICAL) appears nowhere in the file header spec or its `Usage:` invocation list; the header still describes the pre-fix auto-soft-delete. |
| A warning a message tells the operator to look for became structurally unreachable on the default path | 2026-08-18 | 1 | 2026-08-18 | WATCHING → §10. Drift warning says "Check above for a 'matched no existing rows' warning; its absence means only SOME rows drifted" — `findReplaceTargets` now runs only under `--prune`, so absence is the norm and carries no signal; and the sibling comment credits `findLiveNumbersInScope` with a warning it never emits. |
