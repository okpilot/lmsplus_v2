...
| Same-commit self-contradiction: a comment restates the exact claim ANOTHER file in the SAME staged diff retracts | 2026-08-18 | 1 | 2026-08-18 | WATCHING. New test comment cited "content-ids.ts docblock" for the prefix-is-the-whole-disjointness-guarantee sentence that this diff deletes from that docblock. On any claim-correction commit, grep the diff for the OLD wording, not just the file being corrected. † RESOLVED-WATCH: closed in round 2 — the fixture switched to the `o`/`l` pair the comment identifies as load-bearing, so the retracted z/l framing is gone from both the title and the body rather than merely disclaimed. |
