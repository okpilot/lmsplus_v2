...
| Behavior-change fix to a tested util ships with no regression test for the NEW behavior | 2026-08-07 | 3 | 2026-08-18 | WATCHING (→ RULE CANDIDATE: count=3, first cross-PR occurrence on `content/vfr-rt-part3` — `assertOptionTextsDistinct` key swapped to `normalizeForId`, probe spec removed, `mc-content.test.ts` has no interior-whitespace dup case). Needs a case that FAILS pre-fix. † |
...
| Doc **or code comment** describes DB guard behavior the migration body contradicts | 2026-06-06 | 6 | 2026-08-18 | PROMOTED → code-style §10. Covers code comments and RLS POLICY migrations. 6th: `docs/plan.md` named the red-team E2E fixture helper `buildVfrRtAnswers` as "the grader" blocking a sampler widening; the real blocker is `submit_vfr_rt_exam_answers` (latest def mig `20260815000300` L241 `RAISE 'unsupported_question_type'`). Claim propagated verbatim from issue #1216 — §10's don't-propagate clause. † |
| Same-commit self-contradiction: a comment restates the exact claim ANOTHER file in the SAME staged diff retracts | 2026-08-18 | 1 | 2026-08-18 | RESOLVED-WATCH. History: first seen when a new test comment cited "content-ids.ts docblock" for the prefix-is-the-whole-disjointness-guarantee sentence that this diff deletes from that docblock. On any claim-correction commit, grep the diff for the OLD wording, not just the file being corrected. Closed in round 2 — the fixture switched to the `o`/`l` pair the comment identifies as load-bearing, so the retracted z/l framing is gone from both the title and the body rather than merely disclaimed. |
