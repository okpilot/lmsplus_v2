...
| Doc asserts an issue is CLOSED/resolved while `gh issue view` reports OPEN — closure only pending an UNMERGED local `Closes #N` | 2026-08-18 | 2 | 2026-08-19 | RULE CANDIDATE. **Run `gh issue view <N> --json state` before writing any closed/resolved/fixed claim.** 2nd: `docs/plan.md` L1483 "#1194 … is CLOSED as decided" on an unpushed branch whose `Closes #1194` sits in local commit `2c39367b`; #1194 was OPEN. Self-resolves at merge, so the hazard is a squash that drops the trailer — then the doc asserts a false state permanently. Prefer "closed by this branch". Distinct mechanism from the same-commit self-contradiction row above (authority is GitHub, not a sibling file). |
