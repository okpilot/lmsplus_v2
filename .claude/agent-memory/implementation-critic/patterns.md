...
### 2026-05-27 — issue #668 instance #5: get_admin_dashboard_students RPC (#682) — APPROVED

- Migration: SECURITY DEFINER + SET search_path = public + auth.uid() null-check + is_admin() guard + org from auth.uid(). Dynamic ORDER BY safe (p_sort whitelisted via CASE; p_dir normalised to 'ASC'/'DESC'; only v_order + hardcoded identifiers reach EXECUTE). count(*) OVER() total_count. GRANT EXECUTE with exact signature (TEXT,TEXT,TEXT,INT,INT). DROP FUNCTION IF EXISTS for old get_admin_student_stats. §9 exception comment present and accurate (admin-only, org-scoped, p_status controls visibility). session_stats CTE: deleted_at IS NULL on quiz_sessions (correct). mastery_stats CTE: no deleted_at filter on student_responses (correct — immutable table, no such column). Confirmed pattern via migration 20260406000002 comment.
- queries.ts: single authRpc call. organizationId correctly removed from getDashboardStudents destructure only; getRecentSessions still has organizationId — adminClient query preserved. Local Row type correctly annotates nullable fields (string | null, number | null). Array.isArray cast guard present. console.error + generic throw (no raw error.message to client). totalCount from Number(rows[0]?.total_count ?? 0). Return shape unchanged.
- queries.test.ts: makeFromChain retained and used for getRecentSessions tests (no dead helper). Old makeStatRow / makeUserRow / mockStudents correctly removed; mockStudentsRpc replaces them. BASE_FILTERS constant introduced to avoid repetition. Param-forwarding tests cover p_status (null, 'active', 'inactive'), p_sort+p_dir, p_limit+p_offset. totalCount tests cover window value and out-of-range page. Error + non-array guard tests present. No dead code.
- types.ts: avg_score/full_name/last_active_at/deleted_at typed non-nullable (consistent with existing admin RPC entries like get_admin_student_stats which also used number/string for nullable SQL cols). Production queries.ts uses authRpc wrapper with its own local Row type that correctly captures nullability — types.ts entry is not exercised at runtime. Pattern is established codebase convention, not a deviation.
