...
### [2026-05-28] UNIQUE constraint widening breaks existing ON CONFLICT (column-list) DO NOTHING in living RPCs
When a plan widens a UNIQUE constraint on an immutable answers table (e.g., `quiz_session_answers` UNIQUE(session_id, question_id) → UNIQUE(session_id, question_id, blank_index)), every existing SECURITY DEFINER RPC that uses `ON CONFLICT (session_id, question_id) DO NOTHING` becomes a runtime error — Postgres requires an exact matching unique constraint for that ON CONFLICT form. The plan's migration list must include a companion migration that rewrites the live RPC's ON CONFLICT clause to match the new constraint. This was missed for `batch_submit_quiz` and `submit_quiz_answer` when mig084 widened the constraint. Always grep for `ON CONFLICT (` across all migrations to find all callers before widening any unique constraint.

### [2026-05-28] New question type columns must account for existing NOT NULL columns on the base table
When adding new question-type-specific columns to `questions`, plans must check all NOT NULL columns that existing code inserts. `questions.options` is `JSONB NOT NULL` with no DEFAULT. A plan that adds `short_answer` and `dialog_fill` types without either (a) making `options` nullable, or (b) adding `DEFAULT '[]'::jsonb`, or (c) documenting that the upsert action must pass `options: '[]'` for non-MC types, will produce silent insert failures or require undefined behavior. Always list every NOT NULL column on the base table and document how each is satisfied for every new type variant.

### [2026-05-28] Plans referencing a DO-block CHECK constraint lookup pattern must verify whether the constraint was given a name by an intermediate migration
The DO $$-block pg_constraint lookup pattern (searching by predicate text) is needed only for unnamed constraints. Migration 058 (`058_extend_quiz_sessions_mode_check.sql`) gave the mode CHECK a canonical name `quiz_sessions_mode_check`. Any subsequent plan that needs to widen this constraint should use `ALTER TABLE ... DROP CONSTRAINT quiz_sessions_mode_check` + `ADD CONSTRAINT ... CHECK(...)` — not the DO-block lookup. Plans citing the "DO-block lookup pattern" without tracing the full migration chain to check for an intermediate naming migration will produce unnecessary and misleading complexity.

### [2026-05-28] Phase A migration sets for new exam modes must include a get-questions RPC for the new question types
When a new exam mode introduces question types whose student-facing shape differs from the existing MC shape (new types that have a `dialog_template` or `canonical_answer` that must NOT be returned to the client), Phase A migrations must include a migration for a new/extended `get_*_questions` RPC that strips correct-answer fields. The security requirement (never expose canonical_answer / blanks_config canonical / accepted_synonyms to students) cannot be satisfied by the existing `get_quiz_questions()` which only strips MC correct-option markers. Missing this migration means the student UI either has no safe read path or must select directly via PostgREST (which exposes answer columns). Always check: "for every new column on questions that holds a correct-answer signal, is there a server-side projection that strips it?"
...
### [2026-05-29] Agent-tooling plans: Phase 0 spikes that cannot distinguish native-feature engagement from body-instruction compliance

When a plan's Phase 0 spike asks "spawn the agent and confirm it (a) reads MEMORY.md from injection and (b) writes back", the verification method is unverifiable if the agent body also contains an explicit Read/Write instruction for the same file. The agent will write to the file whether or not native injection engaged — the spike produces a false positive. To isolate the variable: strip the explicit body Read/Write instruction before the test run. Only if the agent still reads and references MEMORY.md content (that was not in the orchestrator's prompt) can engagement be confirmed. First seen: agent-memory refactor plan (2026-05-29).

### [2026-05-29] Agent-tooling plans: "delete journal bodies, git is archive" loses load-bearing boundary-condition evidence

For learner-style journals with tracker rows at count ≥ 2, deleting journal bodies removes the mechanistically distinct examples that establish boundary conditions. A tracker row with "count: 5, PROMOTED" plus a topic-file rule statement does not preserve the five distinct commit+mechanism examples that motivated promotion. Future agents reading only the topic file cannot reconstruct why the rule has the shape it does. Migration must extract at least one example per count increment for any tracker row at count ≥ 2 with mechanistically distinct occurrences. First seen: agent-memory refactor plan (2026-05-29) — "Partial fix applied to sibling file group" (count 5) and "Auth callback guard ordering error" (count 2) entries in learner/patterns.md are representative cases.

### [2026-05-29] Agent-tooling plans: files that are reference matrices (not journals) must not be renamed to MEMORY.md if the curation nudge can delete active rows

`attack-surface.md` (red-team) is a vector-to-spec mapping table with 30+ rows — many marked GAP or PASSING. Native injection's curation nudge ("curate MEMORY.md if >200 lines") is semantically dangerous for this file: an LLM agent receiving a trim-down instruction may prune GAP rows that represent active coverage gaps. Plans that include attack-surface.md in a bulk "rename to MEMORY.md" sweep must add an explicit no-delete rule to the file header and document that vector rows transition status (GAP→PASSING→FIXED) but are never deleted. Additionally, insights.md hardcodes "attack-surface.md" as an operational reference — that reference must be updated in the same commit as the rename.

### [2026-05-29] Agent-tooling plans: body-level "Read your memory file at start" instructions become redundant double-loads after native injection is enabled

Several agents have explicit unconditional Read instructions in their bodies (e.g., test-writer.md line 78: "Read that file at the start of every invocation"). After `memory: project` is added to frontmatter, native injection delivers the same content automatically. Phase 1 must enumerate and remove these per-agent body instructions or the agent redundantly reads the file twice. Check every agent whose body contains "Read … patterns.md" or "Read that file at the start" before committing frontmatter changes. First seen: agent-memory refactor plan (2026-05-29) — test-writer, code-reviewer, plan-critic, implementation-critic, security-auditor all have explicit body references.
...
### [2026-05-28] fetchAllRows contract: data is always T[], never null — test mocks must use { data: [], error } not { data: null, error }

When a plan introduces `fetchAllRows` as a delegate and rewrites tests, the existing Supabase chain mocks often use `{ data: null, error: {...} }` to simulate a DB error. This shape is NOT the contract of `fetchAllRows`, which always returns `{ data: T[]; error: ... | null }` — data is guaranteed non-null. The plan must specify that mocked helpers return `{ data: [], error: { message: '...' } }` for error paths, not `{ data: null, ... }`. Using the null shape in the new mock is a latent type bug (TypeScript accepts it through `vi.fn()`) but exercises an impossible production state. First seen: #668 instance #7 (comment-queries.ts).

### [2026-05-28] fetchAllRows with Supabase join in select string requires explicit cast at getPage return site

`fetchAllRows<T>` requires `getPage` to return `PromiseLike<PageResult<T>>`. When the `getPage` closure's `.select()` string includes an embedded resource join (e.g. `users(full_name, role)`), Supabase's TypeScript codegen returns a structured join type that is NOT assignable to `PageResult<ManuallyDeclaredRow>` without a cast. Plans must explicitly specify the cast approach — either `as unknown as PromiseLike<PageResult<T>>` on the closure return, or match the `CommentRow` type precisely to Supabase's inferred join shape. The GDPR precedent avoids this entirely (no joins in any paginated select). First seen: #668 instance #7 (comment-queries.ts with users join).

### [2026-05-28] Companion query modules extracted from utility files may leave the parent file still above the 200-line limit

When a plan extracts a function from a large utility file (e.g., queries.ts at 296 lines) into a new companion module, the plan must verify the post-extraction line count against the 200-line utility cap. Removing a 25-line function from a 296-line file still leaves ~271 lines — above the limit. Plans must either acknowledge the remaining violation (and defer it explicitly) or scope a second split. Without this, the code-reviewer blocks the commit. First seen: #668 instance #7 (queries.ts ~269 lines after listOrgStudents removal).

### [2026-05-28] Query helper modules extracted from 'use server' files must not include 'use server'

When a plan extracts a query helper into a companion module and the source file has `'use server'`, plans consistently omit an explicit note that the new companion file must NOT include `'use server'`. The companion module accepts a `supabase: SupabaseClient` parameter (not bound server action semantics). Including `'use server'` in the companion file causes Next.js to treat its exports as Server Actions and breaks runtime calls from other Server Actions. The GDPR precedent (collect-user-data-queries.ts) never includes the directive. Plans must state this explicitly. First seen: #668 instance #7 (comment-queries.ts).

### [2026-05-28] Test rewrites for Server Action delegation must enumerate which existing tests are affected vs. unaffected

When a Server Action is refactored to delegate a DB query to a helper, the test file contains tests at multiple layers: (a) tests that short-circuit before the query (auth checks, Zod validation) — these are unaffected by the helper mock and must NOT have their infrastructure changed; (b) tests that exercise the query path — these must be rewritten. Plans that say "rewrite N tests" without enumerating which tests are in each category cause implementers to either over-mock (adding helper mocks to validation tests) or under-mock (missing the query-path tests). Plans must list the tests by name or describe-block membership. First seen: #668 instance #7 (getComments — 4 non-query tests + 3 query tests, plan said "4 tests" without the category split).
...
### [2026-05-29] TS→SQL aggregation move with stale test-helper artifacts (SUGGESTION only)
`#668 instance #8` plan for `get_student_profile_stats` correctly identified all callers, verified §11 multi-permissive-RLS scoping via explicit `WHERE qs.student_id = auth.uid()`, preserved behavioral equivalence (COUNT/AVG over the identical `score_percentage IS NOT NULL` filtered set), confirmed NUMERIC AVG serializes as string and planned Number() coercion. One SUGGESTION: the plan specifies removing `SessionRow` from profile.ts and the `quiz_sessions` branch from `setupMocks`, but does not explicitly list removing the `sessions` field from the `FromSetup` type alias and its default array in the test helper function. These are dead artifacts that TypeScript will not catch (optional field). Plans for this class of TS→SQL moves should enumerate ALL artifacts to remove from test helper types, not just the production-code type and the mockFrom branch.
