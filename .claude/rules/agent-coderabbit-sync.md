...
> Model: haiku | Trigger: once per branch, after the review loop ends, when the branch diff matches § Trigger Conditions | Non-blocking
...
Runs ONCE per branch, after the review loop ends — and only when the above files are in the branch diff. A rules-file fixup committed during the loop can itself put a rules file in scope, so run it after those edits land; any `.coderabbit.yaml` change it prompts must be committed BEFORE the push.

## Handling Results
### DO
- Run after the review loop ends, once any rules-file fixups are on disk — they can themselves trigger the sync.
