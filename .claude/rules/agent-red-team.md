...
> Model: sonnet | Trigger: once per branch, after the review loop ends, when the branch diff matches § Trigger Conditions | Non-blocking
...
- The agent has no Write/Edit tool: it REPORTS the rows `apps/web/e2e/redteam/attack-surface.md` needs added or changed, and the orchestrator applies them — the same report-then-apply pattern as doc-updater.
...
- Let the agent edit `apps/web/e2e/redteam/attack-surface.md` itself — it has no Write/Edit tool; report rows, the orchestrator applies them.
