...
- The soft-delete **column-existence guard** (`.claude/hooks/check-soft-delete-guard.mjs`, code-style.md §5) mechanically blocks `.is('deleted_at')` on the seven no-soft-delete tables in PRODUCTION code — but red-team spec files are NOT covered by it, so the "read the migration" rule above still applies when authoring a spec that asserts soft-delete behavior.
