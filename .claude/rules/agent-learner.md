...
- **`code-review (skill)` findings — the built-in `/code-review` skill, dispatched forked, round 1 only — are ORDINARY round-1 learner input**, pooled with the other round-1 reviewers' and counted toward rule promotion on the same terms. This is NOT the cloud-CodeRabbit case in the next bullet: that reviewer runs after the gate ends; this one runs inside round 1.
