...
> Model: **opus** — named exception to `agent-critic.md § Model tier` | Dispatched as a subagent in an isolated worktree | Trigger: EVERY round of the pre-push review gate | Blocking: on CRITICAL/ISSUE
...
## Trigger Conditions
- **EVERY round**, in the same parallel batch as the round's other members, on the same range they read.
- Round-1-only was a conservative default and did not survive its own measurement (Decision 77).
...
- Hand every round's table to the branch's learner run — ordinary learner input (`agent-learner.md`).
