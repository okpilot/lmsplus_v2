...
- Its brief is its template in `.claude/hooks/gate-briefs.json`; `guard-agent-brief.js` blocks any other, the brief sent to any other recipient, and a dispatch without `isolation: "worktree"` or with a model other than its `.claude/pipeline.json` alias.
