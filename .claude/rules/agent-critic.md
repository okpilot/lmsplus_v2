...
- **Minimum consecutive-clean floor (stability rounds).** The gate is NOT satisfied by one clean round:
  ...
  - **N = 3** when the plan/diff touches a security path — the objective trigger set from `agent-workflow.md § Red-Team Agent Trigger` (`supabase/migrations/`, `packages/db/`, `apps/web/app/app/quiz/actions/`, `apps/web/app/auth/`, `apps/web/proxy.ts`, `docs/security.md`), determined via `git diff --name-only`.
...
*Last updated: 2026-06-20 (added Multi-Round Review Discipline — consecutive-clean floor N=2/3 + ceiling 4→escalate for plan-critic & post-commit reviewers; impl-critic exempt)*
