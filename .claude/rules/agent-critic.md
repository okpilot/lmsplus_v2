...
- **Model tier — EVERY subagent runs Sonnet (or Haiku for mechanical checks), with ONE named exception: `code-review (skill)` runs **opus** (`agent-code-review.md`). Otherwise Opus is the orchestrator, and only the orchestrator. No security-path exception.** Where a security diff needs Opus-grade scrutiny, the ORCHESTRATOR reads it itself.
