...
`gh label list` is the source of truth if the label set has moved. `ops` marks work no PR can close — infra, DNS, vendor tiers, tokens, compliance, production script runs. `product-decision` marks work blocked on scoping rather than engineering capacity. **Both excluded when counting the engineering backlog.**

### Board fields are NOT labels
Priority / Size / Sprint above are **project-board fields**. `actions/stale` matches **repo labels only** — a board field value in `exempt-issue-labels` does nothing. `P0 - Critical` exists in two places (a board field value and a label) and **nothing keeps them in sync** — set both when an issue is genuinely a blocker.

### Stale bot
`.github/workflows/stale-issues.yml` marks an issue stale after 30 days of inactivity, closes it 30 days later as `not_planned`. `exempt-issue-labels` skip list: `P0 - Critical`, `bug`, `security`, `ops`, `product-decision`. Every entry must be a label that actually exists.
...
- Use `Closes #N` or `Fixes #N` — GitHub auto-closes the issue, board automation moves it to Done.
...
- Assign to current Sprint if urgent, else leave in backlog.
