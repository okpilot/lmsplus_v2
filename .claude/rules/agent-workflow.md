...
| **Pattern scan** | How similar code is written elsewhere in the repo; for every new helper, query or external-API call, the existing implementation of the same operation (`git grep` the SDK method, table or RPC) | Explore agents: find 2-3 similar files; `git grep` the SDK method, table or RPC of each new operation | Your approach diverges from established patterns, or reimplements an existing operation |
