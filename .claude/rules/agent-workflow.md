...
FINISHED, and that is the gap the record shows: on one branch a mirror sweep came up short four
times, each by a different mechanism, each caught by a different reviewer, with this very section
open in front of the orchestrator. Every one was a good-faith sweep, so the remedy is a terminal
check, not more diligence.
