...
| `.claude/hooks/*.sh` | **executable mirrors** — inspect every matching hook at sweep time for emitted pipeline guidance; they PRINT the agent list at commit time, so a stale one instructs the orchestrator directly. Not `.md`, so every doc-shaped grep misses them. As of 2026-08-19 that meant `post-commit-reminder.sh` and `cr-local-plan-reminder.sh` |
