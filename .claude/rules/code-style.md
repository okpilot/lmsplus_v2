...
4. **Soft-delete, not hard-delete**, when the table has FK children (`student_responses`, `quiz_session_answers`, `flagged_questions`, `question_comments` reference `questions(id)`) — hard DELETE risks `23503` and violates `docs/security.md` rule 6. **Exception:** a table documented as hard-delete-by-design in `docs/database.md` §3 uses `.delete()` — derive that set there, never from a list here. A MISSING `deleted_at` column is a schema fact, never on its own the authorization: `question_comments` HAS the column and is hard-delete by policy, while a soft-delete attempt on a table lacking it errors at runtime (`column "deleted_at" does not exist`).
...
- **`.every()` / `.some()` on a possibly-empty array:** `[].every(pred)` is `true` and `[].some(pred)` is `false` for ANY predicate. An assertion expecting `.every()` to be true, or `.some()` to be false, following an assertion that the same array is empty can never fail — delete it, or guard it with a `length > 0` assertion first.
