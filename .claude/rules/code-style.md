...
2. **Never enumerate an OPEN set — state how to derive it.** A set that can gain a member after
   ...
   config files that commit touched — `.claude/rules/security.md`, `docs/security.md`,
   `.coderabbit.yaml`, `agent-memory.md`, `agent-code-reviewer.md`, this file and `CLAUDE.md`, one
   of them already false — plus the matching entry in `.claude/agents/security-auditor.md`, the
   BLOCKING pre-push gate, per `agent-learner.md`'s Downstream-enforcer sync. One known offender
