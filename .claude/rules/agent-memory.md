...
TERMINAL is `PROMOTED`, `SATURATED`, `RESOLVED` and `FALSE POSITIVE`; `/insights` relocates those from `MEMORY.md` to `topics/tracker-archive.md` to hold the injection budget. `RESOLVED-WATCH` is NOT terminal — only `MEMORY.md` is injected, so archiving one ends the watch it names. Archived rows already carry it, including one this rule's own branch archived in `05832c23`. A COMPOUND row (`PROMOTED … RESOLVED-WATCH`) IS archived — `PROMOTED` is terminal — and archiving ends its RESOLVED-WATCH component. Where the watch must continue, split the row. Derive the set before any sweep: `grep -c RESOLVED-WATCH .claude/agent-memory/*/topics/tracker-archive.md`.
