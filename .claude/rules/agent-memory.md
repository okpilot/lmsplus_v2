...
`/insights` relocates TERMINAL rows from `MEMORY.md` to `topics/tracker-archive.md` to hold the injection budget. `RESOLVED-WATCH` is NOT terminal — only `MEMORY.md` is injected, so archiving one ends the watch it names. Rows already archived carrying it pre-date this; derive them with `grep -c RESOLVED-WATCH .claude/agent-memory/*/topics/tracker-archive.md` before any sweep.
